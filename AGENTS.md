...
This file provides guidance to all coding agents working in this repository. It is the single source of truth for agent instructions; `CLAUDE.md` is a compatibility symlink to this file.
...
**Dependency direction:** `views/ → core/ + ui/`. Core and UI are independent of each other. No package imports from `next/*` or app-specific code.

**Platform bridge:** `packages/core/platform/` provides `CoreProvider` — initializes API client, auth/workspace stores, WS connection, and QueryClient. The web app wraps its root with `<CoreProvider>` and provides a `NavigationAdapter` for routing.
...
- **Web** uses business logic, components, hooks, stores, and views from `packages/core/`, `packages/ui/`, and `packages/views/`.
...
pnpm build            # Build the web frontend and its dependencies
...
- `packages/views/` — zero `next/*` imports and zero stores. Use `NavigationAdapter` for all routing.
...
| Web-specific wiring (cookies, redirects, searchParams) | `apps/web/*.test.tsx` | Needs framework-specific mocks |
| End-to-end user flows | `e2e/*.spec.ts` | Real browser, real backend |

**Never test shared component behavior in an app's test file.** If a test requires mocking `next/navigation` to test a component from `@multica/views`, the test is in the wrong place — move it to `packages/views/` and mock `@multica/core` instead.
...
- In `packages/views/` tests: never mock `next/*` — those APIs don't exist here.
...
- **Every agent must pass frontend lint, frontend tests, and backend tests before creating a commit.** Run `pnpm lint`, `pnpm test`, and `make test`; do not commit while any command is failing.
- `make check` includes the frontend and backend tests plus type checking and Playwright E2E, but it does not include frontend lint. When using it, run `pnpm lint` separately.

## Minimum Pre-Commit Checks

```bash
pnpm lint    # Required frontend lint
pnpm test    # Required frontend tests
make test    # Required backend tests

# Or combine frontend lint with the full pipeline:
pnpm lint
make check   # Typecheck, frontend tests, backend tests, and Playwright E2E
```
