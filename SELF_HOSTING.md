...
>
> **CLI only?** If the self-host server is already running and you only need the CLI on a macOS/Linux machine, run the installer without `--with-server`:
>
> ```bash
...
```bash
helm install multica oci://ghcr.io/lrm-teams/charts/multica \
...
```bash
helm show values oci://ghcr.io/lrm-teams/charts/multica \
...
- **Deterministic local/private testing:** set `backend.config.appEnv: development` in your values file and `MULTICA_DEV_VERIFICATION_CODE=888888` in the Secret, then `helm upgrade` and restart. This fixed code is ignored when `APP_ENV=production`.
  ...
  ```bash
  helm upgrade multica oci://ghcr.io/lrm-teams/charts/multica \
    --version <chart-version> \
    -n multica \
...
```bash
curl -fsSL https://raw.githubusercontent.com/LRM-Teams/multica/main/scripts/install.sh | bash -s -- --stop
```
