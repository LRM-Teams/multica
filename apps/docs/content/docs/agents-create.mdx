...
| **OpenCode** | Open-source, bring-your-own model catalog |

The remaining tools (Kiro CLI, Pi), along with each tool's full capability matrix (session resume, MCP, skill injection path, model selection), are covered in [AI coding tools comparison](/providers).
...
- [AI coding tools comparison](/providers) — full capability matrix across all 7 tools
