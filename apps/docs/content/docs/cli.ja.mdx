...
<Callout type="info">
`list` 系のコマンド（`multica issue list`、`project list` など）は、デフォルトで短く**そのままコピー＆ペーストできる** ID を出力します。イシューは `MUL-123` のようなイシューキー、それ以外のリソースは短い UUID プレフィックスです。以下の後続コマンドの `<id>` 引数は短い ID と完全な UUID のどちらも受け取るため、一般的な流れは `multica issue list` → キーをコピー → `multica issue get MUL-123` となります。正式な UUID が必要なときは `list` コマンドに `--full-id` を渡してください。
</Callout>
...
| `multica attachment view --id <id> --output <path>` | 添付ファイルをローカルパスへダウンロード |
| `multica attachment upload --path <file> [--target '#channel']` | ファイルをアップロードし、返却 id を `--attachment-id` で利用 |
