...
| `multica computer status` | Show redacted identity, environment, channel, and Workspace connections |
...
| `multica computer channel [latest\|alpha]` | Show or select the release channel while stopped |
| `multica computer upgrade [--target-version <version>]` | Upgrade through the selected channel, or to an exact version |
