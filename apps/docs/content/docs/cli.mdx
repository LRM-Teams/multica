...
| `multica computer upgrade [--target-version <version>]` | Upgrade from the current environment's package source, or to an exact recovery version |
...
| `multica update` | Upgrade from the current environment's fixed package source |
