...
The Multica CLI mirrors almost everything the Web UI can do (create [issues](/issues), assign [agents](/agents), start the machine-wide [Computer](/daemon-runtimes), and more). This page lists every top-level command with a one-line description. For the full set of flags and examples, run `multica <command> --help`.
...
| `multica setup /<workspace>` | Connect the machine-wide Computer to one production Workspace |
| `multica setup --environment test --server-url <api-origin> --app-url <app-origin> /<workspace>` | Select the Tencent test environment and connect one Workspace |
...
<Callout type="info">
`list` commands such as `multica issue list` print short, copy-paste-ready IDs by default. The `<id>` argument on follow-up commands accepts either the short ID or the full UUID. Use `multica workspace info --projects` to inspect projects and their bound resources.
</Callout>
...
| `multica workspace info --projects` | List projects and their bound resources |
...
| `multica skill list/get` | Read-only skill inspection |

Agent CLI exposes only read-only skill inspection (`list` and `get`). Skill
imports remain an administrator/workspace API operation and are not available
through the Agent CLI.



## Computer and runtimes

| Command | Purpose |
|---|---|
| `multica computer start [/<workspace>]` | Start the one machine-wide resident |
| `multica computer stop` | Stop the resident gracefully |
| `multica computer restart [/<workspace>]` | Restart the same resident |
| `multica computer status` | Show redacted identity, environment, fixed package source, and Workspace connections |
| `multica computer logs [/<workspace>]` | View resident logs, optionally scoped to one Workspace |
| `multica computer doctor [/<workspace>]` | Diagnose local Computer state without changing it |
| `multica computer upgrade [--target-version <version>]` | Route through the live Computer owner, or install for the next start when stopped |
...
| Command | Purpose |
|---|---|
| `multica config show` | Show the active environment and redacted saved environment sessions |
| `multica config use <production\|test>` | Safely switch environment and its matching stable/preview package |
...
- [Computer and runtimes](/daemon-runtimes) — how the machine-wide resident and runtimes work
