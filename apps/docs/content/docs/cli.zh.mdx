...
| `multica computer status` | 查看脱敏后的身份、环境、通道和工作区连接 |
| `multica computer logs [/<workspace>]` | 查看 resident 日志，可按工作区过滤 |
| `multica computer doctor [/<workspace>]` | 只读诊断本机 Computer 状态 |
| `multica computer channel [latest\|alpha]` | 在 Computer 停止时查看或切换发布通道 |
| `multica computer upgrade [--target-version <version>]` | 按所选通道升级，或安装指定版本 |
