...
Multica CLI 把 Web UI 能做的事几乎全部搬到了命令行上（创建 [issue](/issues)、分配 [智能体](/agents)、启动机器级 [Computer](/daemon-runtimes) 等等）。这一页把所有顶级命令列出来，每条配一句用途。完整 flag 和示例用 `multica <命令> --help` 查。
...
| `multica setup /<workspace>` | 把机器级 Computer 连接到正式环境中的一个工作区 |
| `multica setup --environment test --server-url <api-origin> --app-url <app-origin> /<workspace>` | 选择腾讯云测试环境并连接一个工作区 |
...
<Callout type="info">
`list` 类命令（例如 `multica issue list`）表格里默认显示**可直接复制**的短 ID。后续命令的 `<id>` 同时接受短 ID 和完整 UUID。项目及其绑定资源统一使用 `multica workspace info --projects` 查看。
</Callout>
...
| `multica workspace info --projects` | 列出项目及其绑定资源 |
...
| `multica skill list/get` | 只读查看 Skill |

Agent CLI 仅提供只读 Skill 查看（`list` 和 `get`）。Skill 导入仍是管理员/工作区 API 操作，Agent CLI 不提供该入口。
...
| `multica computer stop` | 平滑停止 resident |
| `multica computer restart [/<workspace>]` | 重启同一个 resident |
| `multica computer status` | 查看脱敏后的身份、环境、固定包源和工作区连接 |
| `multica computer logs [/<workspace>]` | 查看 resident 日志，可按工作区过滤 |
| `multica computer doctor [/<workspace>]` | 只读诊断本机 Computer 状态 |
| `multica computer upgrade [--target-version <version>]` | 交给 live Computer owner 升级；resident 停止时为下次启动离线安装 |
...
| 命令 | 用途 |
|---|---|
| `multica config show` | 查看当前环境和脱敏后的各环境登录状态 |
| `multica config use <production\|test>` | 安全切换环境及其匹配的 stable/preview 包 |
| `multica version` | 显示 CLI 版本 |
| `multica attachment view --id <id> --output <path>` | 将附件下载到本地文件路径 |
| `multica attachment upload --path <file> [--target '#channel']` | 上传文件；返回的 id 用于 `--attachment-id` |
...
- [Computer 与运行时](/daemon-runtimes) —— 机器级 resident 和运行时的工作机制
