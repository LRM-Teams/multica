...
<Callout type="info">
`list` 类命令（`multica issue list`、`project list` 等）表格里默认显示**可直接复制**的短 ID：issue 是 key（如 `MUL-123`），其余资源是 UUID 短前缀。下面表格里的 `<id>` 同时接受短 ID 和完整 UUID，所以典型用法是 `multica issue list` → 复制 key → `multica issue get MUL-123`。需要完整 UUID 时给 `list` 加 `--full-id`。
</Callout>
...
| `multica computer upgrade [--target-version <version>]` | 按当前环境的固定包源升级，或安装排障用精确版本 |
...
| `multica update` | 按当前环境的固定包源升级 CLI |
