...
The machine-wide Computer resident detects available AI CLIs, restores every explicit Workspace connection in the current environment, and executes assigned work. Profiles cannot create a second resident.
...
By default, the resident runs in the background and logs to `~/.multica/computer/daemon.log`.
...
multica computer status --output json
```

Shows redacted identity, current environment, its fixed stable/preview package source, and Workspace connections.

### Logs

```bash
multica computer logs              # Last 50 lines
multica computer logs -f           # Follow (tail -f)
multica computer logs -n 100       # Last 100 lines
```
...
### Production and Tencent test environments

Production is fixed to app `https://www.leagent.me` plus API/auth/WebSocket `https://api.leagent.me`. Test is an explicit environment and accepts either a Tencent Cloud IP origin or `test.leagent.me`:
...
multica setup --environment test --server-url http://203.0.113.8:8080 --app-url http://203.0.113.8:8080 /my-workspace
```

The same Computer retains connections and login sessions from both environments, keyed by `(environment, workspace_id)`, but one resident generation serves only the current environment. Package source is fixed by environment: production uses stable, test uses preview.

```bash
multica config show
multica config use test
multica config use production
multica computer upgrade          # stable in production, preview in test
```
...
Use `multica workspace info --projects --output json` to inspect projects and
their bound resources. Project administration is managed through the workspace
UI/API; the legacy `multica project` command is not available.
...
multica computer upgrade     # Upgrade through the machine-wide Computer owner
