...
Shows redacted identity, current environment, release channel, and Workspace connections.
...
Production is fixed to `https://leagent.me`. Test is an explicit stage and accepts either a Tencent Cloud IP origin or `test.leagent.me`:
...
The same Computer retains connections from both environments, keyed by `(environment, workspace_id)`, but one resident generation serves only the current environment.

Release maturity is separate:

```bash
multica computer channel          # alpha or latest
multica computer channel alpha    # Computer must be stopped
multica computer upgrade          # uses the selected channel
```
