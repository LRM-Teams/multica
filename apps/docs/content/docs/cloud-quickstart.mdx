...
Sign up at [leagent.me](https://leagent.me). You can log in with email (6-digit verification code) or Google.
...
1. Select the fixed production origin, `https://leagent.me`
...
<Callout type="info">
For the Tencent test stage, use `multica setup --environment test --test-url https://test.leagent.me /my-workspace`. An explicit `http://IP:port` also works before DNS/TLS is ready. This changes the current service environment; it does not create a second Computer.
</Callout>
