---
title: Desktop app
description: Historical behavior of the legacy, unmaintained Multica Desktop app.
---

import { Callout } from "fumadocs-ui/components/callout";

<Callout type="warning">
Desktop is a legacy, unmaintained surface and is not part of the machine-wide Computer v1 contract. Use the web app plus `multica computer` for supported lifecycle operations.
</Callout>
...
The details below document the existing legacy app; they are not a recommendation or a Computer v1 support promise.
...
| Local execution | You run `multica computer start` yourself | Legacy profile daemon (unsupported by Computer v1) |
...
Use the Web app for supported workflows. Do not choose Desktop for a new Computer setup.
...
## Do I still need the standalone CLI and Computer?

**Yes, for the supported Computer v1 path.** Desktop ships a legacy embedded CLI and launches an isolated profile daemon, but that lifecycle is unmaintained and outside the Computer contract.

If you've already started the machine-wide Computer, Desktop does not take it over. Its old profile-specific daemon behavior is retained only as legacy implementation and is not a supported Computer lifecycle.
