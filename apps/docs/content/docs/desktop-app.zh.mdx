---
title: 桌面应用
description: 无人维护的 Multica Desktop 遗留行为说明。
---

import { Callout } from "fumadocs-ui/components/callout";

<Callout type="warning">
Desktop 是无人维护的遗留入口，不属于机器级 Computer v1 合同。受支持的方式是 Web 加 `multica computer` 生命周期。
</Callout>

下面只记录现有遗留应用的行为，不代表推荐使用，也不是 Computer v1 的支持承诺。

Multica Desktop 是原生桌面应用——macOS / Windows / Linux 三个平台。对它当前配置的环境来说，它和 Web 版连同一个后端、看到的数据完全一样。Desktop 默认使用 Multica Cloud；自部署实例可以通过本地运行时配置文件接入。它还保留了几个旧能力：**[工作区](/workspaces) 独立的多标签页**、**自动启动遗留守护进程**、**一键升级**。
...
| 本地执行 | 自己运行 `multica computer start` | 遗留 profile daemon（不受 Computer v1 支持）|
| 升级 | 刷新页面就是最新 | 应用自动检查 + 下次启动安装 |
| 登录后的数据 | 完全一样 | 完全一样 |

受支持的工作流请使用 Web。新的 Computer 安装不要选择 Desktop。
...
## 还要单独装 CLI 和 Computer 吗

**受支持的 Computer v1 路径需要单独安装。** Desktop 包里虽内置了旧 `multica` CLI，并会启动独立 profile daemon，但这套生命周期无人维护，也不属于 Computer 合同。

如果机器级 Computer 已经启动，Desktop 不会接管它。Desktop 旧的 profile daemon 行为只作为遗留实现保留，不属于受支持的 Computer 生命周期。
...
如果 `desktop.json` 存在但内容无效，Desktop 会 fail closed，显示阻塞式配置错误，而不是悄悄回退到 Cloud。开发构建里，`electron-vite dev` 仍然优先使用 `VITE_API_URL` / `VITE_WS_URL` / `VITE_APP_URL`。Desktop 运行时自部署配置能力对应 [issue #1371](https://github.com/LRM-Teams/multica/issues/1371)。
</Callout>
