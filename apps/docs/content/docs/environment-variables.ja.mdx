...
この独立した `RATE_LIMIT_TRUSTED_PROXIES` は、レガシー webhook 入站のリミッターを制御する `MULTICA_TRUSTED_PROXIES` とは**異なります**。各リミッターは自身のリストをパースするため、プロキシの背後にあるデプロイは両方を設定すべきです。
...
| `MULTICA_DAEMON_POLL_INTERVAL` | `2s` | タスクのポーリング間隔 |
...
デーモン、GitHub 自動更新のダウンロード、および環境を継承する子プロセスは、標準の `HTTP_PROXY`、`HTTPS_PROXY`、`NO_PROXY`（小文字表記も可）に従います。タスク起動 WebSocket も同じ規則を使用し、`ws://` は `HTTP_PROXY`、`wss://` は `HTTPS_PROXY` に従います。

マシン/profile ごとに永続化する場合：
...
環境変数は設定ファイルのプロキシ URL より優先されます。`NO_PROXY`、`no_proxy`、`proxy.no_proxy` はマージされ、`127.0.0.1,localhost` は常に保持されます。ローカルの daemon/runtime 制御通信がプロキシを通らないよう、両方の表記をエクスポートします。プロキシ URL には資格情報が含まれる場合があるため、設定ファイルは `0600` で保存され、`multica config show` は HTTP(S) プロキシの値を表示せず `(set)` のみを表示します。
