...
이 별도의 `RATE_LIMIT_TRUSTED_PROXIES`는 레거시 webhook 수신 경로 리미터를 제어하는 `MULTICA_TRUSTED_PROXIES`와는 **다릅니다**. 각 리미터는 자체 목록을 파싱하므로, 프록시 뒤에 있는 배포는 둘 다 설정해야 합니다.
...
| `MULTICA_DAEMON_POLL_INTERVAL` | `2s` | 작업 폴링 간격 |
...
데몬, GitHub 자동 업데이트 다운로드, 환경을 상속하는 자식 프로세스는 표준 `HTTP_PROXY`, `HTTPS_PROXY`, `NO_PROXY` 변수(소문자 표기도 지원)를 따릅니다. 작업 깨우기 WebSocket도 같은 규칙을 사용하며 `ws://`는 `HTTP_PROXY`, `wss://`는 `HTTPS_PROXY`를 따릅니다.

머신/profile별로 영구 설정하려면:
...
환경 변수는 설정 파일의 프록시 URL보다 우선합니다. `NO_PROXY`, `no_proxy`, `proxy.no_proxy`는 합쳐지며 `127.0.0.1,localhost`는 항상 유지됩니다. 로컬 daemon/runtime 제어 트래픽이 프록시를 거치지 않도록 두 표기를 모두 내보냅니다. 프록시 URL에는 자격 증명이 포함될 수 있으므로 설정 파일은 `0600`으로 저장되고, `multica config show`는 HTTP(S) 프록시 원문 대신 `(set)`만 표시합니다.
