...
|---|---|---|
| `DATABASE_MAX_CONNS` | `25` | pgxpool max connections. The daemon polls frequently (every 2s) and uses connections; larger deployments may need a higher value |
...
This separate `RATE_LIMIT_TRUSTED_PROXIES` is **not** the same as `MULTICA_TRUSTED_PROXIES`, which controls webhook rate limiting for legacy ingress paths. Each limiter parses its own list, so a deployment behind a proxy should set both.
...
| `MULTICA_DAEMON_POLL_INTERVAL` | `2s` | Task polling interval |
...
### Daemon outbound proxy

The daemon, its GitHub update downloads, and inherited child-process egress honor the standard `HTTP_PROXY`, `HTTPS_PROXY`, and `NO_PROXY` variables (lowercase spellings are also accepted). Task-wakeup WebSockets use the same rules: `ws://` follows `HTTP_PROXY`, while `wss://` follows `HTTPS_PROXY`.

For a persistent per-machine/profile setting, use:

```bash
multica config set proxy.http http://proxy.example:8080
multica config set proxy.https http://proxy.example:8080
multica config set proxy.no_proxy .corp.example,metadata.internal
```

Environment variables take priority over config-file proxy URLs. `NO_PROXY`, `no_proxy`, and `proxy.no_proxy` are merged; `127.0.0.1,localhost` is always retained and both env spellings are exported so local daemon/runtime control traffic never goes through the proxy. Proxy URLs may contain credentials: the config file is mode `0600`, and `multica config show` reports only `(set)` rather than printing HTTP(S) proxy values.
...
| `MULTICA_PUBLIC_URL` | empty | Public API URL, without trailing slash. Used as the public base URL and by the web UI as the daemon `server_url` |
