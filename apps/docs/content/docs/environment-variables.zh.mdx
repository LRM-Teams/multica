...
这里的 `RATE_LIMIT_TRUSTED_PROXIES` 和 `MULTICA_TRUSTED_PROXIES` **不是同一个**变量——后者控制遗留 webhook 入站路径的限流器。两个限流器各自读各自的列表，部署在代理后面的实例需要两个都配上。
...
| `MULTICA_DAEMON_POLL_INTERVAL` | `2s` | 任务轮询频率 |
| `MULTICA_DAEMON_MAX_CONCURRENT_TASKS` | `20` | 并发任务上限 |
| `MULTICA_AGENT_TIMEOUT` | `0` | 单次任务的绝对墙钟上限；`0` = 不设上限。想要硬性成本/资源天花板时再设一个正值 |
| `MULTICA_RUNTIME_PROGRESS_STALE` | `15m` | resident Message runtime 没有新的 runtime event 这么久后，发布 `runtime_stalled` Activity 并进入恢复判断。`0` = 关闭 stalled 监控 |
| `MULTICA_<PROVIDER>_PATH` | 对应 CLI 名 | 各 AI 编程工具的可执行文件路径（如 `MULTICA_CLAUDE_PATH`）|
| `MULTICA_<PROVIDER>_MODEL` | 空 | 各 AI 编程工具的默认模型 |

### 守护进程出站代理

守护进程、GitHub 自动更新下载以及继承环境的子进程都遵循标准 `HTTP_PROXY`、`HTTPS_PROXY`、`NO_PROXY`（也接受小写形式）。任务唤醒 WebSocket 使用同一规则：`ws://` 走 `HTTP_PROXY`，`wss://` 走 `HTTPS_PROXY`。
...
环境变量优先于配置文件中的代理 URL。`NO_PROXY`、`no_proxy` 与 `proxy.no_proxy` 会合并，并始终保留 `127.0.0.1,localhost`；守护进程同时导出大小写两种形式，保证本机 daemon/runtime 控制流量不经过代理。代理 URL 可能含凭据：配置文件权限为 `0600`，`multica config show` 对 HTTP(S) 代理只显示 `(set)`，不会打印原值。
...
| `MULTICA_PUBLIC_URL` | 空 | 公开 API URL，不带结尾 slash。用作公开 API 基址，也会被 Web UI 用作守护进程的 `server_url` |
