...
Each user who wants to run AI agents locally also installs the **`multica` CLI** and runs one machine-wide **Computer resident** on their own machine.
...
# After creating a Workspace in the Web app, connect through one same-origin endpoint
multica setup --environment test --server-url https://multica.example.com --app-url https://multica.example.com /<workspace-slug>
```
...
<Callout>
If the self-host server is already running and you only need the CLI on a macOS/Linux machine, run: `curl -fsSL https://cdn.leagent.me/computer/install.sh | bash`.
</Callout>
...
### Step 3 — Install CLI & Start the Computer

The resident runs on your local machine (not inside Docker). It detects installed AI agent CLIs, registers them with the server, and executes tasks.
...
This automatically:
1. Selects the explicit test service origin
2. Opens your browser for authentication
3. Resolves exactly the requested Workspace
4. Adds one Workspace connection and starts the machine-wide resident detached

The test URL must be the final same-origin HTTP(S) endpoint serving the app,
API, and WebSocket. It may be a Tencent Cloud IP/port or a domain such as
`https://test.leagent.me`; it cannot be a production leagent.me host.
...
<Callout>
The retired `multica setup self-host`, profile selector, and custom `config set server_url` lifecycle do not create a supported Computer. Pass the API and Web origins explicitly with `--environment test --server-url <api-origin> --app-url <app-origin>`; they may be the same reverse-proxy origin.
</Callout>
...
Full reference (audit table semantics, advisory lock 4246, the standalone backfill command, flag descriptions, the migration auto-hook) lives in [`SELF_HOSTING_ADVANCED.md → Usage Dashboard Rollup`](https://github.com/LRM-Teams/multica/blob/main/SELF_HOSTING_ADVANCED.md#usage-dashboard-rollup).
...
# Stop the local Computer resident
multica computer stop
```
...
This switches the selected service to production, clears the test session/current
selection, re-authenticates, and converges the same resident. Stored test
connections, Computer Identity, and Agent Roots remain on disk.
