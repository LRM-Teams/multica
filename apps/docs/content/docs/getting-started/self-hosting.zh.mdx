...
multica setup --environment test --test-url https://multica.example.com /<workspace-slug>
```
...
<Callout>
The retired `multica setup self-host`, profile selector, and custom `config set server_url` lifecycle do not create a supported Computer. Put a reverse proxy in front of split frontend/backend ports and pass its one final origin with `--environment test --test-url`.
</Callout>
