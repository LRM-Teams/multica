...
- **AI coding tools** — one of the seven (or several in parallel): [Claude Code](/providers#claude-code), [Codex](/providers#codex), [Cursor](/providers#cursor), [Grok](/providers#grok), [Kiro CLI](/providers#kiro-cli), [OpenCode](/providers#opencode), [Pi](/providers#pi). Once the daemon has picked up a task, it uses these tools to actually do the work.
