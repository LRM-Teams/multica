...
description: Multica drives whichever AI coding tools you have on your machine. This page shows you how to install each of the 12 supported tools so the Computer can detect them.
---

import { Callout } from "fumadocs-ui/components/callout";

A **runtime** in Multica is one Workspace connection paired with one AI coding tool found by your machine-wide Computer resident. If onboarding shows **No supported tools detected**, the Computer scanned `PATH` and found none of the 12 tools it knows how to drive. Install one (or several), then re-scan; the runtime appears within a few seconds.
...
Google's Antigravity CLI (`agy`). Pairs with Google's Antigravity service and runs Gemini-backed models. Session resumption works through `--conversation <id>`, captured by the Computer resident from the CLI log file. Model selection is managed inside the Antigravity CLI itself — Multica disables the per-agent model picker for this provider. Skills are written to `.agents/skills/` (the CLI inherits Gemini CLI's workspace skill layout — see [Antigravity docs](https://antigravity.google/docs/gcli-migration)).

| | |
|---|---|
| Computer looks for | `agy` |
...
1. **Confirm the binary is on `PATH`.** Open a fresh terminal and run `which <name>` (for example `which claude`, `which cursor-agent`, `which kiro-cli`, `which agy`). If it prints a path, the Computer can find it. If it prints nothing, fix your shell `PATH` first (the typical cause is a per-shell rc file that wasn't reloaded).
