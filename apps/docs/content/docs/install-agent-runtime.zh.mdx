...
description: Multica 驱动本机上已安装的 AI 编程工具。这一页讲清楚怎么安装目前支持的 7 款工具，让守护进程能扫到。
---

import { Callout } from "fumadocs-ui/components/callout";

在 Multica 里，一个**运行时**（runtime）就是你机器上的守护进程，配上守护进程在 `PATH` 里扫到的某一款 AI 编程工具。如果 onboarding 的 "连接运行时" 这一步显示 **未检测到支持的工具**，说明守护进程扫了 `PATH`，但 7 款它认得的工具一个都没找到。装下面任意一款（或几款），回到这一步重新扫描，几秒内运行时就会出现。
...
ACP-over-stdio 传输。会话续接走 ACP session。skills 落在 `.grok/skills/`。Multica 不会把 `mcp_config` 转发给这个 runtime。

| | |
|---|---|
| 守护进程扫描 | `grok` |
| 安装 | 看 xAI 的 Grok Build / `grok` CLI 官方指引。 |
| 认证 | 按 xAI 文档。 |
...
1. **确认可执行文件在 `PATH` 上。** 开一个新终端，跑 `which <名字>`（比如 `which claude`、`which cursor-agent`、`which kiro-cli`、`which grok`）。打印出路径，守护进程就找得到；什么都不打印，先修 shell 的 `PATH`（最常见原因是 rc 文件没重新加载）。
