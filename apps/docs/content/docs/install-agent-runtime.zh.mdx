...
description: Multica 驱动本机上已安装的 AI 编程工具。这一页讲清楚怎么安装目前支持的 12 款工具，让 Computer 能扫到。
---

import { Callout } from "fumadocs-ui/components/callout";

在 Multica 里，一个**运行时**（runtime）就是一个工作区连接，配上一款由全机唯一 Computer resident 在 `PATH` 里找到的 AI 编程工具。如果 onboarding 显示 **未检测到支持的工具**，说明 Computer 扫描了 `PATH`，但 12 款支持工具一个都没找到。装下面任意一款（或几款）再重新扫描，几秒内运行时就会出现。

这一页是装机的入口，和它配套的是：

- [Computer 与运行时](/zh/daemon-runtimes) — 检测是怎么工作的
...
1. **Multica Computer 在运行。** 装完 [Multica CLI](/zh/cli) 后用 `multica setup /<workspace>` 连接工作区。全机 resident 没起来，就没人去扫工具。
2. **工具的可执行文件在 `PATH` 上。** Computer resident 按名字调用工具（见下面的 **Computer 扫描**）。终端里 `which <名字>` 找不到，Computer 也找不到。装完后打开新终端，再重启 Computer，让新的 `PATH` 生效。
...
Google 的 Antigravity CLI（`agy`）。搭配 Google Antigravity 服务，默认走 Gemini 系列模型。会话续接通过 `--conversation <id>` 工作——Computer resident 从 CLI 日志里抓取 conversation UUID。模型选择保存在 Antigravity CLI 自己的设置里——Multica 里这款工具的「模型」选择项被禁用。Skill 文件写入 `.agents/skills/`（CLI 沿用 Gemini CLI 的 workspace 布局——见 [Antigravity 文档](https://antigravity.google/docs/gcli-migration)）。
...
1. **确认可执行文件在 `PATH` 上。** 开一个新终端，跑 `which <名字>`（比如 `which claude`、`which cursor-agent`、`which kiro-cli`、`which agy`）。打印出路径，Computer 就找得到；什么都不打印，先修 shell 的 `PATH`（最常见原因是 rc 文件没重新加载）。
2. **重启 Computer。** `multica computer restart`。resident 只在启动时扫一次 `PATH`。
3. **看 Runtimes 页面。** Multica UI 的 **Runtimes** 页应该会出现一行 `(工作区连接 × 工具)`。如果显示 "offline"，看[Computer 与运行时 → 运行时何时被标记为离线](/zh/daemon-runtimes#运行时何时被标记为离线)。
4. **回到 onboarding。** "连接运行时" 这一步会一直轮询，几秒内就能扫到新运行时，不需要手动刷新。

## 排错

- **`which` 找得到，但 Computer 找不到。** resident 是用旧 `PATH` 启的，重启 Computer。
- **可执行文件在，但启动就失败。** 在终端单独跑一次工具的 `--version` 或 `--help`，绝大多数失败都是登录没做、token 过期、Node.js / 运行时版本不对。
- **Runtimes 页面看到行，但任务一跑就失败。** 一边触发任务一边跑 `multica computer logs -f`。resident 会把工具自己的报错原样吐出来。

更宽的症状看[排错指南](/zh/troubleshooting)。

## 接下来

- [Computer 与运行时](/zh/daemon-runtimes) — 检测、心跳、离线处理
