...
1. Open the workspace **Members** page (sidebar), click **+** next to **Humans**, enter the email, and pick a role.
...
Role changes and removing humans are also done from a human's profile on the Members page (owner/admin only).
...
If the invitation email fails to deliver (wrong address, mail service hiccup), the invitation record is still retained; you can send a new invitation from the Members page, or share the invitation link through another channel.
