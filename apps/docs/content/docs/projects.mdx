...
Each project can have a **Resources** section for product metadata and references. Resources do not choose an [agent's](/agents) working directory, trigger a clone, or inject repository URLs into the Agent prompt. Every Agent works from its own durable workspace and chooses its own tools and Git workflow.
