...
After creating a Workspace in the web app, connect the Computer explicitly as a test environment:

```bash
multica setup --environment test --test-url http://localhost:8080 /<workspace-slug>
```

The server-provided device link opens the frontend login, while the Computer API origin remains `http://localhost:8080`. Setup stores the PAT, adds one Workspace connection, and starts the resident.
