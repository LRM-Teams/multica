...
The CLI install is the same as in [Cloud quickstart → 2. Install the CLI](/cloud-quickstart#2-install-the-multica-cli): use the user-owned install script on macOS/Linux or PowerShell on Windows.

### 5a. Same machine

The default Compose stack exposes frontend and backend on separate local ports,
so neither port alone is a valid Computer v1 service origin. Put a same-origin
local reverse proxy in front of both ports (the route shape in 5b works on
localhost too), then connect the Computer explicitly as a test environment:

```bash
multica setup --environment test --server-url http://localhost:8081 --app-url http://localhost:8081 /<workspace-slug>
```

In this example the proxy listens on `http://localhost:8081`, sends `/api/*`
and `/ws` to backend port 8080, and everything else to frontend port 3000.
Setup stores the PAT, adds one Workspace connection, and starts the resident.
...
```bash
multica setup --environment test --server-url https://<your-domain> --app-url https://<your-domain> /<workspace-slug>
```

<Callout type="info">
The `test` label is explicit here: production cannot be redirected away from `leagent.me`, and the retired profile/self-host setup path cannot create a second resident.
</Callout>
...
Full reference — including operations notes and the Kubernetes deployment shape — lives in the repo's [`SELF_HOSTING_ADVANCED.md → Usage Dashboard Rollup`](https://github.com/LRM-Teams/multica/blob/main/SELF_HOSTING_ADVANCED.md#usage-dashboard-rollup).
...
```bash
multica setup --environment test --server-url http://api.multica.dev.lan --app-url http://api.multica.dev.lan /<workspace-slug>
```
...
The full reference — three login modes, the `backend` ExternalName workaround for the build-time-baked `REMOTE_API_URL` in the web image, resource limits, and TLS — lives in the repo's [`SELF_HOSTING.md`](https://github.com/LRM-Teams/multica/blob/main/SELF_HOSTING.md#kubernetes-deployment-alternative).
