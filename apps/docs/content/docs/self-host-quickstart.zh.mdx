...
命令行装法和 [Cloud 快速上手 → 2. 装命令行工具](/cloud-quickstart#2-装-multica-命令行工具) 一样：macOS/Linux 使用用户目录安装脚本，Windows 使用 PowerShell。

### 5a. 同一台机器

默认 Compose 把前端和后端暴露在两个本地端口，所以任何一个端口单独拿来
都不是 Computer v1 的完整服务 origin。先用本地反向代理把两者收到同一个
origin（5b 的路由在 localhost 也适用），再把 Computer 显式连接为测试环境：

```bash
multica setup --environment test --server-url http://localhost:8081 --app-url http://localhost:8081 /<workspace-slug>
```

这里假设代理监听 `http://localhost:8081`，把 `/api/*` 和 `/ws` 转给后端
8080，其它请求转给前端 3000。setup 保存 PAT、添加一个工作区连接并启动
resident。
...
```bash
multica setup --environment test --server-url https://<你的域名> --app-url https://<你的域名> /<workspace-slug>
```

<Callout type="info">
这里显式使用 `test`：production 永远不能从 `leagent.me` 重定向，废弃的 profile/self-host setup 也不能创建第二个 resident。
</Callout>
...
完整参考（含运维注意事项和 Kubernetes 部署形态）见仓库的 [`SELF_HOSTING_ADVANCED.md → Usage Dashboard Rollup`](https://github.com/LRM-Teams/multica/blob/main/SELF_HOSTING_ADVANCED.md#usage-dashboard-rollup)。
...
完整参考——三种登录方式、为了绕过 web 镜像 build-time 写死的 `REMOTE_API_URL` 而加的 `backend` ExternalName 别名、资源限制、TLS——都在仓库的 [`SELF_HOSTING.md`](https://github.com/LRM-Teams/multica/blob/main/SELF_HOSTING.md#kubernetes-deployment-alternative)。
