...
After attaching, the agent picks up its skills the next time it starts a task — each AI coding tool has its own skill discovery path (Claude Code uses `.claude/skills/`, Cursor uses `.cursor/skills/`, Grok uses `.grok/skills/`, etc.), and Multica drops files in the right place automatically. Full path mapping is in [AI coding tools comparison → Where skill files go](/providers#where-skill-files-go).
...
The two are complementary. In Multica today, MCP support is **provider-specific**: Claude Code, Codex, Cursor, Kiro CLI, OpenCode, and Pi consume `mcp_config`; Grok receives the field but doesn't actually use it. A dedicated MCP section will come in a later release.
...
- [AI coding tools comparison](/providers) — full comparison of all 7 tools (including each one's skill injection path)
