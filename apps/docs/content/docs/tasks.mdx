...
A **task** is the unit of every [agent](/agents) run — [assigning an issue to an agent](/assigning-issues), [@-mentioning an agent in a comment](/mentioning-agents), sending a message in [chat](/chat) all produce a task. Multica puts it in a queue; a [daemon](/daemon-runtimes) picks it up and hands it off to the corresponding [AI coding tool](/providers), then writes the result back to the server when it finishes.
...
Issue work uses one execution lane per agent and issue. The same agent's follow-up tasks on the same issue wait for the active run to finish, which prevents two sessions from overwriting the same issue workspace. Tasks for different issues can run in parallel up to that agent's **Concurrency limit** (`max_concurrent_tasks`, default 6).

Chat, DM, channel, and other issue-less work share one separate conversation lane per agent. That lane stays serial so mutable conversation context cannot race, but it does not consume or block the agent's issue-task slots. Different agents are always admitted independently.
...
2. **Only for issue- and chat-triggered tasks** — other sources do **not** retry automatically.

<Callout type="info">
**How you'll know a task failed**: a notification lands in your [Inbox](/inbox), and the associated issue's status reverts from `in_progress` back to `todo`.
</Callout>
...
- The execution-log retry button on a specific row sends that row's task ID alongside, so the rerun targets **the agent that ran that exact task** — not the current assignee. This makes per-row retry meaningful for parallel @-mention agents, or rows whose agent has since been displaced by a reassignment.
...
- ✅ **Real support** — Claude Code, Grok, Kiro CLI, OpenCode, Pi
...
- [Providers Matrix](/providers) — capability differences across the 7 AI coding tools (including the exact session-resumption status)
- [Assigning issues to agents](/assigning-issues) / [@-mentioning agents in comments](/mentioning-agents) / [Chat](/chat) — the three ways to trigger a task
