...
**执行任务**（task）是 [智能体](/agents) 每一次工作的单位——把一个 [issue 分给智能体](/assigning-issues)、[在评论里 @提及智能体](/mentioning-agents)、在 [聊天](/chat) 里发一条消息，都会产生一个执行任务。Multica 把它放进队列，由 [守护进程](/daemon-runtimes) 领走后交给对应的 [AI 编程工具](/providers) 执行，结束时把结果写回服务器。
...
Issue 工作按智能体和 issue 划分执行 lane。同一个智能体在同一个 issue 上的后续任务，必须等当前 run 结束，避免两个会话互相覆盖同一份 issue 工作区；不同 issue 的任务可以并行，最多同时运行该智能体的**并发上限**（`max_concurrent_tasks`，默认 6）个。

聊天、DM、频道以及其它不关联 issue 的工作，共用该智能体单独的一条 conversation lane。这条 lane 保持串行，避免可变对话上下文竞态，但不会占用或阻塞 Issue 任务槽位。不同智能体始终独立领取任务。
...
上面这层是**服务端的粗粒度兜底**——按任务启动时间算，不看任务是否还在活动。任务执行路径不再额外维护 stalled 看门狗；resident Message runtime 统一根据 runtime progress 判断静默，并在需要时发布 `runtime_stalled` Activity。agent 仍可使用 `MULTICA_AGENT_TIMEOUT` 设置可选的墙钟上限（默认 `0` = 不设上限）。各参数见 [环境变量 → 守护进程的调节参数](/environment-variables#守护进程的调节参数)。
...
2. **只对 issue 和聊天触发的任务生效**——其他来源的任务**不自动重试**。

<Callout type="info">
**怎么知道任务失败了**：失败任务会在你的 [收件箱](/inbox) 里出现一条通知，关联的 issue 状态也会从 `in_progress` 退回 `todo`。
</Callout>
...
- ✅ **真支持**——Claude Code、Grok、Kiro CLI、OpenCode、Pi
...
- [Providers Matrix](/providers) —— 7 款 AI 编程工具的能力差异对照（包括会话恢复的精确状态）
- [分配 issue 给智能体](/assigning-issues) / [在评论里 @智能体](/mentioning-agents) / [聊天](/chat) —— 触发执行任务的三种方式
