...
Look up issues by symptom. Each entry gives you **symptom / likely causes / how to diagnose / how to fix**. If your situation isn't listed, open an issue on [GitHub](https://github.com/LRM-Teams/multica/issues).

## Computer can't connect to the selected environment

**Symptom**: `multica computer status` shows `connected: false` or the logs show `connection refused`.

**Likely causes**:

1. **The selected service environment is wrong** — production is fixed to `leagent.me`; test must carry an explicit Tencent origin
2. **Network / firewall blocking** — the Computer and server aren't on the same network, or outbound traffic is blocked
...
5. **DNS resolution failure** — the hostname doesn't resolve on the Computer's machine

**How to diagnose**:

```bash
multica computer status --output json     # confirm environment + service_origin
multica computer logs --lines 100         # look for resident-side errors
...
**How to fix**: re-run `multica setup /<workspace>` for production, or `multica setup --environment test --server-url <api-origin> --app-url <app-origin> /<workspace>` for Tencent test. To refresh an expired session, run `multica auth logout` and setup again.

## Tasks stuck in `queued`

**Symptom**: after assigning an issue to an agent, the issue status flips to `in_progress` immediately, but a long time passes with no sign of agent execution; `multica computer status` shows the resident connected.
...
4. **Computer hasn't registered this runtime for the current Workspace connection** — restart the Computer or reselect the runtime in the UI
5. **Computer disconnected** — no resident heartbeat in the last 45 seconds. `multica computer status` separates the local process from service connectivity

**How to diagnose**:

```bash
multica computer status --output json      # environment + Workspace connections
...
- Runtime not registered → `multica computer restart`, and the resident will re-register
...
**Symptom**: `multica server` or `multica computer start` fails with `address already in use`.

**Likely causes**:

1. **Server port taken** (default `8080`)
2. **Computer health port taken** (the one machine-wide port is `19514`)
...
|---|---|---|
| **Computer** | `~/.multica/computer/daemon.log` (background mode) or foreground stdout | `multica computer logs -f --lines 100` |
...
For more detailed resident logs, move it from background to foreground: `multica computer stop && multica computer start --foreground`.
