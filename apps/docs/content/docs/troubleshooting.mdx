...
**How to fix**: re-run `multica setup /<workspace>` for production, or `multica setup --environment test --test-url <origin> /<workspace>` for Tencent test. To refresh an expired session, run `multica auth logout` and setup again.
