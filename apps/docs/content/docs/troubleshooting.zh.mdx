...
按症状查问题。每条问题都给**症状 / 可能原因 / 怎么查 / 怎么修**四段。如果你的情况不在下面，到 [GitHub](https://github.com/LRM-Teams/multica/issues) 提 issue。

## Computer 连不上当前环境

**症状**：`multica computer status` 显示 `connected: false`，或日志出现 `connection refused`。

**可能原因**：

1. **选错服务环境** —— 正式固定 `leagent.me`；测试必须显式配置腾讯云 origin
2. **网络 / 防火墙阻挡** —— Computer 和 server 不在同一网络，或出站被 block
3. **Token 过期或无效** —— 你从来没跑过 `multica login`，或 PAT 被撤销
4. **服务器拒绝注册** —— 你登录的账号不在目标工作区（register 返 403）
5. **DNS 解析失败** —— hostname 在 Computer 所在机器上解不出来

**怎么查**：

```bash
multica computer status --output json     # 确认 environment + service_origin
multica computer logs --lines 100         # 看 resident 侧错误
...
**怎么修**：正式环境重新执行 `multica setup /<workspace>`；腾讯测试环境执行 `multica setup --environment test --server-url <api-origin> --app-url <app-origin> /<workspace>`。登录失效时先 `multica auth logout`，再 setup。

## 任务一直卡在 queued

**症状**：把 issue 分给 agent 后，issue 状态立刻变 `in_progress`，但过了很久页面没有 agent 执行的迹象；`multica computer status` 显示 resident 已连接。
...
4. **Computer 没为当前 Workspace connection 注册该 runtime** —— 重启 Computer 或在 UI 重新选一次 runtime
5. **Computer 失联** —— 最近 45 秒没 resident 心跳。`multica computer status` 会把本地进程和服务端连接状态分开显示

**怎么查**：

```bash
multica computer status --output json      # 环境 + 工作区连接
...
- Runtime 未注册 → `multica computer restart`，resident 会重新注册
...
**症状**：`multica server` 或 `multica computer start` 启动失败，报 `address already in use`。

**可能原因**：

1. **Server 端口被占用**（默认 `8080`）
2. **Computer health 端口被占用**（全机唯一端口 `19514`）
...
| 组件 | 位置 | 命令 |
|---|---|---|
| **Computer** | `~/.multica/computer/daemon.log`（后台模式）或前台 stdout | `multica computer logs -f --lines 100` |
...
需要更详细的 resident 日志，把它从后台挪到前台跑：`multica computer stop && multica computer start --foreground`。

## CLI / daemon 卡在旧版本、页面也不提示升级

常见原因：Machine Upgrade 已改为仅显式触发、老二进制编译进了错误的 release feed、或版本不是三段 `x.y.z`。

**处理**：不要等旧进程自己升。按 [CLI Installation](/docs/cli/installation) 的旧版本处理章节用 OSS `install.sh` / 所选通道 manifest **带外装包**，再 `multica computer restart`。
