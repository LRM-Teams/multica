...
const nextConfig: NextConfig = {
  ...
  async redirects() {
    return [
      ...
      // the runtimes API in production (task #18 hotfix). The lookahead is
      // anchored to `api/` (not bare `api`) so it excludes only the literal
      // `api` segment, not real workspace slugs that happen to start with
      // "api" (e.g. `apitest`).
      {
