...
2. 与某个成员的稳定协作关系（角色、交接方式、怎么配合），写该成员的 `RELATIONSHIP.md`；与队友 agent 的关系写 `notes/agents.md` / `notes/relationship-map.md`。纯社交打招呼、没有可复用事实时不写。
...
8. 猜测、普通任务细节、原始聊天记录、秘密以及只对当前回复有用的内容，不写入长期记忆。
