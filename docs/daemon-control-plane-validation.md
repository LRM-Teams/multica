...
| Three Agent restarts | Run `restart`, `reset_session_restart`, and `full_reset_restart`, including one active turn | Each operation is delivered once through the control heartbeat with no parallel lifecycle `agent:stop`; the managed launch remains authoritative while its provider restarts; Activity follows `Stopped → Starting → Idle`; only the chosen mode's session/root retention boundary changes; a subsequent message receives a reply. |
