...
| Process restart recovery | Restart the Computer process while an unacknowledged Reminder receipt exists | The current Runner starts its control heartbeat after Ready; per-Runtime Reminder snapshots restore scheduled state and the pending receipt resumes without duplicate local input; Agent replies after recovery. |
