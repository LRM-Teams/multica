...
| Three Agent restarts | Run `restart`, `reset_session_restart`, and `full_reset_restart`, including one active turn | The durable server orchestrator delivers discrete `agent:stop → agent:reset-workspace? → agent:start` commands through the current Runner. Inactive is emitted only after the old launch's startup owner, provider lease, and resident process have quiesced; Activity follows `Stopped → Starting → Idle`; only the chosen mode's session/root retention boundary changes; a subsequent message receives a reply. |
