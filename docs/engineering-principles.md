...
- V6 Node ID 必须由 `entity_kind` 对应的 canonical identity 字段生成，不能在 payload 中按固定优先级寻找任意 `*_id`；lineage ID 不是节点自身 ID。已知 kind 缺少自身 identity 或 payload malformed 时 V6 Snapshot/live frame fail closed；未知未来 kind 使用稳定 source node ID 降级 generic，不读取其私有 `entity_id`。
