...
- Search/Corpus 谱系必须以一个原子、可重放的 `Search Plan → Query Execution → Source Candidate → Screening Decision` 批次持久化；每个 canonical 行同时注册 Artifact Passport，Candidate/Decision/Execution 是 append-only。Source Snapshot 必须显式区分 `agent_direct_evidence` 与 `screened_retrieval`；后者必须引用同 scope、`accepted` 且 URL/已知 content hash 相符的 Screening Decision，禁止仅凭 Agent prose 伪造检索来源。
