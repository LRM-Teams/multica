...
- 生产评测 Artifact 只能从 canonical ledger/projection 提取：Source metadata 必须绑定同一 sealed subject hash 与受控 Document ID，Observation datum 必须携带显式 fact key/value，Claim 只沿 canonical Evidence 汇总；报告成员、Dispute、Action 和 V6 Graph/Projection 由各自 owner 提供。Adapter 不得通过 URL、标题、摘要或正文匹配来猜 Document、Fact、Conflict 或 Report membership，无法证明的字段保持缺失并让现有 grader 失败。
