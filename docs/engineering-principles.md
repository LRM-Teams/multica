...
- Research Result 的接纳前规则由 `resultAcceptanceModule` 拥有：先验证 Attempt 属于目标 Task 且 Inbox 绑定一致，再按 Run 固定的 orchestrator version 解码，并验证新任务所需 capability 当前可用，最后调用 PostgreSQL 原子接纳。PostgreSQL 事务仍必须重新验证 Agent/Inbox、Attempt/Run 状态；D-enabled Result replay 除 request/result hash 外，还必须匹配接纳时封存且不可改写的 Manifest ID/hash、resolved input Version set 与 Input Reference lineage，任一漂移均冲突。预检不能替代事务内授权。Engine 只在已接纳后触发 Reconcile。
