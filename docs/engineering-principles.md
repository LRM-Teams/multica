...
- Research Source 的独立性不能由不同 URL、Agent 自报 family 或不同 publisher 名称直接推断。服务端必须对可定位的 publisher、owner、dataset、syndication 事实做传递依赖闭包；同一 family 最多贡献一份独立支持，代表来源必须同时具备 verified publisher 与 owner，缺失或未验证身份一律不计独立票。当前可执行分类器为 `server/internal/researchrun/source_independence.go`；持久 family registry、duplicate cluster 合并与 Gate 计数接线前不得宣称来源独立性已生产化。
