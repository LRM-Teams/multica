...
- **SSH 与执行 ownership**：SSH 凭据只来自受保护 Environment，进入 `root` 仅用于创建/chown 单次 remote temp；所有部署脚本立即通过 `runuser` 降为 `dev`，remote temp 与 `/data/multica` 均归 `dev`，终局删除 remote temp。禁止把密码写进 workflow 文件或日志，禁止关闭 host-key 校验，禁止让 production deploy 依赖 Aliyun 到 GitHub Actions task endpoint 的持续出站连接。
