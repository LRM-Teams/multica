...
### 4.19.1 Agent Attachment 是机器责任，不是进程状态 — `可执行`（② registry interface + ③单一 apply core + ⑤ generation/restart 回归）
...
- 迁移期继续读写现有 `.daemon/reminder_agents.json` 字段和路径；`reminderAgentManager` facade 已删除，旧 start/stop/replay-end wire 只通过同一 registry core 的 protocol adapter，不建第二份状态、不让旧 lifecycle 失效。generation-zero 的 credential/task observation 仅作 provisional compatibility，正式 `Resolve/List` 不返回它。完整决策见 [`ADR-0013`](adr/0013-use-agent-attachment-for-durable-machine-responsibility.md)。
...
- **物**：`workspace_runner_state.go` 的 constructor/connection ownership 与 current-connection fence；`workspace_runner.go` 的 Runner-owned frame loop/dispatcher；`workspace_runner_delivery.go` 的 current Runner resolution；`workspace_runner_state_test.go`、`workspace_runner_test.go` 与 `workspace_runner_delivery_test.go` 的 identity、shared dependency、ready/ping/reconnect、serialized writer、FIFO/concurrency、stale callback、replacement cancellation、旧 map 不存在与 compile-time `Run(ctx)` guard。
