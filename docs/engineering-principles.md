...
- Steering 的破坏性范围只能来自 `ComputeSteeringImpact` 的最小结构闭包：Branch descendant、`decomposes/refines` 下游、`depends_on` 依赖者、命中 target 的 Task 及其 Task dependents。语义关联边不得被当作失效传播边；闭包外 Task 和既有证据必须保留。
