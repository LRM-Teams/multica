...
- **SSH 与执行 ownership**：SSH 只以受保护 Environment 的专用 key 进入 `dev`，不允许 root 登录；remote temp 与 `/data/multica` 均归 `dev`，终局删除 remote temp。禁止把密码写进 workflow，禁止关闭 host-key 校验，禁止让 production deploy 依赖 Aliyun 到 GitHub Actions task endpoint 的持续出站连接。
