...
- **Notes 页助手泡泡**：选中笔记时右下角 FAB 打开绑定 `chat_session.context_note_page_id` 的 standalone 对话（非 Worker→Messages）。Agent 可读该页及其子树（`notes get` / `notes tree`）；delivery 前缀 `<note_chat_context>`。合同见 `docs/notes-editor-worker-contract.md` § Notes assistant bubble。
