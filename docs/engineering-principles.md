...
| "扫不到"的前提是"知道要扫什么"——负向扫描（0 命中=干净）必须先验证模式覆盖目标的全部形态，并配一条正向对照（直接对照原文 vs 呈现，不依赖猜对模式）。内部 handle 具体规矩：断言必须同时覆盖 `actor_<N>` 与 `agent_<hex>` 两种格式，只写一种=一半漏检不到 | ⑤测试（handle 双格式可直接进断言模板） | #649 验收近失：Iris 按截图扫 `/actor_\d+/`，实际泄漏体是 `@agent_4fce5ed5`——若真漏了该正则报"0 命中、干净"；救场的是 fiber 直接对照（原文 handle vs 屏幕显示名），不是扫描 | ⛔仅文档 owner @iris 自签，断言模板待 #537/#531 落地时成用例 |
