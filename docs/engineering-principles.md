...
| 被推翻的书面结论必须当场改掉原记录——"我认错了"≠"记录已修正"（认错发生在聊天里，记录活在任务卡/文档里，前者不会自动传播到后者，而后者才是下一个人读的）。三种病形共一个根：**没写**（低害：人去查）、**写了但过期**（高害：人照做）、**写了但一开始就错**（高害且更难发现，没有"曾经对过"的线索）——根 = 坐在权威位置上、没人负责让它保持真 | ⑥review 红线（结构候选：任务卡"方向"字段带日期+被否即划线，待 owner 评估） | task #537 旧 fix direction 被否两次仍挂任务卡权威位（实测：照做会把契约 3 打红）；Iris `#MUL-123` 笔误挂设计稿权威位被 Frank 当规范读 | ⛔仅文档 owner @Felix/@iris 合签 |
| 撤回结论时必须同时搜它的下游——"我改了源头"≠"记录已修正"：结论会被抄进任务卡/文档/别人的稿，抄走的那份不随你的撤回自动更新；对"从各人稿抄进总台账"的工作方式这是结构性风险非个人疏忽 | ⑥review 红线 | #537 任务卡案例：错误方向被三道推翻（Felix 认错/Barry 打掉/五契约否决），但没人想到它被抄进了卡里；Iris `#MUL-123` 未进 repo 靠的是"抄的恰好是另两份稿"=运气非纪律 | ⛔仅文档 owner @iris 自签 |
