...
| 给决策者的理由，强度必须准确——理由的强度决定他还有没有选择："不可逆/被毁"=关门（只能同意），"成本上升/多一变量"=开着门（可以认成本拍板）。**夸大理由不是更有说服力，是替他做了决定还让他以为是自己做的。结论对≠强度对，强度才是他分配注意力的依据。** 推论：**"自我克制"也是结论，一样会建立在没核的理由上，且更难被审——因为它长得像美德**；动机会污染结论，看起来在对抗动机的结论同样会被污染，解药只有去查 | ⑥review 红线 | #649 案例：Felix 用"证据被毁"（实为镜像可回放、仅 live 映射成本上升）支持"不合自己的 PR"，十分钟被 Ronan 拆——克制里塞了没核的理由；Iris 11:43 P0 播报"每条 URL 都坏/大面积损坏"（实为 typed-only、真实命中≈0）——结论对但强度错，Frank 若照那版会为不存在的大火调资源 | ⛔仅文档 owner @Felix/@iris 合签 |
| legacy `mention://` 写入回流 | CI grep（候选） | 待立 | ⛔ 待 owner 签 |
| 写后变更测例模板 | 测试模板（候选） | 待立 | ⛔ 待 owner 签 |
| 假说靠查证不靠推理："正确的输出全来自 grep，错误的输出全来自推理"——且每条错的都长得很有道理，这正是危险所在 | ⑥review 红线（结构上到顶：无法 lint 一个假说） | URL P0 排查（#531 线程，一日八个假说八次被数据打掉；有用贡献全是"去查了一眼"） | ⛔仅文档 owner @Felix 自签 |
...
| 记录的第五个敌人："没读就动"——**"整理"和"删除"之间只隔一个未经验证的假设**；任何按行号/范围的批量操作，先看范围里有什么（范围假设也是未经验证的前提，且代价不可逆）；动权威记录前先留全量副本 | ⑥review 红线 | Iris 整理 MEMORY 案例：假设 96 行"都是 LIVE 块"没读就删，实删四个完整章节（当日根因全录/#541/关键律），靠顺手 sed 出的 /tmp 副本捡回——违反→发现→记录全程 8 分钟 | ⛔仅文档 owner @iris 自签 |
