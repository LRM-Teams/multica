...
> （活反例：2026-06-29 commit `8fa3f2f90` "keep issues out of mention picker"——一个被亲手拆掉的设计，commit body 为空、无理由无任务号；17 天后同一个设计差点被原样重建，因为"为什么拆"只存在于记忆里。）
...
**"以后照这个查"的方法必须先证明能重现你这次的发现**（Iris 案：实际用函数名 grep 找到四处，写进规矩的却是 `?? content`——照写下的执行=命中一个已修的、得出"全清"假结论）。**重现不了=你写的不是你做的。**
  ...
  互为两面（"看起来在工作"是最贵的假象——装置是"绿着但没测"，方法是"搜着但没找"）；且后者能逮住前者盖不住的东西：**一个方法可以从头到尾没机会红**。方法选错是能力问题，下次学会就好；**"写下来的不是做过的"是转录问题——与做得多好无关，产出的规矩会在别人手里安静地失败**。**复制=把我看到的拿走**：剪贴板必须与屏幕一致（`@小雅` 非 `@actor_14`），round-trip 归 mention picker 不归贴文本。
...
- **"看起来等于原文"≠"是原文"**：渲染碎片拼回去可能正好复原——**那正是它最会骗人的时候**（P0 案：textContent 重拼="干净"→"BE 没问题"的假结论把全队钉死在读路径一上午；真相=写入时已损坏落库）。**要看原文就去查存储原文（DB/API），别拼渲染结果。** 写入面（编辑器 preprocess）与读取面是两条路，各需自己的回归——渲染侧全绿拦不住写入侧的损坏。
