...
- Passport Prompt 的 representation shadow 必须比较每个保留 Source/Observation/Claim/Evidence Link 的 canonical bytes、hash 和父子嵌套；只比较顶层 ID 或集合数量不构成等价证明。Claim 内嵌 Evidence Link 仍是独立工件，必须携带内部 Artifact ID 并按 Manifest 单独过滤，不能因父 Claim 可读而继承可见性。
