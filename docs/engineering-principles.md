...
- Integration 近重复只能产生“候选合并”，不能直接改写 canonical 实体。Claim、Question、Hypothesis 必须同 kind，且访问权、非终态、scope、method、time 均相容；相似度必须来自服务端有界 signals 或相同语义指纹，拒绝理由按稳定枚举排序。`EvaluateIntegrationMergeCandidate` 冻结该决策边界并使左右输入对称；持久化候选、计算 Adapter 和应用合并的事务/Event 未接线前，不得宣称生产 Integration 已自动去重。
