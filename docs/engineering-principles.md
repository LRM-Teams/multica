...
- 每个 V6 execution Task 必须通过 `research_task_inquiry_target` 持久绑定 1–32 个 ordered typed Inquiry targets；绑定与 Task 创建属于同一个 accepted Plan Result 事务。目标存在性由 migration 352 在数据库层复核，不能把只存在于 Prompt/Result JSON 的 key 当作执行范围。
