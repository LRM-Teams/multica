...
- Research Retrieval 失败不得由自由文本直接复制 Query。服务端恢复矩阵只允许：retryable 失败原请求退避重试、过期 cursor 清空重启、not-found 改写 query、许可失败切换 Adapter、unsafe-target 严格缩小 scope，或对确定候选执行排除；除被许可字段外，语言、时间窗和范围必须保持冻结。失败事实、Goal/Plan、原 Query Execution 和新决定生成稳定 recovery key，相同决定不可重复创建；同类失败与总尝试预算耗尽时服务端直接停止。当前可执行边界为 `server/internal/researchrun/retrieval_recovery.go`；持久 Query Execution 接线前不得宣称失败查询已自动恢复。
