...
- **物**：`docs/notes-editor-worker-contract.md`；`docs/agent-memory-model.md` §10；migration `338_note_worker_job` / `420_chat_session_context_note_page` / `440_note_page_virtual_share`；`note_intent.go` / `note_brief.go` / `note_worker_prompt.go` / `note_writeback_events.go` / `appendAgentNoteWritePart` + 误用、dispatch/ACL、prompt breakout、白名单、`--note-write` 测；`note_chat_context.go` / `ListAgentNoteTree` / Notes FAB；`requestInlineNotePageAI` + EmptyLineAiMenu `onRequestPageAI`（无笔记助手时回车发送才开边栏配置卡）。
