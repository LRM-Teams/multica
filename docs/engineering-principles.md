...
| 绿可能是因为错的理由绿的——两个机制在某格给出同一答案时，分歧被掩盖；矩阵缺的永远是"两个机制会不一致"的那一格，设计用例时专找它（oracle 失败+兜底恰好同答 ≠ oracle 验过） | ⑤测试 | #537 案例：`x.中国 ✅` 是因为 oracle 对 IDN TLD 全瞎、兜底"保留整串"恰好同答——能暴露分歧的 `x.中国吗` 不在矩阵里（Felix 跑出）；同形状：fixture ordering 掩盖 #616 | ⛔仅文档 owner @Felix 自签；`x.中国吗` 预期不裁已成钉死测试 |
