...
- Product/API 只暴露 `resetAgent(agentId, mode)`，mode 固定为 `restart | session | full`；没有 public `action_kind`、`execution_mode` 或 `after_current_run`。`AgentRestartOperation` 持久化 `stopping → resetting_workspace? → starting → terminal`；现有 `agent_lifecycle_operation` 仅作为 legacy physical table name，不是产品 contract。
