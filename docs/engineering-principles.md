...
- 每个 dispatch Manifest 候选版本必须恰好得到一种可审计 disposition：作为 entry 被接纳，或以精确 policy reason 写入 omission；候选族必须由完整 Artifact registry 分区，未知新类型 fail closed，禁止因 SQL 白名单或解析跳过而静默消失。
