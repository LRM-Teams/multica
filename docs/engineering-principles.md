...
- Inquiry 状态不得裸 UPDATE：`TransitionInquiry` 必须同时校验 Run state version、stored before status、assigned Attempt/Agent，以及该实体当前 Artifact Version 确实位于 Attempt 的冻结 Manifest；领域状态、新 production Version、`revises` 输入引用、policy mutation/eligibility revision 和 before/after/reason Run Event 必须原子提交。
