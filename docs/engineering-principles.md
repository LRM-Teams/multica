...
### 4.15 Agent 重启控制面对齐 Raft 1.0.16 离散生命周期 — `可执行`（②服务端持久 orchestrator + ⑤三模式/断线恢复回归；owner: @Codex）
- Raft 1.0.16 的 Computer wire 只有 `agent:stop`、`agent:reset-workspace`、`agent:start`，没有 `agent:lifecycle`。`agent_lifecycle_operation` 持久化 `stopping → resetting_workspace? → starting → terminal`；Runner 只执行单步本地命令。禁止 heartbeat pending queue、复合 lifecycle payload、另一套 stop/start 投影或 delivery lease。
- Stop 先把旧 `launchId` 持久化为 `stop_launch_id`，只有该 launch 的 inactive status 才推进；Runner 必须先撤销 APM admission、fence 迟到 startup，并等待 startup owner、provider lease 与 resident process 全部 quiescent 后才能发布 inactive。Start 先持久化新 `launchId/startDispatchId`，且 accepted start 在 ACK write 前已有唯一 startup owner。Restart 复用本机持久 provider session；两种 reset 通过 `config.sessionId=""` 显式 fresh。Full Reset 必须在上述 stop 证据后删/重建 AgentRoot，并且 Server 收到 reset terminal result 后才允许 start。
- Runner ready 按持久 step 恢复；stop/reset 阶段屏蔽通用 desired/observed start reconcile，starting 阶段复用它。relay publish 不等于 accepted/completed；未送达操作保持 running，直到 ready 恢复或 timeout。Activity 只消费正常 `Stopped → Starting → Idle/Working` 事实，业务 step 不伪装成 runtime Activity。
- **物**：离散 `WorkspaceRunnerAgentStopPayload` / `WorkspaceRunnerAgentResetWorkspacePayload+Result` / `WorkspaceRunnerAgentStartPayload.SessionID`、`agentLifecycleOrchestrator`、status/reset-result 驱动回归，以及 production resident prestart 的 provider-session 回归；旧 `AgentLifecycleDispatchStore`、`WorkspaceRunnerAgentLifecyclePayload/Ack/Result`、heartbeat lifecycle envelope 和本地 `.multica/lifecycle-commands` ledger 不得恢复。
...
- V6 的结构合同独立冻结在 `docs/contracts/research-run-v6.schema.json`，覆盖 Plan Result、Task Result、Prompt Context 与 Gate Input；跨对象规则见 `docs/research-run-v6-contract.md`。V6 字段不得作为共享 optional 字段加入 V1–V5 envelope 来绕过 strict decoder。默认版本、supported-version 列表和生产 decoder 在 E–K 退出门通过前必须保持 V5/unsupported；合同 hash 变化代表 V7，而不是原地修改 V6。
...
- 控制 heartbeat 必须在 Attachment replay 完成后启动；replay request 的 cursor set 是 Runner 当前本地 Runtime set，Server 逐个校验 Computer/Workspace ownership，不得要求它等于该 Computer 的全部历史 DB Runtime 行，否则已卸载 provider 的离线残留会永久阻塞握手。reconnect replacement 先 fence 旧 socket，再由新 current Runner 恢复。多 Workspace 共用一台 Computer 时每个 Binding 独立发送和鉴权，禁止跨 Workspace 或跨 Computer 执行动作。
