...
- Research Integration Round 的调度决定必须绑定版本化 Policy、Goal/Plan version、连续 Event 水位和每个 accepted Result Artifact 的 Passport、Version、content hash、原 Task/Attempt/Agent。至少两个原 Agent 的新结果才能触发跨 Agent 整合；阻断冲突、待交付、结果阈值和信息增益有固定优先级，活动轮次与轮次/成本预算先行阻断。相同冻结输入必须产生相同 Round Key，旧水位结果不得被重复消费。当前可执行决定边界为 `server/internal/researchrun/integration_trigger.go`；持久轮次预留和状态应用接入前不得宣称 G 章完成。
