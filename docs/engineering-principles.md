...
- V6 主理人不得因单个 Work Item 失败自行暂停整场 Run；Run 暂停只属于用户 Stop 与发布维护控制。Director Brief 必须携带失败 Work 的小目标、Attempt 次数/预算、最新失败分类与诊断及终止原因，使主理人能够重试、改派或创建替代工作。Work 投影必须回退读取最新 Attempt 失败事实，不能把真实错误压成 `other / terminal canonical state`。历史上由 `v6_pause_run` 造成且没有用户 Stop 标记的暂停 Run 在发布控制开放时自动恢复。**物**：Director action schema、migration `440_research_v6_director_pause_recovery`、`RonaldoV6DirectorSystemProtocol`、`LoadDirectorBriefFacts`、`projectionTerminationForWork`、`TestDecodeV6ContractRejectsDirectorPauseRun`、`TestV6DirectorBriefIncludesWorkFailureRecoveryFacts`。
