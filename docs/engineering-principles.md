...
- V6 默认画布把 Work S 当作「Agent + Work Item + Attempt」的执行投影：运行中 Work 必须直接读取 `research_v6_work_item_branch` 的已分配 Branch scope；已产出 Result 的终态 Work 用非 canonical `collapsed_path` 收束，终态 Director 控制 Work 只留在 canonical slice，不占默认研究画布。选中 Work 后的 Agent、Inbox Task、进度来自独立 activity read model；该 read model 直接按 Attempt 所绑定 Inbox Task 的真实 `started_at` / `completed_at` 生命周期窗口读取有界的持久化 Runner Activity，dispatch 时间不得冒充执行起点。既有 `agent:activity` Query-cache 写入只负责叠加实时过程，匹配 Inbox Task 的 `task:running`、progress 与终态事件负责重取 lifecycle/progress。Projection 节点 revision 必须包含最新 Attempt、Inbox Task 生命周期与 durable progress 的时间戳以支持重连恢复；不得使用只在 WS 内存中存在的 `task:message`、轮询、Zustand 或坐标状态。**物**：`postgres_projection_v6.go`、`research_v6_work_activity.go`、`star-graph-layout.ts`、`director-session-adapter.ts`、`research-v6-node-detail.tsx` 及对应回归。
