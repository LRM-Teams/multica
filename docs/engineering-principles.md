...
- **先让人看见，再让 agent 决策**：普通群的 system row 必须先以其 message UUID 作为稳定 realtime event id 发布成功，publication fence 才允许生成 target-only onboarding inbox lease。system row 本身不唤醒 agent；除新加入的目标 agent 外，其他 agent inbox 必须为 0。在线路径的可见顺序固定为 `joined system event → Agent intro`。system `#general` 是 workspace roster，不是一次邀请：入群事件照常 durable publication，但 onboarding 直接以 `system_general_silent_join` 完成，不创建 provider turn、不消耗 token；普通群手动加 Agent 的决策回合不变。
...
- Agent Presence 只有 `online|offline`：服务端由当前 ready WorkspaceDaemon 连接与该实例持有的 active managed launch 共同投影。Runtime heartbeat、Task workload、Runner Activity、Health、provider quota、crash reason 都不能覆盖这个结果；加载或坏响应不造第三态，也不冒充 Online。
...
- **发布分支决定版本成熟度**：`dev` 只允许 `vX.Y.Z-alpha.N` / `vX.Y.Z-beta.N`，写入 test 包源；`main` 只允许 `vX.Y.Z`，写入 production 包源。禁止 feature branch 发版、`dev` 发正式版或 `main` 发预发布版。发布者必须给目标远端分支中的提交打 tag，不能给本地工作分支打 tag。**物**：`.github/workflows/release.yml` 的 tag 格式与 source-branch ancestry gate；Agent 操作说明见 `docs/release.md`。
...
- **liveness 是 Computer 级的事实**：由 daemon 的 WorkspaceDaemon WS socket 决定（`computerConnectedByRunner` → `DaemonHub.HasWorkspaceDaemon`），HTTP 心跳只是 legacy fallback。**不得再用 runtime 的 `status` / `last_seen_at` 推在线**——那条路径随 runtime 全面走 WS 已经过时。
...
- daemon 内只有一个 update-observation coordinator；自动轮询和 server 下发更新都必须通过它写入。每次语义变化先把完整 snapshot 原子持久化到本机，再唤醒 current WorkspaceDaemon control heartbeat；持久化失败时拒绝开始更新、重启或对外宣称新状态。进程重启后创建新 `session_id`，`revision` 从 1 开始；未终结的 `checking|updating` 归一为 `interrupted`，已成功的 `restart_pending` 结果必须重放。
...
- V6 默认画布显式投影 Run-scoped Team Membership Agent 节点，并以 `assigned_to` 连接 Work 与执行 Agent；Work 标题取 Director 冻结的任务使命，不能用 Agent 名称制造一圈同名假任务。Agent Projection 必须区分 `idle`、`running` 与 `offline`；谱系视图必须把 `succeeded`/`accepted` 显示为完成，不能把空闲成员或终态结果计作等待 Work。运行中 Work 必须直接读取 `research_v6_work_item_branch` 的已分配 Branch scope；已产出 Result 的终态 Work 用非 canonical `collapsed_path` 收束，终态 Director 控制 Work 只留在 canonical slice，不占默认研究画布。选中 Work 后的 Inbox Task、进度与工作过程来自独立 activity read model；`task:message` 先按精确 Attempt+Inbox Task 身份投影为有界、脱敏的 `research_work_item_activity_entry`，再发布 WS 刷新信号。页面只读该持久化表，不按 Agent+时间窗拼接通用活动，也不展示隐藏推理、原始工具结果或未识别工具参数。Projection 节点 revision 必须包含最新 Attempt、Inbox Task 生命周期与 durable progress 的时间戳以支持重连恢复；不得使用只在 WS 内存中存在的事实、轮询、Zustand 或坐标状态。**物**：migration `443_research_v6_work_activity`、`postgres_projection_v6.go`、`research_v6_work_activity.go`、`activityprojection/projection.go`、`star-graph-layout.ts`、`director-session-adapter.ts`、`research-v6-node-detail.tsx` 及对应回归。
- V6 Director 只能规划、组队、派工和整合，不能把原子调研 Work 指派给自己。原子 Work 必须使用 `atomic_result_submission`、非 `no_op.v1` 的 `payload_schema_id`，并携带非空 `payload.task_specific_schema`。同一 proposal 中新建 Agent 后必须等待 joined 再派工；若派工因合同拒绝且已有空闲专属 Agent，下一轮必须修正合同并重新派工，不能以 `no_op` 掩盖失败。运行中没有专属成员、没有待处理 Agent 创建时同样不得 `no_op`。**物**：`director_prompt_v6.go`、`director_actions_core_v6.go`、`director_actions_lifecycle_v6.go` 及对应回归。
- 每条成功投影的 committed Research Event 必须同时保留 V5 graph event，并发布独立的 `research_projection_v6:delta`。因为 Director Projection Delta 的身份（`snapshot_id` + projection hash 链）按客户端快照固定，一条广播帧不可能携带对所有订阅者都能验证的 Delta，所以 WS payload 只承载 run-scoped 序号推进信号 `{run_id, through_sequence}`，sequence 必须来自该 committed Event；Delta 本体必须由客户端经认证 HTTP resume 按自己的快照增量获取，Node/Edge 身份复用 Snapshot mapper。实时路径不得广播带全局 `snapshot_id` 的 Delta、把旧 V5 payload 冒充 V6 Delta，也不得让客户端因无法解析帧而整图重拉。**物**：`research_run_adapter.go` 的 `researchV6SequenceAdvanceEnvelope`、`director-controller.ts` 的 `catchUp`/`resume` 及对应回归。
...
- V6 的自然语言执行进度、智能体协作消息、错误说明和交付摘要默认必须使用简体中文；JSON 字段、枚举、命令、代码、专有名词和来源原文保持合同原样。运行时违反语言要求时，Research 消息镜像边界不得把纯英文操作流水写入用户会话，必须替换为中文状态摘要；隐藏推理、CLI 探查和 JSON 拼装过程无论语言都不得成为产品输出。**物**：`RonaldoV6DirectorSystemProtocol`、`BuildV6WorkDispatchPrompt`、`visibleV6ResearchReply`、`TestVisibleV6ResearchReplyRejectsEnglishOperationalNarration`。
...
- V6 Result/Insight 已持久化的 `open_questions` 必须进入下一轮有界 Director Brief，不能只在节点详情展示后从控制环消失。为保持冻结 V6 envelope 不变，Context Compiler 将问题确定性携带在对应 Frontier `brief_summary` 中；Director 必须为仍有价值且没有活动 Work 覆盖的问题创建或改派 Work，出现能力、容量或独立性缺口时再创建 Agent，不继续则记录收敛理由。已覆盖 Result Event 的历史 Brief 若有 Frontier 节点但遗漏问题，reconciler 必须自动创建一次修复 Cycle。**物**：`directorBriefFrontierSummary`、`ProcessV6EventTriggers`、`RonaldoV6DirectorSystemProtocol`、`TestDirectorBriefFrontierSummaryBoundsOpenQuestions`、`TestV6DirectorBriefIncludesAtomicResultFrontier`、`TestV6EventTriggerRepairsAtomicResultMissingFromCoveredBrief`。
...
- V6 主理人不得因单个 Work Item 失败自行暂停整场 Run；Run 暂停只属于用户 Stop 与发布维护控制。冻结 V6 schema 保留形状词汇，但执行授权必须拒绝 Director 的 `pause_run`。Director Brief 的既有 `summary` 字段必须携带失败 Work 的小目标、Attempt 次数/预算、最新失败分类与诊断及终止原因，使主理人能够重试、改派或创建替代工作。Work 投影必须回退读取最新 Attempt 失败事实，不能把真实错误压成 `other / terminal canonical state`。历史上由 `v6_pause_run` 造成且没有用户 Stop 标记的暂停 Run 在发布控制开放时自动恢复。**物**：`v6DirectorRunLifecycleTarget`、migration `440_research_v6_director_pause_recovery`、`RonaldoV6DirectorSystemProtocol`、`LoadDirectorBriefFacts`、`projectionTerminationForWork`、`TestV6DirectorRunLifecycleRejectsAutonomousPause`、`TestV6DirectorBriefIncludesWorkFailureRecoveryFacts`。
...
- V6 Inbox 若把同一非 Director Work 的全部 Attempt 以可重试 `agent_error.model_not_found_or_unavailable` 终止，说明是 provider/model 容量故障而非调研合同失败：恢复器在预算耗尽后只允许一次带两分钟退避的自动重开，并以稳定 Run Event key 封住第二次重开；Attempt 终态且该成员无其他 active Attempt 时，membership 必须回到 `idle`，不能让星图永久显示“工作中”。若失败 Work 仍存在且当前没有 active Agent Work，Director 不得 `no_op`，必须重试或改派。**物**：`RecoverExpiredV6WorkItems`、`recordV6DirectorNoOp`、`TestRecoverV6RetryableRuntimeCapacityFailureReopensOnce`、`TestV6DirectorCannotNoOpWithFailedWorkerWork`。
...
### 4.19 Agent 消息链路硬切到 Raft 风格 coordinator — `可执行`（WorkspaceDaemon 单一 Inbox + 本地 coverage receipt + MessageDraftStore）
...
- Agent 本机 admission 只认 current WorkspaceDaemon 接受的 `agent:start`；`agent:stop` 删除该 launch。不存在独立 Attachment registry、attach/detach wire、generation、replay cursor 或本地兼容 ledger。Message coordinator 可从同一 Runner 的 APM start snapshot 修复，不能读取第二套 ownership state。
...
### 4.19.2 WorkspaceDaemon identity 不随 Runtime 变化 — `可执行`（② narrow constructor + ③ WorkspaceDaemon-owned Message/Activity behavior + ⑤ executable ownership guards）
- private `workspaceSessionConfig` 固定 `DaemonID + DaemonInstanceID + WorkspaceID`；Runtime set 是可变输入，不进入 WorkspaceDaemon identity。缺任一 identity 或 dependency 时 constructor fail closed。
- `WorkspaceDaemonCore` 只拥有一个 private `workspaceSession`；该 session 持有 Process Manager、Activity producer 与 `InboxRegistry`。canonical Runtime pool 与 diagnostics 只注入引用，不复制状态、不另建 singleton。Credential Proxy 只解析当前 session 并调用 Message 方法，不取得 coordinator 或 registry。
- Process Manager 与 Activity producer 没有 Workspace map 或 lazy singleton；start/stop/probe/reconnect 都通过同一 session-owned instance。socket reconnect 只替换 transport，保留 launch identity 与 Activity sequence；WorkspaceDaemon teardown 才释放 process/Activity/Inbox state。
- `workspaceSession.Run(ctx)` 唯一持有 Workspace auth、dial/reconnect backoff、ready identity、connection context/cancel、ping/pong 与 serialized frame writer；replacement 先 cancel/close 旧 connection。socket callback 是 private implementation，不引入第二个 transport component。完整收敛决策见 [`ADR-0020`](adr/0020-converge-computer-daemon-workspace-daemon.md)。
- 每个 live connection 自己持有 per-Agent Delivery dispatcher 与 current-socket Message callback：同 Agent FIFO、不同 Agent 可并行；callback 只在 exact connection 仍 current 时可写，replacement 后 stale callback fail closed。WorkspaceDaemonCore 只保存这一个 session，不保存 parallel transport/generation map。
- `InboxRegistry` 固定一个 Workspace scope；仅在该 scope 内 Runtime ownership 匹配时创建 coordinator。Delivery、coverage 与 reconnect 都经过 current session；网络 reconnect 保留 registry 和进程内 Context Boundary，WorkspaceDaemon close 只关闭自己的 Inboxes。
- **物**：`workspace_daemon_state.go` 的 narrow constructor/connection ownership 与 current-connection fence；`workspace_daemon_message.go`、`workspace_daemon_activity.go` 的行为归属；`workspace_daemon_delivery.go` 只返回 current session、不暴露 coordinator；`workspace_daemon_state_test.go` 的 whole-core dependency 与 field reach-through source guard，以及 identity/reconnect/isolation/recovery 回归。

### 4.19.4 DaemonCore 按 Raft 对账 WorkspaceDaemonCore — `可执行`（③ reconcile + ⑤ crash/degrade/process identity tests）
- **口径（2026-08-24）**：`ComputerCore` 与每个 managed `WorkspaceDaemonCore` 是独立 OS process。local `DaemonCore` 每 5s 对账 desired Binding；crash 后 2s backoff；60s 内 3 次 crash 进入 degraded、不再自动拉起。摘掉 Binding 是 graceful stop，不是 unlinked/degraded。
- **process boundary**：每个 WorkspaceDaemonCore 持有自己的 `workspaceSession + AgentProcessManager + Inbox/MessageCoordinator + Activity + provider runtimes`。DaemonCore 通过 Unix socket / Windows named pipe 上的 length-prefixed JSON RPC 管理 child；Credential Proxy 仍是独立 loopback HTTP。DaemonCore 在 spawn 时只登记 process handle/PID；`daemonInstanceId` 等 child Ready 自报后再记录。
- **process identity fence**：每次 resident 启动生成 UUID `serviceGeneration`。DaemonCore 不预发 process 门票；WorkspaceDaemonCore 自己生成 `daemonInstanceId`，Ready 时按进程句柄 + PID 收下。之后 local control、Runtime report、diagnostic、capacity grant 和 exit observation 校验 exact Workspace + `daemonInstanceId` + PID；stale process 必须 no-op / fail closed。当前 wire 与持久化 JSON 只使用 camelCase。
- **package boundary**：`internal/computer` 的 `ComputerCore` 拥有 resident/health、diagnostic aggregation 与 Machine Upgrade；同包 `DaemonCore` 拥有 desired set/supervision/crash/backoff/machine capacity/local child control。`internal/daemon` 只拥有 `WorkspaceDaemonCore` execution。Public resident 直接 `computer.NewComputerCore → ComputerCore.RunProcess`，禁止构造或依赖 `daemon.WorkspaceDaemonCore`；反向也禁止 daemon 生产代码持有 `computer.ComputerCore`。CLI 只 wiring，不新增平行 host/supervisor package。
- **Machine Upgrade**：ComputerCore 让 DaemonCore prepare 所有 current children；每个 WorkspaceDaemonCore drain/terminate 自己的 execution。任一 prepare 失败必须 release 已暂停 siblings；already-current / rollback re-register 也由 child-reported `daemonInstanceId`-fenced child 执行，ComputerCore 禁止 provider probing。
- **物**：`computer.ComputerCore` / `ComputerCore.RunProcess` / `hostMachineUpgrade` / `computer.DaemonCore` / private local control/capacity helpers / WorkspaceDaemon process handle；真实 bootstrap/Ready/control process protocol；per-WorkspaceDaemon state root；双向 architecture guard；process-identity/crash/sibling/capacity/Machine Upgrade tests。
...
### 4.19.8 Computer 控制动作只走 current WorkspaceDaemon — `可执行`（② capability + ③单一 carrier + ⑤正反向回归）
- 声明 `workspace_runner_control_plane_v1`（保留的 wire 值）的 current WorkspaceDaemon 是该 Computer-Workspace Binding 内 Runtime heartbeat 与 Server pending action 的唯一 carrier。WorkspaceDaemon 每次按当前 Workspace Runtime set 发送，Server 每次重新验证 Workspace、Computer 与 generation；ack 只在同一 WorkspaceDaemon socket、同一 Workspace Runtime scope 内消费。
...
- 控制 heartbeat 在 current WorkspaceDaemon Ready 后立即启动，不等待额外 lifecycle replay。Server 对每个 heartbeat 逐个校验 Computer/Workspace ownership，不得要求当前 Runtime set 等于该 Computer 的全部历史 DB Runtime 行。Reminder reconnect 另按 current Runtime set 请求完整 snapshot，在线 mutation 直接发送 version-fenced upsert/cancel。reconnect replacement 先 fence 旧 socket，再由新 current WorkspaceDaemon 恢复。多 Workspace 共用一台 Computer 时每个 Binding 独立发送和鉴权，禁止跨 Workspace 或跨 Computer 执行动作。
- Agent Restart 不进入 control heartbeat pending action；它只由 4.15 的进程内 orchestrator 经 current WorkspaceDaemon 投递离散 `agent:stop/reset-workspace/start`。heartbeat 不得恢复 lifecycle envelope 或本机复合 executor，否则会与离散状态机形成两条控制路径。
- **物**：`DaemonCapabilityWorkspaceDaemonControlPlane`；`workspaceSession.runControlPlaneHeartbeats`；`HandleDaemonWSHeartbeat` 的 Workspace/Computer/generation fence；WorkspaceDaemon heartbeat、exact Workspace Runtime set、legacy socket 与 liveness probe 回归；现场验收矩阵见 [`docs/daemon-control-plane-validation.md`](daemon-control-plane-validation.md)。

### 4.19.9 Computer / Daemon / WorkspaceDaemon 收敛为三层 owner — `可执行`（②单一 WS contract + ③组件收敛 + ⑤ burst/legacy 回归）
- 本机层级固定为 `ComputerCore → DaemonCore → *WorkspaceDaemonCore`。`ComputerCore` 只拥有机器生命周期、升级/重启、work journal、本地服务控制和 durable diagnostics；`DaemonCore` 只拥有 desired Workspace 对账、child identity fence、crash/backoff、共享 Agent process capacity 与 local IPC，本身没有 Server WS；每个 `WorkspaceDaemonCore` 是一个 Binding 的 OS child，拥有该 Workspace 的 Agent execution 与恰好一条 Server WS。私有 `workspaceSession` 只是连接状态，不是第四个 domain component。
- canonical endpoint 固定为 `protocol.WorkspaceDaemonConnectPath = /api/workspace/daemon/connect`。`/api/daemon/connect?workspace_id=...` 仅作为已发布旧 Computer 接收 `computer:upgrade` 的升级桥；只有最低支持版本与 fleet upgrade coverage 均证明无旧 Computer 依赖时才能按代码 TODO 删除。runtime/task 形式的 `/api/daemon/connect` 是另一条 legacy transport，不能与 WorkspaceDaemon slot 耦合。
- 不增加 Computer WS。`computer:*` 可借 current WorkspaceDaemon WS 到达本机，再通过 generation-fenced IPC 交给 `ComputerCore`；WorkspaceDaemon 只承载，不拥有 machine operation 的执行、journal、幂等或 attestation。
- Gorilla WS 保持单 writer goroutine。每条连接使用有界 priority/normal outbox；上限是内存安全预算，不由 Agent 数决定。Agent reconcile burst 走 normal lane，稀有 `computer:*`/liveness frame 走 priority lane。队列压力只能拒绝新 enqueue，禁止 unregister/close 健康连接；replacement、鉴权失败、真实 I/O 错误或 inbound watchdog 才能关闭。
- WorkspaceDaemon diagnostics 只经 local IPC 上报；生产 durable writer 的唯一 owner 是 `ComputerCore.diagnosticLoggers[workspaceID]`。禁止 WorkspaceDaemon 再开 parallel diagnostic store。
- **物**：[`ADR-0020`](adr/0020-converge-computer-daemon-workspace-daemon.md)；`computer.ComputerCore`、`computer.DaemonCore`、`daemon.WorkspaceDaemonCore`、private `workspaceSession`；`protocol.WorkspaceDaemonConnectPath`；`daemonws.clientOutbox`；legacy bridge TODO；WorkspaceDaemon burst 不断线与 route/path tests。
...
- **Editor 公式（2026-08-24）**：空行 / 页内 `note_ai_job` 的 `markdown` 只用 `$...$` / `$$...$$`。`\[`、`\(`、` ```latex ` 编辑器当源码。`buildNotePageEditPrompt` + `note-ai-edit-prompt.test.ts`。
- **导出公式（2026-08-24）**：HTML/PDF 必须把 `$` / `$$` 渲成 KaTeX。PDF 打印窗复制当前页同源 KaTeX CSS，并等 `document.fonts.ready`；禁止依赖 jsDelivr 在打印前加载字体。`note-export.ts` + `note-export.test.ts`。
- **Notes 页助手泡泡**：选中笔记时右下角 FAB 打开绑定 `chat_session.context_note_page_id` 的 standalone 对话（非 Worker→Messages）。工作区级专用 Agent「笔记助手」(`notes-assistant`)；首开 soft-probe 不自动创建，对话里出引导卡，仅点「对齐 Wendy」或「自己选择电脑和运行时」（预填身份的创建智能体对话框）才 Ensure。删除后再开同样出卡。气泡内无 Agent 下拉。桌面右边栏可拖左缘调宽（280–640，`noteBubbleSidebarWidth` 一份 store，正文留白与栏同步变窄/变宽）；打开时 Notes 正文行按栏宽留白（`noteAssistantSidebarReservePx`），编辑器 `mx-auto` 在未覆盖区域居中。划词工具栏的 sparkle **不再**走 `note_ai_job` 选区优化/diff 应用：点击立刻打开边栏，输入框顶部用缩略引用展示选区（`noteSelectionQuote`，session-only）。同一页可连续划词追加多段（各自一颗芯片，可单独取消）；用户提问后把全部选区按顺序作为 markdown blockquote 一并发出。空行 AI 行首空格始终弹出输入条；有笔记助手时回车走 Editor job；没有笔记助手时回车才打开气泡右边栏配置卡（与首次点 FAB 相同，`requestInlineNotePageAI`）。扫过泡泡消息：复制后是「插入笔记下面 / 插入子笔记」（写入当前页或子页）。打开时 ESC 收起右边栏（与头上 Minus 相同；对话框/菜单/会话弹出层先关）。关闭且未进过场时栏不进 DOM（`noteAssistantSidebarPresence` omit）。禁止用 `motion.div` + `initial={false}` 藏关闭态——Motion 会写 `transform: none`，刷新后栏会露在 `right: 0`。Delivery 前缀 `<note_chat_context>` 仅含根页 id/标题，子页正文由 Agent 按需 `notes tree` / `notes get`（技能 `multica-notes-assistant`）。合同见 `docs/notes-editor-worker-contract.md` § Notes assistant bubble。
...
- **口径（2026-08-23）**：笔记排版分两层，禁止混写。**默认格式**（字体 / 字号 / 颜色）是用户偏好，存在 `multica_note_format`，用 CSS 变量画编辑器和导出；**不**写入 note markdown。入口是 Notes 页头三点菜单「默认格式」。**选区颜色 / 字号**是 Notes-only `textStyle` mark，存成消毒后的 `<span style="color|font-size">`；只接受调色板 hex 和 14/16/18/20/24px。Issue / 评论编辑器不注册该 mark，避免 Office 粘贴把颜色带进评论。
- **物**：`packages/core/notes/format.ts` + `format-store.ts`；`packages/views/editor/extensions/text-style.ts`；`NoteFormatDefaultsDialog`；`sanitizeTextStyle` 拒绝 `url()` / `expression()` / 未知字号。

### 4.24 写汇报：多 runtime 采集 + 笔记助手合成 — `仅文档`

- **口径（2026-08-17，ADR 0019；采集深度 2026-08-18 增补；结算/重采 2026-08-18；汇报叙事 2026-08-18；fresh session / 失败后仍收包 2026-08-18；无 stub 采集包 2026-08-19；Brief 板块 Summary/Technique/Achievements/Research 2026-08-19；严格时间窗 + 自定义起止 2026-08-19；Brief 无证据层/给别人看 2026-08-19；按事项归组非按时间 2026-08-19；采集端 Work groups 初分 2026-08-19；采集包隐式存放 2026-08-19；仅自己电脑 2026-08-19；可选 focus 由笔记助手先派发 2026-08-21；泡泡内点选+输入、文字优先、不跳消息页 2026-08-21；芯片一轮即失效、进度写进当前页会话、确认后插到下发页的子笔记 2026-08-21；空会话开芯片输入框不顶上去、⊕ 新建会话发写汇报不复用旧会话 2026-08-21；芯片卡可取消、发送后取消失效 2026-08-21；采集包/汇报稿折叠展示 + 插入笔记下面/插入子笔记 2026-08-21；笔记助手桌面改为右侧通顶边栏 2026-08-21；泡泡成稿只取本轮 note_write/底稿、禁止上一轮工作介绍/ 成稿 2026-08-21；采集员不静默创建、对话卡+锁定电脑选运行时 2026-08-21）**：入口是笔记助手卫星「写汇报」，或在泡泡里说出写汇报意图（`looksLikePeriodBriefRequest`）——后者先弹出同一套时间/电脑芯片，再点发送才开跑；**不再**用页头按钮或独立对话框。缺/删/绑错的采集员在芯片列表里出可忽略提醒（电脑锁定，只选运行时）；可忽略后继续用其余电脑。`EnsurePeriodBriefCollectors` 无 `runtime_id` 只探测不创建。条件芯片（时间段 + 自己电脑）只作用于 **发出去的那一轮**，与可选文字合成一条普通用户消息后芯片收起；**任务结束前锁输入**；结束后才恢复普通对话。进度（先让谁采、已分派、收到谁的包、材料齐了、问要不要插入）写进 **当前气泡会话**（有 `chat_session_id` 则沿用；⊕ 新建未带 id 则开新会话，禁止复用该笔记最近一条），关掉再开仍在。空会话弹出芯片时中间留白，输入框仍在底部。收到采集包和成稿后在泡泡里用折叠 `note_brief` 展示（采集包不是笔记页）。成稿提供 **插入笔记下面**（接到下发页正文后）和 **插入子笔记**（下发页的子页），不再落到全局「工作介绍/」。人勾选若干 **自己创建的 Computer** 上的采集员（含自己的云端 runtime；**不得**采他人电脑，即使 runtime 可见/`public` 或本人是 workspace admin）。人可附带可选 **采集要求**（路径/主题/方面，也可在文字里改时间/电脑）；**留空则全量采集（与现在相同）**。有要求时 **笔记助手先 collect-plan wake**（`submit-collect-plan`）理解需求并只派给所需采集员，再各自在 **所在 OS** 按范围搜集最近工作痕迹（`SCAN_ROOTS`：整机 HOME ∪ 沙箱 `/workspace` ∪ 可见项目目录，禁止只扫 HOME；允许短 diff、文件摘要、关键片段、窗口内非 git 源文件；denylist 去密钥噪声）。采集员可见该机最全信息，应做 **初步分类（`## Work groups`）**：同工程/同 git 仓默认一组；跨仓/跨文件但同一事项结果的合并为一组并写明 why；不相关工作分列——**完整性优先**，证据层（Repos/Highlights）不可被分组/图替代。采集包经 `submit-pack` 写入 `note_period_brief_run.collectors[].pack_markdown`（**不**建「采集包」笔记页）；**已 submit-pack 的包仍算 ready**，即使随后 inbox 任务记 `failed`；合成 wake 后 **清空** pack_markdown。平台 Facts 仍由服务端确定性拉取。平台 **按状态等到采集 settle**（不得用固定时钟把仍在跑当成 empty）；未交付 / 失败时状态板写明 **「调用采集 Agent 失败了」**，合成 **不得** 把空包当证据。采集 / 重采 / 合成每次 wake **`force_fresh_session=true`**（一次性 prompt，禁止续上一次 Pi 会话）。合成 wake 带 **状态板**（`retryable` / `abandon_why`）。永久失败（无 API key / 模型配置 / 鉴权 / 配额等）放弃；可恢复失败只由笔记助手经窄工具再采一次（inbox 不自动重采）；再失败即最终失败。未用尽这一次重采前不得说材料齐了。全部交给 Workspace 内 **笔记助手**（写汇报 wake，不可改选）做 Period Work Synthesis，产物是 Notes 里的 Period Work Brief。采集/合成 **严格按所选时间窗**（日/周/月或自定义起止，半开区间）；**汇报叙事**：固定板块 `Summary`（必有 `Work Summary` 详述本期工作 + `Next Steps` 可据现状推测）以及按需的 `Technique` / `Achievements` / `Research`（无相关工作则整节省略、禁止空壳）；Work Summary **从采集 `## Work groups` 展开**（一组一主标题，组内再细分；跨采集员仅在同一事项身份时合并；不因时间穿插拆开；不相关工作不因同窗并句）；标题与正文给人看、禁止路径/目录当标题；**禁止证据层**（哈希/diff/snippet/「证据」——Facts/采集包仅作私有素材）；ready 采集包里利于理解的 Mermaid **必须**带入 Brief。**废弃 Host Digest 作为 Brief 的本机源。** 不导出 PPT。
- **仍禁止**：键鼠、截屏、剪贴板、浏览器历史、全仓灌模型、Daily 当源、密钥与 runtime 诊断、回顾 API 跑模型、静默 `replace_page`。
- **平台 Facts（2026-08-24）**：写汇报**不用笔记当原始数据**——`touched_notes` 即使请求也 skipped；合成器禁止 notes 读页/搜页。`agent_runs` 排除 `note_worker` 以及 `notes-assistant` / `period-collect-*` / `weekly-report`（写汇报自己的机械，不是本周工作）。Fact 行只写 Issue 标识+标题+结果或 `agent · reason · outcome`，禁止贴 `trigger_summary` 指令原文。回顾（`回顾/`）仍可用笔记源，合同不变。
- **重采（2026-08-24）**：采集失败后只由笔记助手再发起一次采集；inbox `max_attempts=1`，不自动重采。笔记助手必须等这次完成或失败后才把该采集员结果当作已收到，再写 Brief。未用尽这一次重采前不得说材料齐了，也不得 `completePeriodBriefRunAfterSynth`。
- **失败后两拍唤醒（2026-08-24）**：还欠一次重采时只派 **retry-only** 指令（禁止 `--note-write`）；重采 settle 后再派写稿。收成稿只认写稿唤醒之后的 `note_write`。采集包按采集员原子 `jsonb ||` 写入，等待按本次 `pack_job_id` 对齐。全失败/空计划不说「材料齐了」。卫星 `Write report` / `Report` 算写汇报意图。FAB 创建不带 `chat_session_id`。
- **插入卡不锁输入（2026-08-24）**：锁输入只在 `planning|collecting|synthesizing`。插入卡出现（`awaiting_confirm`）即解锁——人可以舍弃结果、继续对话或再开一轮，不必先点插入。同一页只有采集/合成未结束才 `409`。`periodBriefRunLocksComposer` / `TestCreateNotePeriodBriefAllowsNewRunAfterInsertCard`。
- **进行中只认这一页最新一轮（2026-08-24）**：`GetActiveNotePeriodBrief` 看该页 **最新一条** run。更新的一轮已经 `done` 时，不得再拿更早的 `awaiting_confirm` 当进行中。`TestGetActiveNotePeriodBriefIgnoresStaleAwaitingConfirmAfterNewerDone`。
- **进行中呼吸（2026-08-24）**：写汇报 `planning|collecting|synthesizing` 以及本页未完成问答时，右下角笔记圆钮和输入框停止钮用 `animate-chat-impulse`；插入卡后停。`TestNoteAssistantBubble` FAB `data-running` / ChatInput stop `animate-chat-impulse`。
