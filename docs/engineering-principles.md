...
**"以后照这个查"的方法必须先证明能重现你这次的发现**（Iris 案：实际用函数名 grep 找到四处，写进规矩的却是 `?? content`——照写下的执行=命中一个已修的、得出"全清"假结论）。**重现不了=你写的不是你做的。** 与"见过它红"互为两面：那条管装置会不会失败，这条管方法会不会漏。**复制=把我看到的拿走**：剪贴板必须与屏幕一致（`@小雅` 非 `@actor_14`），round-trip 归 mention picker 不归贴文本。
