...
- Exploration 候选只能从可寻址 canonical need signal 生成，Agent 自由文本不能直接发明 Task kind。required Question、Evidence Gap、blocking Dispute、Hypothesis change、stale Insight、同质来源和 Method Gap 使用版本化映射，输出固定 capability/purpose、分项评分、成本、硬约束和稳定 identity；相同 target/purpose/method/source/perspective 必须去重。`GenerateExplorationCandidates` 只生成候选，不选择、不持久化也不派发；Candidate Set、Portfolio Decision、Event 与 Task 事务未接线前，不得宣称生产系统已自主扩展工作。
