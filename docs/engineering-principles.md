...
- 线上 Research Strategy 的质量/成本窗口只有在达到最小样本且所有边界均通过时才能维持“安全”结论；任何充分样本窗口的越界都必须产生显式 rollback obligation，previous version 缺失或等于当前版本时必须冻结新流量并升级，不能静默继续。窗口不足只能继续采样，不能提前判定安全或触发回滚。当前确定性响应装置为 `researcheval.DecideProductionMonitorResponse`；持久 Decision、告警投递和流量控制接线完成前仍为未闭环。
