...
- Research canonical source URL 是公开资源身份，不是凭据载体。任何带 URL userinfo（用户名、密码、token 或其 percent-encoded 形式）的来源或 Retrieval 输入必须在规范化边界拒绝，不能持久化、参与去重或传给 Adapter；需要认证的 provider 只能通过独立、不可回显的凭据通道授权。
