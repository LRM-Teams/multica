...
- Research 的递归 Integration 不能把“当前没有队列项”当作收敛。决定必须绑定 Policy、Goal/Plan、Event 水位和 canonical state hash；未同化 Result、待建 Derivation、stale Insight 或变化中的 Frontier 必须继续，阻断争议和未回答必答问题必须等待其所属子系统。只有稳定 Frontier 的连续低边际增益窗口才能收敛；轮次、Insight 深度或成本上限遇到未完成工作只能升级，不能制造成功终态。当前可执行决定边界为 `server/internal/researchrun/integration_convergence.go`；接入持久 Round 和 Delivery Gate 前不得宣称 G 章完成。
