...
- Reporter Task 只能绑定当前 workspace/session、goal/plan 和 canonical state 下最新且 completed 的 Integration Snapshot，并消费该 Snapshot 精确 `input_set_hash` 对应的 accepted、verified、task-execution 可读且非 evaluation-private 工件版本。任一 scope、版本、水位、状态哈希、输入集合或访问事实变化都必须使准入失败并重新解析，禁止退回旧 Snapshot、遗漏输入或把隐藏 Evaluation 工件混入报告上下文。当前 `ValidateReporterInput` 是可执行准入合同；canonical 查询、dispatch manifest 持久化与 Report Revision 物化完成前不得宣称 K 章已接入生产。
