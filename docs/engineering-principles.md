...
- D-enabled task-bound ArtifactProjection 必须是 Manifest selection-time 投影，不能只限制 Artifact ID 后回查 live Passport 元数据。Entry 必须冻结 lifecycle、provenance、version count 和 input/output reference count，并由 Manifest hash 连同 selected version、access、eligibility 和 representation 一起绑定；task-bound read 与 Result acceptance 在解码/投影前共用完整 hash 重建，projection 只读 entry snapshot 与 immutable Version。旧 NULL、篡改或无法重建一律 fail closed。Manifest 身份由 `AttemptContext` 单独表达，不能把 Manifest 自身递归伪装成 entry。执行装置：migration `354_research_manifest_projection_snapshot`、`artifactProjectionModule.LoadManifest`、`verifyAttemptManifestHashPool` 和 withdrawal/selection-count 篡改回归。
