...
- D-enabled task-bound Research context 的“冻结”必须同时覆盖 Run、Contract、Method、Question、Task 和可见 Evidence，不能先读 live 核心值再只按 Manifest 过滤证据 ID。完整 representation 必须在 dispatch 所属事务的锁定行上编码；后续读取从 bytes 重建值，live 集合最多用于保持历史列表顺序，派发后新实体不得进入。无法重建的旧 hash-placeholder、重复唯一核心实体或已消失的冻结实体一律 fail closed，禁止静默回退 live。执行装置：`freezeEvidenceRepresentationsTx`（历史名，现覆盖核心上下文）、`loadFrozenCoreContextPool`、`applyFrozenCoreContext` 和派发后 live mutation 回归。
