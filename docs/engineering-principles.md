...
- Process Manager 与 Activity producer 没有 Daemon-side Workspace map 或 lazy singleton；start/stop/probe/reconnect 都通过同一 WorkspaceDaemon-owned instance。socket reconnect 只替换 Activity transport，保留本机 process identity 与 Activity sequence；Binding teardown 才释放该 WorkspaceDaemon 的 process/Activity/Inbox state。
