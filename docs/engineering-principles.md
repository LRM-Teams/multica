...
- **指针**：决策表 `docs/members-directory-decisions.md`；ADR `docs/adr/0013-members-directory-replaces-agents-page.md`。
...
- Agent Presence 只有 `online|offline`：服务端由当前 ready WorkspaceDaemon 连接与该实例持有的 active managed process 共同投影。Runtime heartbeat、Task workload、WorkspaceDaemon Activity、Health、provider quota、crash reason 都不能覆盖这个结果；加载或坏响应不造第三态，也不冒充 Online。
...
- **物**：migration 231 `daemon_update_status`、register/heartbeat session-revision CAS、`daemon-update-status.json` atomic snapshot、WorkspaceDaemon control change wake、runtime typed projection、old-daemon clear、duplicate-zero-write / stale-session / conflicting-revision / interrupted-replay / malformed-future-enum regressions。此阶段只增加观测，不改变 host default、claim barrier、supervisor 激活或 UI。
...
- Product/API 暴露 `startAgent(agentId)`、`stopAgent(agentId)` 与 `resetAgent(agentId, mode)`；Reset mode 固定为 `restart | session | full`，没有 public `action_kind`、`execution_mode`、幂等键或 `after_current_run`。显式 Start 投递期望的 `AgentID + RuntimeID`；manual Stop 只投递 Agent 的 Computer 命令，不写 durable Stop intent。Agent Panel 用 managed process Presence 在同一按钮切换 Start/Stop，不读 work/activity status。
...
- Stop 必须先撤销 APM admission、以本机 `AgentInstanceID` fence 迟到 startup，并请求终止旧 provider。若退出暂未确认，Daemon 保留 APM 现有 `stopping` 状态；同 Runtime Start 继续等待，不同 Runtime Start 也必须等 startup owner、provider lease 与 resident process 全部 quiescent 后才进入普通 Start，绝不能启动第二个 provider。Start 在当前 socket 上发送期望的 `AgentID + RuntimeID`。Full 必须在 stop 证据后 reset AgentRoot，并且 Server 收到同场 restart 的 terminal reset result 后才允许 start。
- 当前 WorkspaceDaemon socket 拥有整场 stop → start。WorkspaceDaemon Ready 不按 step 恢复，desired/observed reconcile 跳过任何仍在内存中的 restart。relay publish 不等于 accepted/completed；中断的 restart 留在进程内存里，直到当前 socket 走完或人再点一次。Activity 只消费正常 `Stopped → Starting → Idle/Working` 事实；Agent Restart 不新增 toast stream。
- **物**：离散 `AgentStopPayload` / `AgentWorkspaceResetPayload+Result` / `AgentStartPayload.RuntimeID+Config.SessionID`、内存 `agentRestartStore`、三模式成功链、本机 instance fence、reset completion proof；旧 composite payload、heartbeat pending queue、delivery lease、持久幂等账本和本地 `.multica/lifecycle-commands` ledger 不得恢复。
...
- Agent 本机 admission 只认 current WorkspaceDaemon 接受的 `agent:start`；`agent:stop` 清除该 Agent 的本机 admission。不存在独立 Attachment registry、attach/detach wire、generation、replay cursor 或本地兼容 ledger。Message coordinator 可从同一 WorkspaceDaemon 的 APM start snapshot 修复，不能读取第二套 ownership state。
...
- restart-time Message Delivery 只能从固定 WorkspaceDaemon scope 调用 `Resolve(workspaceID, agentID)` 重建 Inbox coordinator；Runtime 与 AgentRoot 必须来自该 Attachment 且 Runtime 仍属于同一 Workspace。detached 或 wrong-Workspace Agent 在 ACK 前拒绝，已有 coordinator 也必须通过 WorkspaceDaemon→Runtime Workspace proof。
...
### 4.19.2 WorkspaceDaemon identity 不随 Runtime 变化 — `可执行`（② narrow constructor + ③ WorkspaceDaemon-owned Message/Activity/Attachment behavior + ⑤ executable ownership guards）
- `WorkspaceDaemonConfig` 固定 `DaemonID + DaemonInstanceID + WorkspaceID`；Runtime set 是可变输入，不进入 WorkspaceDaemon identity。缺任一 identity 或 machine-wide dependency 时 constructor fail closed。
- WorkspaceDaemon 本地构造并持有 Process Manager、Activity producer 与 `InboxRegistry`；durable Attachment、Daemon-owned Agent runtime pool、diagnostics 只注入同一 Daemon owner 的引用，不复制状态、不另建 singleton。Credential Proxy 只解析唯一 WorkspaceDaemon 并调用其 Message 方法，不取得 coordinator 或 registry。
- Process Manager 与 Activity producer 没有 Daemon-side Workspace map 或 lazy singleton；start/stop/reconnect 都通过同一 WorkspaceDaemon-owned instance。Activity 是 WorkspaceDaemon 形成的 best-effort 展示观测：daemon 直接发送 activity kind、summary 与 display-ready timeline rows，Server 只授权、持久化、读取和广播，不维护 cursor、重放排序或 probe 可靠性，也不据此改变 Agent lifecycle。socket reconnect 只替换 Activity transport并保留本机 process identity；Binding teardown 才释放该 WorkspaceDaemon 的 process/Activity/Inbox state。
- `WorkspaceDaemon.Run(ctx)` 已唯一持有 Workspace auth、dial/reconnect backoff、ready identity、connection context/cancel、ping/pong 与 serialized frame writer；replacement 先 cancel/close 旧 connection。socket callback 是 private implementation，不引入额外 transport abstraction。
- 每个 live connection 自己持有 per-Agent Delivery dispatcher 与 current-socket Message callback：同 Agent FIFO、不同 Agent 可并行；callback 只在 exact connection 仍 current 时可写，replacement 后 stale callback fail closed。Daemon 只按 Workspace 找 current WorkspaceDaemon，不再保存 transport/generation parallel maps。
- `InboxRegistry` 固定一个 Workspace scope；仅在该 scope 内 shared Attachment owner `Resolve` 成功且 Runtime ownership 匹配时创建 coordinator。Delivery、coverage 与 reconnect 都经过 current WorkspaceDaemon 方法；网络 reconnect 保留 registry 和进程内 Context Boundary，WorkspaceDaemon close 只关闭自己的 Inboxes。旧 machine-local 仅 Agent lookup 必须唯一，否则 fail closed，不能隐式选另一个 Workspace。
- **物**：`workspace_daemon_state.go` 的 narrow constructor/connection ownership 与 current-connection fence；`workspace_daemon_message.go`、`workspace_daemon_activity.go` 的行为归属；`workspace_daemon_delivery.go` 只返回 current WorkspaceDaemon、不暴露 coordinator；`workspace_daemon_state_test.go` 的 whole-Daemon dependency 与 field reach-through source guard，以及 identity/reconnect/isolation/recovery 回归。
...
- **物**：`computer.Host` / `Host.RunProcess` / `hostMachineUpgrade` / `BindingSupervisor` / `HostControl` / `BindingRunner`；真实 bootstrap/Ready/control process protocol；per-Binding child state root；双向 architecture guard；process-identity/crash/sibling/Machine Upgrade tests。
...
- 声明 `workspace_daemon_control_plane_v1` 的 current WorkspaceDaemon 是该 Computer-Workspace Binding 内 Runtime heartbeat 与 Server pending action 的唯一 carrier。WorkspaceDaemon 每次按当前 Workspace Runtime set 发送，Server 每次重新验证 Workspace、Computer 与 generation；ack 只在同一 WorkspaceDaemon socket、同一 Workspace Runtime scope 内消费。
...
- **物**：`DaemonCapabilityWorkspaceDaemonControlPlane`；`WorkspaceDaemon.runControlPlaneHeartbeats`；`HandleDaemonWSHeartbeat` 的 Workspace/Computer/generation fence；`TestWorkspaceDaemonOwnsCurrentControlPlaneHeartbeat`、`TestWorkspaceDaemonControlHeartbeatUsesExactWorkspaceRuntimeSet`、`TestLegacyRuntimeWakeSocketDoesNotExecuteControlAcknowledgements`、`TestLivenessProbeAcknowledgesWithoutInvokingHeartbeatActions`、`TestWorkspaceDaemonHeartbeatRejectsRuntimeAssignedToAnotherComputer`。
...
- **指针**：ADR `docs/adr/0019-runtime-agent-collectors-period-brief.md`（取代 0018 Host Digest 路径）；skill `multica-period-work-collect` / `multica-period-work-plan` / `multica-period-work-brief`。
