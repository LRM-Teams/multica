...
- Retrieval Adapter 只负责访问外部 provider；Research Run 拥有统一 Search/Fetch contract、规范 URL/identity/content hash、成本与失败分类，以及 redirect/DNS/credential/scan 安全事实的 fail-closed 校验。Adapter 输出未经 `ValidateRetrievalSearchPage` 或 `ValidateRetrievalDocument` 验收不得进入 Corpus；私网地址、凭据转发、未知失败类和内容 hash 不一致一律拒绝。
