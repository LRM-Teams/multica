...
- D-enabled task-bound Attempt 列表不得从 live runtime 状态透传。当前派发事务创建 Attempt passport/version 后，必须把当前与历史可见 Attempt 的执行目标、Inbox binding、状态、runtime/lease、取消、失败和 Result lifecycle 冻结进同一 Manifest；后续读取从 representation 重建，live 集合最多用于稳定排序。当前 Attempt 不在 Manifest、重复或已无 domain 排序源时 fail closed；旧 Manifest 不得因缺少 Attempt entry 回退 live。执行装置：Attempt candidate admission、`freezeEvidenceRepresentationsTx` 的 Attempt 分支、`loadFrozenAttemptRepresentationsPool`、`applyFrozenAttempts` 和真实 `AttachInboxTask` 漂移回归。
