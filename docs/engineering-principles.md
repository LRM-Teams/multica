...
- 后续 Research Task 读取跨 Agent Integration Context 时，只能选择当前 workspace/session、Goal/Plan 和 Manifest Event/State 水位内的最新完成 Snapshot 精确 Artifact Version。候选必须有至少两个不同贡献 Agent、Passport、content/input/canonical-state hash 和合法访问级别；最新版本 stale、superseded、无权或 evaluation-private 时必须 omission，禁止降级到旧结论，同一最新位置多身份必须失败。当前可执行选择器为 `server/internal/researchrun/integration_context.go`；持久查询与 `TaskContextForAttempt` 接线前不得宣称跨 Agent Context 已生产化。
