...
- Corpus 去重只合并可确定相同的 canonical URL 或 content hash，并保存稳定 cluster、canonical candidate、rule 和 reason。相同 content 出现在不同 URL 时必须按 mirror 处理并共享 effective independence family；同一 URL 出现不同 content hash 时全部进入 review，不能静默选一个版本。相同 independence family 的不同内容仍可保留，但后续 Evidence Gate 不得把它们计作独立来源。
