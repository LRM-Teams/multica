...
- 显式评测 Run 的冻结 Contract 必须在 `source_policy.evaluation_subject` 绑定 subject schema/hash 与 Document allowlist；Agent Result 的 Source metadata 和 Observation datum 只能回显该身份与显式 fact key/value，不能携带 Oracle 或自增字段。Handler 预检不能替代 `AcceptResult` 在锁住 Run 后读取当前 Contract 并复检，steering 不能在 decode 与 materialize 之间更换评测 Subject。没有该字段的普通 Run 保持原行为。
