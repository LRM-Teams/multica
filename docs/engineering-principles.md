...
- Monitoring Cycle 的变化判断必须复用固定 Search Plan 的完整 canonical query key + hash 集合，并保存每个 Query Execution 与前后工件版本/内容哈希的对应关系。added、removed、modified、unchanged 必须与当前结果集合一致；物质性由服务端解析的版本差异计算，不能接受 Agent 自报 aggregate delta，也不能因 `no_results` 伪造差异。当前 `ValidateMonitoringDiff` 是可执行准入/计算合同；持久化和调度接线完成前不得宣称持续监测已上线。
