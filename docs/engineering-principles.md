...
- D-enabled Attempt 的冻结 Context 读取必须同时验证完整 Manifest Hash，以及每个被解码 representation 的模式、非空原始字节和精确字节哈希；只在派发或 Result 接纳时验证不能证明稍后的 Agent 读取仍是派发时的字节。任一层不匹配都必须拒绝整个 task-bound snapshot，禁止降级回 live Run 数据或跳过损坏条目。
