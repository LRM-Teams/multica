...
### 4.19.2 Workspace Runner identity 不随 Runtime 变化 — `可执行`（② constructor shape + ③ Runner-owned socket/Inbox + ⑤ identity/reconnect/isolation tests；Process/Activity cutover 进行中）
...
- Runner 本地构造并持有 Process Manager、Activity producer 与 `InboxRegistry`；Agent Attachment、canonical Runtime pool、Credential Proxy、diagnostics 只注入同一 machine-wide owner 的引用，不复制状态、不另建 singleton。
