...
- **Computer restart 与升级恢复共享单 owner 栅栏**：`computer restart` 只有在旧 resident 的 local control endpoint 已证明释放后才能启动 successor；仅成功发送 shutdown 不构成 stop proof。Machine Upgrade 的单一本机 handoff journal 记录 `sourceServicePid`、旧 runner PIDs、accepted managed Workspace set/revision，并以 successor 的 exact version/PID/source PID、non-empty `serviceGeneration` 和完整 managed-set attestation 收敛；首次有效证明锁定 `observedTargetGeneration`，target identity 变化必须重新观察，且 source service 和所有旧 runner 死亡前不得清理 journal。不存在 cloud generation CAS、receipt 或双写状态机。完整合同见 [`ADR-0016`](adr/0016-local-upgrade-proof-no-cloud-cas.md)。
...
- **物**：`computer.Host` / `Host.RunProcess` / `hostMachineUpgrade` / `BindingSupervisor` / `HostControl` / `ProcessCapacity` / `BindingRunner`；真实 bootstrap/Ready/control process protocol；per-Binding child state root；双向 architecture guard；process-identity/crash/sibling/capacity/Machine Upgrade tests。完整判断见 [`ADR-0015`](adr/0015-computer-host-supervises-binding-runner.md)。

### 4.19.5 Machine Upgrade 以本机 successor attestation 收敛 — `可执行`（①无 cloud CAS + ⑤ local-proof tests）
- **口径（2026-08-18，Raft 1.0.17）**：successor 必须通过 framed IPC 返回 exact target version、target PID、`sourceServicePid`、non-empty `serviceGeneration` 和完整 managed Workspace set/revision。首次有效证明锁定 `observedTargetGeneration`；target 变化时重新观察。旧 runner 和 source service 全部死亡前不得删除 journal。
- **物**：`machine-attestation` local RPC；包含 `sourceServicePid` / `oldRunnerPids` / `acceptedManagedWorkspaceIds` / `acceptedManagedSetRevision` / `observedTargetGeneration` / `targetServicePid` 的单一 handoff journal；无 loopback HTTP adapter、numeric generation 或 dual-write。判断见 [`ADR-0016`](adr/0016-local-upgrade-proof-no-cloud-cas.md)。
