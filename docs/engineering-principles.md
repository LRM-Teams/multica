...
- **物**：migration `274_research_run_backend` + `275_research_report_quality`；`server/internal/researchrun`；`research_run_adapter.go` / `research_run_http.go` / `research_run_guard.go`；scheduler `research_run_reconcile`；CLI `multica research task-result`；builtin skill `multica-research-fleet`；前端 typed Run snapshot 和 steer API。
