...
- Dispute 交付义务必须针对同一个不可变 Report Revision 聚合验收。blocking `open | investigating` 争议和未完成的人类 Gate 阻断交付；`conditionally_resolved | irreducible` 不因状态终结自动通过，报告必须在当前修订保存可寻址披露，并逐字段匹配 canonical condition、residual uncertainty 和 impact。`EvaluateDisputeDeliveryGate` 只产生稳定 finding 与指纹；持久读取和 deliveryGateModule 未接线前，不得宣称生产交付已执行这套争议门禁。
