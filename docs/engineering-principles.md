...
- Capability gap 不能直接等价为“新建 Agent”。服务端必须先拒绝 stale gap，再依次考虑激活满足全部约束的已有 Agent、对现有 Agent 做最少且已授权的配置变更、提出仍需 Director/Contract 授权的 Team Formation；合同、人数、预算、工具或来源权限不允许时显式 `blocked`。`DecideCapabilityGapAction` 只决定下一动作，不执行 Agent mutation 或绕过 Team Formation authorization；这些接线未完成前不得宣称生产系统会因 gap 自动招聘。
