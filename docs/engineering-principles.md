...
- 生产评测证据必须进入 tenant-scoped 的不可变账本：Evaluation Run 固定 Corpus/Strategy/baseline、去重后的 seed 集合和规范化 Environment hash；成功 Trial 同时封存 Artifact 正文与 hash，并逐 grader 保存分数、指标和 findings，执行失败作为无 Artifact/Grade 的零分 Trial 留在分母；最终 Report/Comparison 与 Run 在同一事务中封存，重复完成只允许同 hash 精确重放。当前可执行装置为迁移 `360_research_evaluation_ledger` 与 `server/internal/researcheval.PostgresLedger`；生产 Adapter 尚未接入时不得把账本存在本身当成策略已通过评测。
