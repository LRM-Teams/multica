...
4. Standalone chat delivery prefixes `<note_chat_context>` with **this bubble `chat_session_id`, root id, and title** (no full subtree dump). After the same `chat_session` finishes 写汇报 (`awaiting_confirm` / `done`), the prefix also appends `<period_brief_residue>` with the finished brief in `<period_brief>` (window, insert mode, live `result_page_id` or draft). A deleted insert still carries the brief snapshot. A finished session also gets `<period_brief_session_materials>` (computer names / id / os / hostname / in_latest_run — not pack bodies). That does **not** replay collect/synthesis, merge Worker sessions, or dump collector packs / the rest of the bubble transcript. `notes get` may read that run's draft or result page even when the draft sits outside the context subtree (exact page only). Idle wake uses `formatStandaloneChatTurnPrompt` (chat turn — **not** channel Canonical Message / `message send`). The agent must use `notes tree` / `notes get` selectively (skill `multica-notes-assistant`) and answer via **final assistant output** (daemon writeback). Redelivery rebuilds the same prefix. Turn failures still write an assistant error row so the UI leaves 排队中.
5. Distinct from Editor (`note_ai_job`) and Worker (`note_worker_job`); replies stay in the bubble transcript. Bubble has no channel `note_write` confirm UI — rewrite proposals stay in final output for the human to apply. Selection sparkle on the Notes editor opens this sidebar immediately and quotes the excerpt in the composer (`askAboutNoteSelection` / `noteSelectionQuote`); a later selection on the same page appends another excerpt. Dragging a whole page from the Notes tree into the composer adds a page reference chip (`addNotePageRef` / `notePageRefs`); send prepends `/notes/<id>` lines so the assistant can `notes get` that page. It does not create an Editor AI job. Empty-line Space / `openPageAI` always opens the in-note prompt. Send uses `note_ai_job` only when 笔记助手 already exists. If it is missing, Enter / send opens the same first-open setup card (`setNoteBubbleOpenPageId` / `requestInlineNotePageAI`) and does not start an Editor job.
6. 写汇报 **product contract:** ambiguous speech matching 写汇报 soft-confirms first (`awaiting_intent`); 「是」 opens the plan card (`clarifying`). Exact 「写汇报」 (in-window quick action / short typed ask) opens the plan card directly. The human chooses window, computers, 取消, or 开始采集. 开始采集 (`POST /api/notes/period-briefs`) walks every selected computer then synthesizes — see `docs/notes-period-brief-assistant-contract.md` (2026-09-08) and ADR 0019. 开始采集 synthesizes from this run only; plain 写汇报 does not reuse prior harvests; explicit prior-harvest ask may. Partial ready still writes; 15-minute settle ceiling. The assistant reads packs / brief / current note; it does not PUT collect scope or call start. Do not extend chat-XML fence intercepts. The 写汇报 button is a direct funnel into that same intercept. Plan = window + owned computers; typed `focus` is optional (empty → per-computer collect-roots / `SCAN_ROOTS`). No global 「全部」 chip. Missing collectors do **not** auto-create: the plan card may show a dismissible reminder; configure opens Create Agent with that Computer locked → `POST /api/members/agents/period-brief-collectors`. Composer locks only while a collector or synthesizer is **actually running** (`planning|collecting|synthesizing`); stop cancels that run. Plan-card 取消 (`DELETE /api/notes/period-briefs/plan`) dismisses the plan and stops in-flight collect/synthesize. The result card (`awaiting_confirm`) unlocks. `409` only for a live in-flight run. Collectors / planner / synthesizer stay Worker `force_fresh_session`. Progress wakes the same bubble with `<period_brief_progress>` (no canned 「刚刚收到了…」). The result card snapshots **this run's** synthesizer write. Insert: card actions or `notes period-brief insert` (`target_page_id`); confirm when the target is not the issuing page. Leftover 「周报」 agents are archived on Ensure. Collectors stay separate.
