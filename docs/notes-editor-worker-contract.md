...
4. Standalone chat delivery prefixes `<note_chat_context>` with **root id + title only** (no full subtree dump). After the same `chat_session` finishes 写汇报 (`awaiting_confirm` / `done`), the prefix also appends `<period_brief_residue>` with the finished brief in `<period_brief>` (window, insert mode, live `result_page_id` or draft). A deleted insert still carries the brief snapshot. That does **not** replay collect/synthesis, merge Worker sessions, or dump collector packs / the rest of the bubble transcript. `notes get` may read that run's draft or result page even when the draft sits outside the context subtree (exact page only). Idle wake uses `formatStandaloneChatTurnPrompt` (chat turn — **not** channel Canonical Message / `message send`). The agent must use `notes tree` / `notes get` selectively (skill `multica-notes-assistant`) and answer via **final assistant output** (daemon writeback). Redelivery rebuilds the same prefix. Turn failures still write an assistant error row so the UI leaves 排队中.
...
6. 写汇报 starts from the Notes FAB satellite (chips immediately) **or** a spoken ask in the bubble. A send matching `looksLikePeriodBriefRequest` (same intent as Go) holds the message and shows a local conversation turn (user bubble + assistant reply 「看起来像让我整理汇报，要走整理汇报的流程吗？」 with 否 / 是). 是 removes that turn, opens the same time + owned-computer chips, and keeps the draft; 否 removes that turn and sends the original text as ordinary notes-assistant chat (that send is not confirmed again). Chat regex no longer starts a new intake — new runs go through `POST /api/notes/period-briefs`. After chips are open, typed text wins on conflict. Missing, deleted, or wrongly-bound collectors do **not** auto-create: the chip list shows a dismissible reminder per broken owned Computer. Configuring is optional — 写汇报 continues with the remaining collectors. Configure opens the Create Agent dialog with that Computer locked (runtime + model only) → `POST /api/members/agents/period-brief-collectors` with `runtime_id`+`model`. Probe without `runtime_id` creates nothing. Once chips are open, the next composer send starts the run. The chip card has **取消** to abort before send; after send the button is gone/disabled (`periodBriefSubmitting` / chips unmount). Chips always sit above the input; an empty thread keeps a flex spacer so opening chips cannot lift the composer. After send, chips disappear, the composer locks only while the run is `planning|collecting|synthesizing`. The composer stop button cancels that run (`POST /api/notes/period-briefs/{runId}/cancel` → `cancelled`), suppresses collector / planner / synthesizer Worker inbox wakes, and unlocks the composer. The insert card (`awaiting_confirm`) **unlocks** the composer — discarding without insert is allowed, and a new 写汇报 may start. The Notes FAB **omits** `chat_session_id` so each 写汇报 opens a new `chat_session`. Passing an id still reuses that thread (spoken chip intake). A second create on the same page is `409` only while collect/synthesis is still running. Collectors / planner / synthesizer still run as Worker `force_fresh_session` wakes. Incoming collector packs and the finished brief render as collapsed `note_brief` cards in this sidebar (packs have no Notes page link). The brief card snapshots **this run's** synthesizer `--note-write` (`created_at >= run.created_at`) or this run's draft — never the latest write on `工作介绍/` from a previous run. The brief card has **插入笔记下面** (append a `##` section onto the issuing page) and **插入子笔记** (create a child of that page) — not under global `工作介绍/`. Leftover 「周报」 agents are archived on Ensure. Collectors stay separate.
