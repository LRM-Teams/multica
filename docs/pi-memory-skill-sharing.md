...
The daemon runs local memory curation for workspaces that have an online Pi runtime. L1/L2/L3/L4 run at 01:00/02:00/03:00/04:00 Asia/Shanghai on the daemon host, so L3 uses that daemon account's configured Pi executable, authentication, and default model. `MEMORY_CURATION_L3_REVIEW_ENABLED` defaults to `true` and can disable only the reviewer; review failures are fail-closed and leave candidates in `REVIEW.md`. The central server scheduler remains useful only for deployments that deliberately mount the same workspaces root and configure Pi on the server host.
