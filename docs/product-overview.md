...
Claude Code · Codex · OpenClaw · OpenCode · Gemini · Pi · Cursor Agent · Kimi · Kiro CLI · Grok
...
- **基本信息**：名字、描述、头像（自动生成）
- **Provider**：选择底层是 Claude / Codex / OpenClaw / OpenCode / Gemini / Pi / Cursor / Kimi / Kiro / Grok 中的哪一个
...
1. **自动探测** `$PATH` 上安装的 coding CLI（`claude`, `codex`, `opencode`, `openclaw`, `gemini`, `pi`, `cursor-agent`, `kimi`, `kiro-cli`, `grok`）
