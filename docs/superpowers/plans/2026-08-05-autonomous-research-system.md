...
- [x] M1a 实现 `researcheval.LifecycleExecutor` 的生产 Run 生命周期边界：Launcher 只接收 `SubjectInput` 与 seed，返回固定 session/workspace 身份；Reader 必须持续读取同一 Run，`running` 和 `awaiting_user_confirm` 均不能提前抽取结果；只有 `completed`、`failed`、`cancelled`、`archived` 明确终态才调用 Artifact Extractor。失败 Run 仍作为可观察评测工件进入 grader，而不是被 Runner 当作基础设施错误跳过；身份漂移、取消和超时均 fail closed。时钟与轮询等待可注入，单测覆盖等待确认、失败工件、身份漂移、取消和确定性超时。该切片建立生产 Adapter 的组合边界，尚未把真实 `ResearchRun` Launcher/Reader、canonical Artifact Extractor 或隐藏 Oracle 持久化接入，因此 M 章仍未完成。
