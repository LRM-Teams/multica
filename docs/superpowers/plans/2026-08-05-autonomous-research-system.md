...
- [ ] evaluation-private 与被评对象隔离。
  - [x] D-shadow-membership-1：Manifest 候选加载改为遍历 scope 内全部 current Passport version，再按完整 Artifact registry 显式排除 Attempt/Manifest/Result 自身；未知未来 kind fail closed。构建时审计每个候选 version 恰好进入 entry 或带精确 policy reason 的 omission，拒绝缺失、重复、双重、错误分桶和错误 reason；25 个注册族的分区与全族 disposition matrix 已冻结。§15.8 仍待 evidence tie-order 与 omission mutation 收口。
