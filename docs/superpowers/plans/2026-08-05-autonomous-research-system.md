...
- [ ] evaluation-private 与被评对象隔离。
  ...
  - [x] D-context-2：Manifest 不再只为 Source/Observation/Claim 保存完整 representation；Run、Contract、Method、Question、Task 同样在 dispatch 事务锁定的确切行上编码完整 JSON。`TaskContextForAttempt` 用这些 bytes 替换 live 核心值，沿用当前列表只作为稳定排序骨架并排除派发后新增实体；冻结实体已从 domain 表消失或重复/缺失唯一核心表示时 fail closed。单测证明 live 值不能覆盖 frozen 值且 Question/Task 顺序稳定；PostgreSQL 回归在派发后改写 live Run/Question/Task 并证明 human read 前进、Attempt read 保持不变。旧 D Manifest 的 hash-placeholder 无法重建原值时不回退 live，而是拒绝该 task-bound read。Attempt runtime 投影本身仍未加入 Manifest candidates，D §15 其余 partial matrix 仍未完成。
