...
- [ ] evaluation-private 与被评对象隔离。
  - [x] D-backfill-1：migration 355 将 325/328 已定义的逐 owner 诊断扫描器接成 session 级、全历史自动 backfill，覆盖 Research Message、Decision 和 Run Event；重复扫描按 owner/field 幂等，已修复或移除的引用会清除旧诊断，回滚保留仍有效的派生诊断记录。PostgreSQL migration 回归固定 malformed/bounded、unresolved、cross-scope、重复执行与修复后清理，§15.6 收口。
