...
- [ ] evaluation-private 与被评对象隔离。
  - [x] D-legacy-1：`legacy-v1-v5-compat-v1` 准入矩阵穷举全部已注册/未知 Artifact kind、全部合法/终止/未知 lifecycle 和全部完整/部分/未知/非法 provenance。只有 registered/accepted 且 provenance 枚举合法的已注册 kind 可进入后续 clearance/compartment 筛选；未知 kind、终止 lifecycle 和未来 provenance 分别 fail closed，§15.7 收口。
  ...
  - [x] D-context-1：D-enabled outbox 的 frozen Manifest ID/Hash 进入不可变 `DispatchRequest` 指纹，并由真实 Agent Inbox Adapter 写入 `research_manifest_id` / `research_manifest_hash` context。Manifest 任一字段变化都会改变 request hash，缺一项 fail closed；outbox claim 每次还要求 payload 身份与独立 manifest 列完全一致，列被篡改时事务回滚且不派发。历史无 Manifest 请求继续省略两字段且固定 V1 hash 不变。PostgreSQL outbox 测试锁定 payload/列/重算 hash 三者一致并注入列篡改，Handler 测试锁定 Inbox JSON 数字、数组和 Manifest 身份不因序列化丢型。该项关闭“数据库有 Manifest、真实 Agent delivery 无 Manifest 身份”的链路缺口；D §15 其余 partial matrix 仍未完成。
...
- [ ] 实现触发策略、固定输入版本、幂等执行和状态变化应用。
  ...
  - [x] G2b-result-core：V6 `integrate/research_integration_v6` 已接入普通入口与 PostgreSQL `AcceptResult` 的锁定重解码/哈希边界。接收事务要求 Result 中所有 Contribution 指向同一个、属于当前 Task 且仍 active 的真实 Integration Round；每个 compared Artifact 必须解析到本 Run 可接纳的 current Passport version。事务写 Integration Contribution 与 production Passport/typed input references，按输入 Claim/Insight 当前版本计算 Insight level 并写 Insight Derivation DAG，应用 Inquiry status update，最后接受 Round 和写幂等 Event。Integration Round 的触发预留/派发、Dispute 与 follow-up Task materialization 仍待 G2c/G3，不能以本项宣称完整 Integration 已运行。
  - [x] G2c-round-dispatch：每次 V6 Result 接纳在同一事务完成 Task 终态与依赖激活后，扫描 planner-authored 且已 ready 的 `integrate/research_integration_v6` Task；仅当全部依赖已有 current accepted Result Artifact 且至少来自两个 Agent 时，按排序后的 Artifact/Version/hash/Agent 与当前 Event 水位生成确定性 input hash 和 Round UUID，创建 Integration Round、production Passport、逐版本 typed input reference 与审计 Event，并把 round ID、水位、hash、精确 Artifact/Version ID 合并进 Task 的冻结 acceptance criteria。V6 专用 Prompt builder 现已支持派发所有 V6 expected-result，并对 Integration Agent 明示不可更改的 Round/输入集合与严格 task_result 合同。动态阈值触发新的非计划 Integration Task、Dispute 与 follow-up Task materialization 仍待后续切片。
  - [x] G3-dispute-follow-up：Integration Result 的 Dispute 与 follow-up work 不再触发 `unsupported`。Dispute subject 只允许同 Run Question/Hypothesis/Claim/Insight，服务端按 materiality 投影 advisory/blocking 严重度，创建 deterministic Dispute/Position、production Passport 和 typed subject/position lineage；migration 385 为冻结 V6 的开放 position object 增加 lossless `position_payload`，避免用臆测字段丢弃 Agent 立场。Contribution 内 follow-up Question 复用 canonical Question 状态机和 Passport；proposed Task 复用预算、DAG、能力、超时、Passport 与关系校验，强制依赖当前 Integration Task，并把所有 Inquiry target 写入 canonical target ledger 与 typed references。Deliberation 的多轮执行、动态阈值触发非计划 Integration Task 仍待 H 阶段。
...
- [ ] 实现盲复核、Methodologist、区分任务和 Adjudicator 输入隔离。
  - [x] H3a-isolated-investigation：Integration 事务在 Dispute/Position 落库后调用服务端 `PlanDisputeReview`，为每个 Position 自动创建独立 verify Task，method 冲突增加 Methodologist Task，并创建区分证据 counter-search；所有 Task 固定 Dispute target、原 Integration dependency、V6 Search Plan、Position/Claim/Source typed inputs、作者排除集合和 production Passport，且受 Run 总 Task budget 约束。调度器对普通选择和 sticky assignment 都执行 excluded Agent fencing，无独立 Agent 时显式保留 capability gap，不能回退给原作者。migration 389 与 runtime policy selection 把 V6 从错误的 legacy admission 切换到 `research-v6-context-v1`，只接纳 complete provenance 的当前 V6 工件，并把 policy version 绑定 Manifest/grant/hash/acceptance；Artifact Context 在初次规划、授权重算和 shadow equivalence 三处共同执行 closed allowlist，Review Attempt 只看 Run/Contract/Method foundation、当前 Task 及显式 Dispute/Position/Claim/Source，其他可读工件以 `irrelevant` omission 冻结，避免读取对方 Position 或私有评分。自动 Deliberation dependency 汇合及其 accepted-review-only 上下文仍待 H3b。
- [ ] 实现 Research Deliberation Turn、进展水位、轮次/成本限制、deadlock 和 Research Director 自动升级。
  ...
  - [x] H4b-director-escalation：migration 387 为每个新 Run 在初始化事务内把当时 Fleet Lead 固定为 versioned Research Director Identity，登记 production Passport；后续 Fleet Lead 变化不能静默改写 Run Director。Deliberation 只读取并锁定最新 pinned identity；deadlock 时验证该 Agent 仍是 active lead，以确定性 Task ID 创建唯一 `director_adjudication` deliberate Task，固定 assigned Agent、identity version、Dispute/Deliberation/deadlock reason，注册 Task Passport、canonical Dispute target 和 typed lineage。该 Task 不能被普通路由改派；其证据约束裁决 Result 与用户显式 Director reassignment 命令仍待 H4c/J4b。
  - [x] H4c-evidence-bound-adjudication：冻结 V6 `task_result` envelope 内以严格单一 adjudication payload 承载 Director 决策，不扩写已冻结 Schema；解码器要求固定 Director identity version、明确 decision/rationale/conditions/residual uncertainty，并要求对每个 Position 给出唯一 assessment、disposition、rationale 和非空 Claim/Source Snapshot evidence。deadlock 自动 Task 直接冻结 Director Identity 与全部 canonical Position 输入；验收事务锁定 Task target、Dispute subject/status、deadlocked Deliberation 和最新 pinned identity，证明每个 Position 恰好覆盖一次且每项 Evidence 的当前 Artifact Version 存在于该 Attempt sealed Manifest，随后才写 Adjudication Decision、production Passport、Director/Dispute/Deliberation/Position/Evidence typed lineage、Event 并转换 Dispute。Director 仅凭权限、使用 Manifest 外证据、漏评/重复评 Position、身份漂移或状态漂移均 fail closed；用户显式 Director reassignment 命令仍待 J4b。
