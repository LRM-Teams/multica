...
- [ ] Planner 输出从“问题列表”升级为 Contract-bound Inquiry 初始图。
  - [x] E3b-target：migration 352 与 `bindTaskInquiryTargetsTx` 建立 ordered、去重、tenant/session scoped 的 Task→typed Inquiry target 持久关系；每个 Task 必须绑定 1–32 个真实端点，数据库复用 Inquiry 多态 resolver 阻止悬空/跨 Run 引用，H 章前 Dispute 继续 fail closed。accepted V6 Plan Result 的统一 ID 解析与同事务调用仍待 E3b-accept。
