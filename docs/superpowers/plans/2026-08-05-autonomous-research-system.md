...
- [ ] evaluation-private 与被评对象隔离。
  - [x] D-shadow-omission-1：migration 359 为历史与新 Manifest 回填/持久化 canonical omission hash，按 ordinal 绑定每个 candidate version 与精确 policy reason；Result acceptance 在提交前从持久 omission 行重算，删除、换序、换候选或改 reason 均 fail closed。§15.8 的 omission mutation 已收口，仍待 full-family shadow 与 evidence tie-order。
