...
- [ ] evaluation-private 与被评对象隔离。
  ...
  - [x] D-context-3：移除 Manifest candidate 对 `attempt` 的临时排除。当前派发事务先创建 Attempt passport/version，再把当前与历史可见 Attempt 的 target、Inbox binding、状态、runtime/lease、取消、失败和 Result lifecycle 字段编码为完整有界 representation。`TaskContextForAttempt` 从这些 bytes 重建 Attempt 列表，live 列表只提供稳定顺序并排除派发后新 Attempt；当前 Attempt 缺失、重复或 frozen/domain 排序源不闭合时 fail closed。单测固定替换、顺序、当前 Attempt 必需和缺失源拒绝；PostgreSQL 回归在 dispatch 后真实 `AttachInboxTask`，证明 human snapshot 看到 Inbox ID 而旧 task-bound snapshot 仍保持派发时空绑定。旧 D Manifest 没有当前 Attempt entry 时不再泄漏 live runtime 状态，而是要求新授权 Attempt。D §15 其余 partial matrix 仍未完成。
