...
- [ ] evaluation-private 与被评对象隔离。
  - [x] D-revoke-1：migration 357 将 policy grant、Context Manifest、entry、omission 和 input reference 设为 Session 存续期间不可直接删除；撤销 normal/evaluation grant 只推进 grant revision/status 与 mutation ledger，不能抹除冻结授权历史。Session/Workspace 整体删除继续允许聚合级 cascade。回归同时证明撤销后 task-bound read 与 Result acceptance fail closed、Manifest/entry/representation/grant/revocation ledger 保持可审计，§15.24 收口。
