...
- [x] M1 建立 tenant-scoped、不可变的 Evaluation Run/Trial/Grade/Report 账本：Run 固定 Corpus/Strategy/baseline、seed 和 Environment hash；成功 Trial 封存 Artifact 正文与 hash，失败 Trial 作为零分样本留在分母；Report/Comparison 同事务完成并按组合 hash 精确重放。装置为迁移 `360_research_evaluation_ledger` 与 `internal/researcheval.PostgresLedger`；生产 Run Adapter、grader 版本身份、Promotion Decision 和在线越界监测仍未完成。
