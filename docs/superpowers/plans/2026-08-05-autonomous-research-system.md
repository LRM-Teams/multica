...
- [ ] evaluation-private 与被评对象隔离。
  - [x] D-version-1：migration 356 将 Passport `current_version` 发布视为授权选择变化；除首次 artifact-create 的 `NULL → 1` 外，必须在同一事务将 eligibility revision 加一、推进 policy watermark、写入带 old/new Version 的 `current_version` mutation 并更新 Passport。Passport→ledger 与 ledger→Passport 双向 deferred guard 拒绝只改指针、错误 mutation kind 和孤立 ledger，§15.5 收口。
