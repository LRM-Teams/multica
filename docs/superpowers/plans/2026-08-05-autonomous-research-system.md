...
- [ ] evaluation-private 与被评对象隔离。
  ...
  - [x] D-context-4：migration 354 为每个 Manifest entry 保存 selection-time lifecycle、provenance、version count、input/output reference count；原有 entry 已无法证明派发时值，因此诚实保留 NULL，新 D dispatch 必须写全。最终 Manifest hash 同时绑定这些字段和所选 version/access/eligibility/representation；task-bound read 和 Result acceptance 共用同一重建函数，遇到 NULL、hash 缺失或篡改均 fail closed。task-bound `ArtifactProjection` 直接从 entry snapshot 与所选 immutable Version 构建，不再按 allowed ID 回查 live Passport 当前版本、生命周期、provenance 或引用计数，也不把 Manifest 自身伪装为递归 entry；AttemptContext 继续提供 Manifest 身份。单测逐字段证明 hash 绑定；PostgreSQL 回归在 dispatch 后合法 withdrawal，证明 human projection 前进而旧 Attempt projection item/hash 完全稳定，并篡改 selection count 证明读取在返回前拒绝。D §15 其余 partial matrix 仍未完成。
