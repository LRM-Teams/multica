...
| zoom | 卡片显示 | 其他节点 | 禁止 |
| --- | --- | --- | --- |
| <35% | root、selected、active/blocking、顶层 Insight/Decision 地标卡，最多 12 张 | Waypoint、Trail Dot、Route Bundle 为主 | 把所有卡缩成不可读白块 |
| 35–65% | 紧凑地标卡，最多 24 张 | 主要节点为 Waypoint，其余为点/路径束 | 细正文、操作按钮 |
| 66–119% | 标准卡全部 7 行中的必要项 | 中等节点保持 Waypoint | 完整证据列表 |
| ≥120% | 增加最多 2 条进展/风险、端口标签 | 被选路标可晋升紧凑卡 | 把 Inspector 全塞进卡片 |

密度切换遵守 route-topology 规格。卡片、路标和微圆点是三个离散渲染形态，使用稳定锚点交叉淡入；不通过缩放字体或整卡 scale 形成 LOD。
