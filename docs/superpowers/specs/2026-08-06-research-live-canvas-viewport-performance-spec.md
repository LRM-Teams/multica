...
| viewport | semantic node soft limit | semantic node hard limit | edge hard limit |
| --- | ---: | ---: | ---: |
| desktop ≥1200 | 120 | 180 | 420 |
| 768–1199 | 72 | 96 | 220 |
| <768 | 32 | 48 | 96 |

semantic node 包含 Landmark Card、Waypoint、Trail Dot、Route Bundle 和 Display Group；各形态子上限见 route-topology 规格。DOM 总预算包含 canonical card、Display Group、route hit target、gutter/anchor，desktop 不超过 220 个图节点元素。Trail Dot 通过单层 route renderer 和 event delegation 批量绘制，不能每个点各建 React component。达到 soft limit 后新展开先折叠远端低优先路径；达到 hard limit 后拒绝直接铺开，显示路径束/分组卡和局部 Spotlight。
...
→ classify Landmark / Waypoint / Trail Dot
→ render canonical nodes + honest Route Bundles / Display Groups
```
...
- Route Bundle 显示真实 outcome 计数、Agent 数和起止锚点；Display Group 显示 relation、节点数、running/failed/stale 摘要和 top 3 Agent；两者都不展示伪结论。
- 点击 Display Group：先计算是否超 hard limit；超出则打开局部 drill view，不铺回主画布。
- 自动整理只改变 display positions/hidden set；同一输入产生稳定顺序。最终路径使用稳定 cubic Bézier/样条与避障 corridor，不能退化成 orthogonal tree 或每层等距列。
- Delta 只重新布局 `affectedRootIds`，未影响节点位置必须逐像素保持。

## 7. 10k 性能门

- 首屏网络不得请求全部 10k 节点。
- Canvas 首屏 semantic nodes ≤soft limit；25% zoom Landmark Card ≤12；Trajectory 使用 window+overscan。
...
- 只画已加载 Slice 的路径密度、Route Bundle 和 Landmark anchor，并在标题写“已加载 x / 发现 y+”；不复制每个 Trail Dot。
