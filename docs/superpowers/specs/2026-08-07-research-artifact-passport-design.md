...
`research_artifact_policy_mutation` is the append-only enforcement ledger. It has scope and watermark plus exactly one target shape: `(artifact_id, old_eligibility_revision, new_eligibility_revision)` or `(policy_grant_id, old_grant_revision, new_grant_revision)`, selected by a CHECK. It has scoped FKs to passport and policy grant and partial unique keys `(workspace_id, session_id, watermark, artifact_id, new_eligibility_revision)` and `(workspace_id, session_id, watermark, policy_grant_id, new_grant_revision)`. An owner increments policy state once with `UPDATE ... SET watermark = watermark + 1 ... RETURNING watermark`, applies every affected passport/grant revision, then inserts one matching ledger row per target at that returned watermark. A newly created artifact uses the virtual predecessor revision `0` and stored initial `eligibility_revision=1`; a newly created grant analogously records `0→1`. Deferred reciprocal guards require the changed target, exact old/new revisions, mutation kind, and policy state at that watermark, so a fabricated ledger row or a direct lifecycle/access/verification/current-version/grant update fails when constraints are checked.

The creation order is executable with the immediate scoped FKs: reserve the watermark; insert each new passport with `current_version=1` and `eligibility_revision=1`; insert version 1 (interleaving its deferred reciprocal domain row where an immediate producer FK requires it); then insert its policy-mutation row `0→1`. Existing artifacts/grants update `n→n+1` before their matching ledger rows. The passport current-version FK and all class/eligibility reciprocity checks remain `DEFERRABLE INITIALLY DEFERRED`; the policy-mutation-to-passport/grant FKs themselves need not be deferred because their targets now exist first. Attempt, Context Manifest, Result Artifact, and every newly materialized output follow this order in their existing owner transaction.
...
Evidence Links are selected as their own manifest entries. Selection is closed over the parent Claim and Observation: an admitted link requires both endpoint versions in the same manifest, and a Claim includes every link that the legacy `ListClaims` result would nest unless a named pre-D deny invariant excludes it. The manifest-to-`RunSnapshot` projector emits no new top-level V1–V5 family; it nests the selected link representations into `Claim.evidence`. Claims are ordered by `(goal_version, plan_version, created_at, id)` and each Claim's links by `(created_at, observation_id, relation, id)`. Those keys, including the final UUID tie-breaker missing from the old SQL, are frozen compatibility order. Claim bytes contain only Claim fields; nested Evidence bytes come from the independently hashed Evidence Link entries, so changing a link does not rewrite the Claim version.
...
Only after those checks does the transaction create Result Artifact and every materialized output with the section 4.5 order: passport with version/revision 1, version 1, reciprocal domain row as immediate producer FKs require, then the matching policy-mutation row `0→1` at the reserved watermark. A semantic update to an existing output instead creates version `n+1`, advances eligibility `n→n+1`, and then writes that exact ledger row. Result Artifact, every Source Snapshot, Observation, Claim, Evidence Link, Report Revision, Evaluation/Decision, and any future output created by the envelope is covered; no output is exempt because it is nested in another representation. The transaction then creates scoped lineage/supersession/lifecycle rows, writes the exact Attempt compatibility projection, Decision, and Event, and terminally transitions the Attempt. Output access taint is derived from locked inputs; callers cannot choose it.
...
For each prospective D-enabled V1–V5 dispatch, shadow mode runs legacy `TaskContext` and manifest selection over one database snapshot and compares, in deterministic order: family/kind, canonical domain ID, version content hash, chosen representation bytes/hash, nesting parent and ordinal, and omission classification. It also renders both Prompts and compares bytes/hash. The expected equal set includes every legacy family enumerated in section 13. Its mandatory positive fixture contains a succeeded Task, its succeeded Attempt, valid Result, Claim, Observation, and durable Evidence Link; the terminal Task/Attempt and link must remain present as read-only lineage, and the link must appear under the same Claim at the same stable ordinal.
...
2. scope-key DDL inspection proves every D version/result/context/entry/omission/input/supersession/lifecycle/policy-state/policy-grant/policy-mutation row has non-null workspace/session and every applicable endpoint has its composite FK; it also asserts the complete class/verification/grant/append-only trigger inventory by the stable names in section 4.7;
...
4. deferred class/reciprocity tests reject missing domain row, missing passport, wrong kind, wrong scope, orphan delete, producer Attempt/Task mismatch, Result/Attempt projection mismatch, one-sided verification change, fabricated verification mutation, one-sided grant change, and fabricated grant mutation. Every invalid and valid control runs once with `SET CONSTRAINTS ALL IMMEDIATE` and once through normal deferred commit;
5. version immutability and current-version deferred FK reject update/delete, missing current version, invalid CAS, and eligibility/grant change without the exact policy-mutation ledger row. Direct `UPDATE` or `DELETE` of lifecycle and policy-mutation ledgers fails through the named append-only guards. Concurrent writers leave one current version and monotonic watermark;
...
8. shadow equivalence fixture contains legacy source, graph nodes/edges, messages, product round, thought strategy, report, stage eval, succeeded Task/Attempt, and every durable Run subfamily including durable Evidence Link. It compares ordered IDs/kinds, hashes, representation bytes, nesting parent/ordinal, omissions, and Prompt hash. Removing any family or Evidence Link, moving a link to another Claim, or swapping two link ordinals makes the test fail;
...
15. Chapter C `txOpDispatchIntentCreate` after-begin, before-commit, successful commit, and after-commit-unknown tests preserve the existing label and runner-hook order and count Attempt passport/version, manifest passport/version, their `0→1` policy-mutation rows, entries/omissions/edges, outbox, Task transition, and Event as one atomic unit. An FK-order fixture proves passport/version precede each initial ledger insert; both `SET CONSTRAINTS ALL IMMEDIATE` and ordinary commit accept the valid unit and reject ledger-first, missing-version, wrong-watermark, and non-`0→1` initial revisions;
...
22. Chapter C `txOpResultAccept` fault matrix preserves the existing label and runner-hook order and proves Result Artifact, compatibility projection, all outputs/passports/versions/links, each new Result/output `0→1` policy-mutation row, Decision, Event, and terminal transition are all-or-none and converge after unknown commit. `SET CONSTRAINTS ALL IMMEDIATE` and ordinary commit fixtures reject ledger-first, missing passport/version, wrong watermark, and initial revisions other than `0→1` for Result, Evidence Link, and every output kind;
