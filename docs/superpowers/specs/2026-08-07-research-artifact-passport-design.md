...
`research_artifact_policy_state` has `workspace_id`, `session_id`, `policy_version`, and `watermark bigint not null`; this `watermark` is the session policy/lifecycle watermark; primary key `(workspace_id, session_id)`, unique `(workspace_id, session_id, watermark)`, and FK `(workspace_id, session_id)` → session `(workspace_id, id)`. Every lifecycle/access/verification/supersession/grant mutation locks this row first and increments `watermark` in the same transaction. Policy deployment that changes admission increments every affected session watermark before new dispatch/acceptance.

`research_artifact_policy_grant` is the durable grant store. It has `id`, `workspace_id`, `session_id`, `principal_kind`, immutable `principal_id`, `purpose`, nullable normal clearance, `evaluation_private boolean`, `policy_version`, `revision bigint not null default 1`, `status` (`active | revoked`), `created_at`, and write-once `revoked_at`. It has `UNIQUE (workspace_id, session_id, id)` and a scoped session FK. A CHECK requires exactly one grant compartment: either one normal clearance or `evaluation_private=true`, never both; a process needing both compartments names two grants. Task/Attempt binding and active membership remain separate required checks, so possession of a grant ID is not authorization. Revocation advances revision and the session watermark; grants are never moved across principal, purpose, compartment, workspace, or session. A Context Manifest stores the exact normal and evaluation grant IDs/revisions used, with scoped FKs; each is nullable only when that compartment was not used.

`research_artifact_policy_mutation` is the append-only enforcement ledger. It has `id uuid primary key`, scope, `watermark`, `mutation_kind`, and exactly one target shape: `(artifact_id, old_eligibility_revision, new_eligibility_revision)` or `(policy_grant_id, old_grant_revision, new_grant_revision)`, selected by a CHECK. Artifact rows additionally carry nullable old/new fact columns required by their kind: `old_current_version`, `new_current_version`, `old_access_level`, `new_access_level`, `old_lifecycle_status`, `new_lifecycle_status`, and `eligibility_reason`. Grant rows carry old/new status. Nullability CHECKs below make a ledger row self-describing rather than asking a trigger to infer a historical fact from the current projection.

`mutation_kind` is normative and closed in D:

| `mutation_kind` | Exact allowed transition |
| --- | --- |
| `artifact_create` | Virtual artifact revision `0→1`, no old facts, `new_current_version=1`, and the inserted passport/version facts. |
| `current_version` | Existing artifact revision `n→n+1`; current version changes to a newly inserted immutable version and old/new access levels are equal. |
| `access` | Existing artifact revision `n→n+1`; current version changes to a newly inserted immutable version and old/new access levels differ. An access label is never updated in place. |
| `lifecycle` | Existing artifact revision `n→n+1`; passport lifecycle changes, current version/access do not, and one exact lifecycle event records the same old/new status. |
| `verification` | Existing artifact revision `n→n+1`; one registered verification-bearing domain fact changes and current version/access/lifecycle do not. |
| `supersession` | Existing artifact revision `n→n+1`; one exact supersession edge makes the target version audit-only and records the same revision and watermark. |
| `eligibility` | Existing artifact revision `n→n+1` for a fail-closed, registry-named admission fact not represented by the preceding kinds; `eligibility_reason` is a nonempty value from the same SQL registry. It is forbidden as a generic substitute for lifecycle, current-version, access, verification, or supersession. |
| `grant_create` | Virtual grant revision `0→1` and the inserted active grant facts. |
| `grant_revoke` | Existing grant revision `n→n+1`, exact `active→revoked` status, and write-once `revoked_at`. |

One row represents one semantic class. If one business operation changes more than one class, it applies consecutive revisions in registry order—`current_version|access`, verification, supersession, lifecycle, then other eligibility—and writes one row per step at the same reserved watermark. A CHECK requires `new_*_revision = old_*_revision + 1`. The target history keys are `UNIQUE (workspace_id, session_id, artifact_id, new_eligibility_revision) WHERE artifact_id IS NOT NULL` and `UNIQUE (workspace_id, session_id, policy_grant_id, new_grant_revision) WHERE policy_grant_id IS NOT NULL`; they are global for that scoped target history and deliberately do **not** contain `watermark`. Additional indexes may begin with watermark, but a duplicate target revision cannot be made distinct by choosing another watermark.

The generic deferred pair is named `research_artifact_passport_to_policy_mutation_guard` and `research_artifact_policy_mutation_to_passport_guard`. The passport-side trigger fires after insert or update of scope, `current_version`, `eligibility_revision`, `lifecycle_status`, or any registry-owned eligibility projection. It requires exactly one matching artifact ledger row for every `0→1` insert or `n→n+1` change, compares the old/new current-version rows to classify `current_version` versus `access`, checks the exact old/new lifecycle and access facts, and rejects a revision jump, no-op revision bump, semantic change without a bump, or multiple matching rows. The ledger-side trigger rejects a row unless the same transaction contains that exact passport transition and the session policy-state row equals its watermark. Because access lives on the immutable version, the trigger follows the old and new passport pointers; inserting a version without moving the pointer is harmless, while moving the pointer without the exact ledger kind is not. The existing verification and grant pairs are kind-specific extensions of this generic pair, not alternate enforcement paths.

An owner increments policy state once with `UPDATE ... SET watermark = watermark + 1 ... RETURNING watermark`, applies every affected passport/grant revision, and inserts one matching ledger row per revision step at that returned watermark. A newly created artifact uses virtual predecessor `0` and stored initial `eligibility_revision=1`; a newly created grant is analogous. The executable creation order is: reserve the watermark; insert each passport with `current_version=1` and revision 1; insert version 1 (interleaving the deferred reciprocal domain row where an immediate producer FK requires it); then insert `artifact_create 0→1`. Existing artifacts/grants update one revision step before its ledger row. Passport current-version/class/policy reciprocity remains `DEFERRABLE INITIALLY DEFERRED`; immediate scoped target FKs remain immediate. Attempt, Context Manifest, Result Artifact, and every newly materialized output follow this order in their owning transaction.

One `research_artifact_context_manifest` is frozen per Attempt. It has `workspace_id`, `session_id`, `id`, `attempt_id`, `task_id`, `purpose`, `policy_version`, `policy_watermark`, `through_state_version`, exact normal/evaluation policy-grant IDs and revisions, bounded `principal_header_bytes`, `principal_header_hash`, `manifest_hash`, and `created_at`. The principal header freezes the compatibility Fleet/roster metadata that is not itself a Research artifact. It has scoped unique keys for `id` and `attempt_id`, FKs to Attempt, Task, and each used policy grant, FK from its `id` to a `context_manifest` passport, and a deferred check that Attempt.task_id equals manifest.task_id and each grant matches scope, purpose, principal, compartment, revision, and policy version.
...
`research_claim_evidence` is the durable Evidence Link, not anonymous nested JSON. Its existing UUID is the `evidence_link` passport ID; its version freezes Claim and Observation endpoint IDs, relation, strength, directness, method fit, verification status, verifier Task, and rationale. Both endpoints and the verifier use scoped composite FKs. A semantic or verification change creates a new Evidence Link version and advances eligibility under section 4.5; it never mutates an old artifact version.

Evidence Links are selected as their own manifest entries. Selection is closed over the parent Claim and Observation: an admitted link requires both endpoint versions in the same manifest, and a Claim includes every link that the legacy `ListClaims` result would nest unless a named pre-D deny invariant excludes it. The manifest-to-`RunSnapshot` projector emits no new top-level V1–V5 family; it nests selected link representations into `Claim.evidence` and continues to omit the link UUID because the current `ClaimEvidence` wire type does not serialize it.

The actual pre-D `ListClaims` SQL orders Claims by `(goal_version, plan_version, created_at, id)` and its second query orders Evidence rows globally by `(created_at, claim_id, observation_id)`. It does **not** order by relation or Evidence Link ID. D does not claim imaginary compatibility fields or call an exact-key tie deterministic. Before shadow comparison or any Run enablement, a compatibility-only prerequisite changes that same legacy query to `ORDER BY created_at, claim_id, observation_id, relation, id`; both the historical serializer and manifest projector call the one shared ordering helper/query. A fixture inserts two links with identical `created_at`, Claim, and Observation but different relation/UUID, proves the pre-change query is under-specified, and pins the post-change order `(created_at, claim_id, observation_id, relation, id)` in both paths. Shadow baselines are captured only after this prerequisite lands. Claim bytes contain only Claim fields; nested Evidence bytes come from independently hashed Evidence Link entries, so changing a link does not rewrite the Claim version.
...
- `research_artifact_supersession`: `successor_version_id`, `superseded_version_id`, denormalized `superseded_artifact_id`, reason, non-null `decision_id`, `policy_watermark`, `old_eligibility_revision`, `new_eligibility_revision`, and created time. A deferred check requires `superseded_artifact_id` to equal the passport owning `superseded_version_id`. Both versions and Decision are scoped FKs; self-reference and cycles are forbidden; the endpoint pair is unique. The superseded version's passport is the mutation target. A composite FK from `(workspace_id, session_id, superseded_artifact_id, new_eligibility_revision)` to the target-history unique key plus deferred pair `research_artifact_supersession_to_policy_guard` / `research_artifact_policy_mutation_to_supersession_guard` requires exactly one `supersession` ledger row with the same old/new revision and watermark. The edge cannot commit without the passport bump, and a supersession ledger/passport bump cannot commit without the exact edge.
- `research_artifact_lifecycle_event`: scoped passport ID, old/new status, old/new eligibility revision, policy watermark, nullable Decision, actor facts, reason, and created time. Passport and Decision are scoped FKs; a composite FK names the target-history ledger key. Deferred pair `research_artifact_lifecycle_event_to_policy_guard` / `research_artifact_policy_mutation_to_lifecycle_event_guard` requires the exact `lifecycle` mutation, passport old/new status, revision, and watermark in both directions. A lifecycle transition, a lifecycle event, or a lifecycle ledger row cannot be committed alone.
...
2. Each registered domain table gets a reciprocal `AFTER INSERT OR UPDATE OF id, workspace_id, session_id CONSTRAINT TRIGGER`, also deferred. Its exact SQL identifier is the table name followed by `_artifact_passport_guard` (for example, `research_task_artifact_passport_guard` and `research_claim_evidence_artifact_passport_guard`); it requires exactly one matching passport with the expected kind. The complete migration trigger inventory is generated from the same fail-closed kind registry and asserted by exact catalog name in migration tests; Result Artifact and Context Manifest use this trigger plus their direct scoped passport FK. No registered kind may rely only on the passport-side guard.
3. `research_artifact_passport_delete_guard` is a `BEFORE DELETE` trigger that checks the kind-selected domain table and rejects while the reciprocal row exists. Each domain table has a matching trigger whose exact SQL identifier is the table name followed by `_artifact_passport_delete_guard` (for example, `research_task_artifact_passport_delete_guard`). Lifecycle withdrawal, not deletion, is the production removal operation. Migration rollback drops the guards before D tables.
...
6. The three verification-bearing domain tables have explicit deferred domain-side triggers: `research_source_snapshot_verification_to_policy_guard`, `research_observation_verification_to_policy_guard`, and `research_claim_evidence_verification_to_policy_guard`. Each fires after insert or change of scope/ID/verification status and requires the corresponding passport revision and exact policy-mutation row. The reciprocal ledger-side `research_artifact_policy_mutation_to_verification_guard` CASEs on `source_snapshot`, `observation`, and `evidence_link` and rejects a `verification` mutation unless that exact scoped domain fact changed in the same transaction. This prevents either half from being fabricated independently.
7. Policy grants have the deferred pair `research_artifact_policy_grant_to_mutation_guard` and `research_artifact_policy_mutation_to_grant_guard`. They enforce exact old/new grant revision, active/revoked fact, compartment/purpose immutability, session watermark, and same-scope target in both directions.
8. `research_artifact_lifecycle_event_append_only_guard` and `research_artifact_policy_mutation_append_only_guard` are `BEFORE UPDATE OR DELETE` triggers that always reject. The version immutable guard, these two ledger guards, and the domain/passport delete guards use stable constraint names and SQLSTATE `55000`; only the down migration may drop them before dropping D tables.

Integration tests exercise every named trigger twice: first force `SET CONSTRAINTS ALL IMMEDIATE` after the statements under test and then roll back; second run the same fixture without forcing constraints and assert the commit result. Positive fixtures prove valid paired writes in both modes. This is the executable enforcement strategy for class polymorphism and reciprocal policy/verification coupling; application checks are defense in depth only.

### 4.8 Exhaustive existing serialized-relationship inventory

The inventory below is normative for D1 migration and D2 admission. “Typed relationship” means an existing relational column, JSON field, local key, nested ownership, or generated projection that current code interprets as pointing to another entity or typed subobject. D never discovers references by guessing that every key ending in `_id` is a relationship.

| Current serialized owner/surface | Existing typed relationships | D enforcement |
| --- | --- | --- |
| Session and Fleet header | `session.workspace_id`, `fleet_id`, `created_by`, `project_id`, `channel_id`; `fleet.lead_agent_id`; `fleet.members[].agent_id`. | Session→Fleet uses the existing workspace-qualified key; user/agent/project/channel remain principal/routing FKs and are copied only into the bounded header. They do not become artifact input edges. |
| Stored legacy graph | Edge `from_node_id`/`to_node_id`; node `actor_agent_id` and `run_event_id`; known node-payload `source_id`, `question_id`, `task_id`, and nested `details.question_id`/`details.task_id`; projected node `parent_id`/`child_ids`; Thought Strategy `node_id`. | D adds scope columns/unique keys where absent and composite FKs for stored edge and Run Event endpoints. Known JSON fields use the registry parser, same-scope resolution, and exact version edges; malformed/unresolved values are diagnosed and denied. Parent/children and Thought Strategy are deterministic projections of admitted nodes/edges and are never separately trusted. |
| Durable graph compatibility projection | Question-node payload `question_id`, `parent_question_id`, `created_by_task_id`; Task-node payload/details `task_id`, `question_id`, `parent_task_id`, `assigned_agent_id`; Attempt-node payload/details `attempt_id`, `task_id`, `assigned_agent_id`, `inbox_task_id`, execution-target agent/runtime IDs; Claim-node payload/details `claim_id`, `produced_by_task_id`. Generated edges encode Question parent, Task parent/Question, Attempt→Task, and Claim→producer Task. | These bytes are generated only from admitted, scoped manifest entries and validated principal facts. They are not stored lineage and cannot substitute for the underlying composite FKs/input edges. Projection tests cover every listed key and endpoint. |
| Research Message | `sender_type+sender_id`, `target_agent_id`, `run_event_id`; `meta.match_decision.utterance_id`, `primary_anchor_node_id`, `matched_node_ids[]`, and `decisions[].node_id`. | Relational fields receive scoped keys where applicable. JSON arrays/polymorphic sender cannot have a direct FK: the migration parser validates sender kind, applies the current empty-utterance fallback to the owning Message ID, otherwise resolves the explicit utterance to a same-session Research Message, resolves every node to the same scoped graph, emits input references, and diagnoses/denies the Message on any unresolved or cross-scope value. |
| Legacy Source | Relational `source_snapshot_id`; currently emitted `payload.snapshot_id` is a duplicate typed reference. | Composite FK to Source Snapshot; a CHECK/trigger requires JSON `snapshot_id`, when present, to equal the relational column. Malformed, mismatched, or JSON-only historical values are diagnosed and denied rather than inferred. |
| Legacy Report/structured reader schema | Producer Task/Attempt/author; `report_claim(report_id, claim_id, section_id)`; outline `children[]`→outline/section IDs; section `citation_ids[]`→citation IDs; citation `source_id`→structured source ID and same-session legacy Source; structured source `source_id`→legacy Source. | Producer/report-claim relationships receive scoped composite FKs. Internal outline/section/citation IDs are document-local strings, so the migration runs the existing strict reader validator, resolves each canonical Source to an input-version edge, and denies/diagnoses missing, duplicate, ambiguous, cross-scope, or dangling local keys. |
| Legacy Stage Evaluation and Product Round | Stage Evaluation owns no current typed artifact reference in `findings`; Product Round `decided_by_agent_id` is a principal reference and `coverage_gaps` has no current typed reference schema. | Preserve these as content. If a later schema interprets a field as a reference, registry/test/inventory update is mandatory before D-enabled reads. |
| Durable Run/Contract/Method | Run `session_id`/`workspace_id`/`fleet_id`/`created_by`; current Contract selected by `(session, goal_version)`; Method Decision `inputs.attempt_id`, `inputs.task_id`, outcome `created_by_task_id`/`created_by_agent_id`; Claim `evidence_standard_key`→Method-local Evidence Standard. | Scope/principal keys are checked. Method JSON cannot carry declarative FKs: a per-`research_method` parser resolves Task/Attempt and records version edges; Method-local standard keys are unique and Claims are denied if the selected Method version does not define the key. |
| Question | `parent_question_id`, `created_by_task_id`, `answer_claim_id`. | Scoped composite FKs to Question/Task/Claim, cycle check, and input edges to the exact referenced versions. |
| Task and Task Dependency | `question_id`, `parent_task_id`, `assigned_agent_id`; dependency `task_id`/`depends_on_task_id`; known control-task `acceptance_criteria.remediation.target_findings[].metadata` fields including `question_id`, `answer_claim_id`, `claim_key`, `evidence_standard_key`, `evaluation_decision_id`, `report_id`, `task_id`, and `attempt_id`. | Task/Question/parent/dependency endpoints receive scoped composite FKs; D adds scope columns to `research_task_dependency`. Principal attribution remains immutable. Known acceptance-criteria schemas get registry parsers and exact version edges; opaque Agent-authored criteria remain bytes, not silently interpreted. Any known typed value that cannot resolve is diagnosed and denies that Task. |
| Attempt | `task_id`, assigned Agent, Inbox Task, and execution-target Agent/runtime; accepted Result projection contains Result-envelope local keys below. | Scoped composite FK to Task; principal/runtime/Inbox bindings remain required authorization facts. Result JSON is validated under its stored orchestrator schema and equals the canonical Result Artifact projection. |
| Source Snapshot and Observation | Source `produced_by_task_id`; Observation `source_snapshot_id`, `produced_by_task_id`. | Scoped composite FKs and exact input-version edges. |
| Claim and Evidence Link | Claim `produced_by_task_id`, Method-local `evidence_standard_key`; nested Evidence ownership implies `claim_id`; Evidence `observation_id`, `verified_by_task_id`; stored link also has `claim_id` although link ID is not serialized in V1–V5. | Scoped composite FKs for Claim/Observation/verifier, Method-key validation, independently versioned Evidence Link, and manifest closure over both endpoint versions. |
| Decision, Gate, and Run Event | Decision `inputs` currently uses typed `task_id`, `attempt_id`, `report_id`, and before/after graph objects; Evaluation outcome uses Claim/section keys. Gate metadata currently emits `question_id`, `answer_claim_id`, `evaluation_decision_id`, `report_id`, `task_id`, `attempt_id`, Claim keys, Evidence Standard keys, and reviewed Claim/section keys. Event payloads currently emit `task_id`, `attempt_id`, `inbox_task_id`, `agent_id`, `question_id`, `report_id`, `repair_id`, and `circuit_id` according to the closed event-type registry. | Each current `decision_kind` and `event_type` has a closed parser that resolves same-scope artifact references into input-version edges; Gate remains a projection from those admitted versions. Polymorphic JSON cannot have a direct FK, so unknown schema, malformed UUID/key, ambiguous local key, cross-scope target, or unresolved reference creates a diagnostic and denies the owning Decision/Event from D admission. |
| Result Envelope local graph | `question.parent_client_key`; `task.question_key` and `depends_on[]`; Observation `source_key`; Claim `evidence_standard_key` and Evidence `observation_key`; top-level `answer_claim_key`; Report Claim `claim_key`/`section_id`; structured outline `children[]`, section `citation_ids[]`, citation/source `source_id`; Evaluation `reviewed_claim_keys[]`/`reviewed_section_ids[]`; Defect `claim_keys[]`/`section_ids[]`. | These are transaction-local keys, not UUID FKs. Strict version-selected validation first resolves each key to either a newly created artifact version or a locked manifested existing version; materialization then writes the normalized composite FK/input edge. Unknown, duplicate, ambiguous, cyclic, unmanifested, or cross-scope resolution rejects the entire Result. |

All feasible session-owned relational relationships are upgraded to `(workspace_id, session_id, target_id)` composite FKs, including graph edges, Task dependencies, Question links, Attempt→Task, producer links, Observation→Source, Evidence Link endpoints/verifier, Report producers and `research_report_claim`, legacy Source→Source Snapshot, projection `run_event_id`, and every D table. D1 creates `research_artifact_migration_diagnostic` with scope, owner kind/ID, column or JSON Pointer, expected target kind, bounded non-content reference value, reason code, and detection time. Every impossible-to-FK relationship listed above has both a migration parser and an admission parser using the same registry. A diagnostic blocks the owner passport/version and every container that would serialize it into a D-enabled task; it does not block the historical live route. There is no “best effort” drop, session-wide fallback, UUID-only lookup, or fabricated edge.

Migration tests insert one broken and one same-scope valid control for every table row and every JSON/local-key relationship row above. They prove: feasible cross-scope references fail at the composite FK; impossible legacy references produce the exact diagnostic and ordinary-admission denial; fixing the reference clears the restartable diagnostic and admits it; and adding a new typed serializer field without a registry classification fails the inventory contract test.
...
2. lock/re-read in this global order: Run/session row; session policy-state row; policy grants by UUID; Task row; candidate passports by UUID; candidate versions by UUID; then kind-specific verification/contract/producer rows by `(kind, UUID)`;
...
7. atomically create Attempt and Context Manifest with this fixed sub-order: Attempt passport/version 1, deferred reciprocal Attempt row; Context Manifest passport/version 1, deferred reciprocal Manifest row; policy-mutation rows `0→1` for both passports at the reserved watermark; then entries/omissions/edges, frozen dispatch outbox payload containing those exact Prompt bytes and manifest ID/hash, Task transition, and dispatch Event. Immediate producer FKs may cause the matching domain row to be interleaved after its passport/version, but neither initial ledger row may precede its target passport and version;
8. call `s.commitResearchTx(ctx, txOpDispatchIntentCreate, tx)`; no direct `Commit` is allowed.

All lifecycle/access/verification/supersession/grant writers use the same Run → policy-state → policy-grant → passport/version lock order and increment the affected grant/passport revision and session policy watermark. They therefore cannot pass between dispatch recheck and commit. An unrelated watermark advance makes dispatch retry and recompute rather than silently using stale policy.
...
3. policy grants ordered by UUID;
4. Task, then Attempt;
5. Context Manifest and its entries;
6. every referenced passport ordered by UUID;
7. every referenced version ordered by UUID;
8. kind-specific verification/contract/producer rows ordered by `(kind, UUID)`.

It then verifies manifest/Attempt/task/principal identity, Prompt/manifest hashes, Result schema version, and all scoped references. If the current policy watermark differs from the manifest watermark, it re-evaluates every reference under the locked current policy; withdrawal, lifecycle, access, verification, or supersession changes reject the Result. If the watermark advanced only for unrelated artifacts and all referenced eligibility revisions/facts still match, acceptance may proceed. Before writing outputs it reserves one new final watermark with the same compare-and-increment statement and records that returned value as `research_result_artifact.acceptance_policy_watermark`. Every referenced passport is CAS-checked on `(current_version, eligibility_revision, lifecycle_status)` and every version on immutable `(id, content_hash, access_level)` predicates before writes. It does not insert a policy-mutation row before its target exists.

Only after those checks does the transaction mutate the complete acceptance set. Before reserving the final watermark it discovers and locks, by `(kind, UUID)`, every existing row that an upsert, replan, progress update, dependency activation, or verification promotion can change; each is compared with the preflight expected version/current-version/eligibility/lifecycle/content hash. A row found only after that lock set was formed is a retryable conflict, not an unlocked upsert.

The complete current acceptance set is normative:

- Run/Session: plan version on replan, current stage, state/eligibility revision, progress fields, and `run_stats`;
- the accepted Method Decision and every other Decision created by the path, including quality/citation Evaluation and Information Gain;
- every Question created or semantically updated, including parent, replan-obsolete state, coverage/status, and answer Claim;
- every Task created or semantically updated, including parent/Question binding, current Task terminal transition, replan-obsolete rows, pending/ready changes, and every Task Dependency inserted or changed;
- the accepting Attempt, canonical Result Artifact, compatibility Result projection, and any execution-circuit/probe settlement that is part of current atomic acceptance;
- every durable Source Snapshot plus its legacy `research_source` projection, Observation, Claim, Evidence Link, Report Revision, `research_report_claim`, Evaluation/Decision, and their exact typed input references;
- the acceptance lifecycle/supersession rows, Run Event, and any other current or future canonical output materialized from the envelope, including an output nested in another representation.

For each new passport-bearing row the order is passport revision 1, version 1, reciprocal domain row as immediate producer FKs require, then `artifact_create 0→1`. For each semantically changed row, immutable content change creates version `n+1`; access change uses `access`; verification, supersession, lifecycle, or other admission-only change retains content version when appropriate but advances eligibility one step with its exact kind, reciprocal event/edge/domain fact, and ledger row. Projection/link rows without their own passport still use scoped composite FKs and are included in the same transaction. Every locked existing target uses CAS on its expected current version, eligibility revision, lifecycle/domain state, and content/access facts. No Run, Method, Question, Task, Source, Observation, Claim, Evidence Link, Report, Evaluation/Decision, nested output, or current side effect is exempt from version/revision/ledger accounting merely because current code uses `ON CONFLICT`, JSON, or a projection table. Output access taint is derived from locked inputs; callers cannot choose it.
...
### 10.2 Exact per-kind admission for `registered + partial|unknown`

Backfilled passports use `lifecycle_status=registered`, `provenance_completeness=partial|unknown`, and migration-time `registered_at`. `registered` is not generally equivalent to `accepted`. The only ordinary-task exception is named policy `legacy-v1-v5-compat-v1`; it has no generic “nonterminal” predicate. It applies only to a V1–V5 task on a Run explicitly D-enabled after shadow success and evaluates the registered kind's admissibility function:

| Registered kind/family | Admissible persisted domain fact | Mandatory exclusion or restriction |
| --- | --- | --- |
| Run/Session, Contract, Method Decision, Report Revision, Evaluation/Decision, Stage Evaluation, Message, Product Round, Run Event, legacy source/graph | A schema-valid row reachable through the exact typed legacy `TaskContext` relationship. A historically terminal Run or immutable decision is not invalid merely because work has ended. | Unknown enum/schema, malformed content, or a row the legacy typed traversal would not select is denied. |
| Question | Any CHECK-valid historical status, including `answered` and `obsolete`; terminal rows are read-only lineage. | Unknown status or policy-withdrawn version is denied. |
| Task | Any CHECK-valid status: `pending`, `ready`, `dispatching`, `running`, `succeeded`, `failed`, `blocked`, `obsolete`, or `cancelled`. `succeeded`, `failed`, `blocked`, `obsolete`, and `cancelled` are preserved as read-only execution lineage and never become dispatch authority. | Unknown/constraint-invalid status, invalid scope, or policy-withdrawn version is denied. |
| Attempt | Any CHECK-valid status: `dispatching`, `running`, `cancelling`, `succeeded`, `failed`, `cancelled`, or `lost`. `succeeded`, `failed`, `cancelled`, and `lost` are preserved as read-only execution lineage and cannot resume, retry, or authorize a Result. | Unknown/constraint-invalid status, impossible Task binding, or policy-withdrawn version is denied. An `obsolete` Attempt is invalid because that value does not exist in the Attempt state machine. |
| Result Artifact | A stored Result whose historical canonical hash verifies and whose Attempt binding is same-scope. A succeeded Attempt/Task is the positive ordinary case. | Missing/malformed Result, hash mismatch, incompatible Attempt binding, or a Result attached to a non-result-bearing terminal fact is denied and diagnosed. |
| Source Snapshot, Observation, Evidence Link | `pending` or `verified` verification state as selected by legacy context, with valid same-scope typed endpoints; `verified_only` still requires the persisted verified invariant. | Verification `rejected`, unknown status, broken endpoint, or policy withdrawal is denied ordinary admission. |
| Claim | `proposed`, `supported`, `disputed`, `refuted`, or `unresolved` remains an honest historical Claim fact; `refuted` is read-only lineage, not support. | Unknown status is denied. Domain `superseded` and passport `superseded` are audit-only and excluded from ordinary context. |
| Context Manifest | Never admitted by the legacy exception; historical pre-D work has no fabricated manifest. | Always denied for legacy backfill. |

Every row must additionally have exact workspace/session identity, a recognized kind, a current immutable version, a stored or recomputed hash that verifies, and no passport/policy `withdrawn`, `rejected`, `stale`, or `superseded` fact. Domain-terminal is not the same as policy-invalid: valid succeeded/failed/cancelled/obsolete Task/Attempt history survives, while rejected evidence and withdrawn policy state do not.

The remaining common rules are:
...
Thus `registered+unknown` is not a wildcard: unknown producer history is tolerable for byte-identified legacy compatibility; unknown content/scope/access is not. Per-kind admission preserves valid history without converting terminal execution facts into current authority.

### 10.3 Shadow equivalence

For each prospective D-enabled V1–V5 dispatch, after the Evidence Link query prerequisite in section 4.6, shadow mode runs legacy `TaskContext` and manifest selection over one database snapshot and compares, in deterministic order: family/kind, canonical domain ID, version content hash, chosen representation bytes/hash, nesting parent and ordinal, and omission classification. It also renders both Prompts and compares bytes/hash. The expected equal set includes every legacy family enumerated in section 13 and every typed relationship in section 4.8. Its mandatory positive fixture contains a succeeded Task, its succeeded Attempt, valid Result, Claim, Observation, and durable Evidence Link; the terminal Task/Attempt and link must remain present as read-only lineage, and the link must appear under the same Claim at the same stable ordinal. The tie fixture with equal Evidence `created_at`/Claim/Observation proves both paths use the shared `(created_at, claim_id, observation_id, relation, id)` order.

There are only two acceptable outcomes: exact equality, or an explicit deny that the old path exposed data already forbidden by a pre-D invariant, with a named policy reason and a positive allowed control. Any unexplained omission, addition, reorder, re-parenting/nesting change, representation change, missing passport, or per-kind `registered` admission failure blocks Run enablement. Dedicated shadow mutations remove one Evidence Link and reverse two links with otherwise identical bytes; each must fail on omission or order rather than pass on set equality. Production task-bound reads switch only per Run after this comparison passes; historical runs continue live behavior.
...
Every transaction that can change context or acceptance eligibility—passport lifecycle, domain verification status, current version/access class, supersession, withdrawal, policy version/grant, or legacy-admission eligibility—locks Run then policy state, policy grants in UUID order, then affected passports in UUID order; increments each affected grant/passport revision; and increments the session policy watermark once. Direct updates and fabricated ledger-only writes are rejected by the reciprocal deferred triggers in section 4.7 because the exact old/new revision, domain/grant fact, mutation kind, and returned watermark must all agree. This is the shared TOCTOU fence used by sections 7 and 8.
...
All D write boundaries use the merged `PostgresStore.beginResearchTx` and `PostgresStore.commitResearchTx` methods with existing labels `txOpDispatchIntentCreate` and `txOpResultAccept` (and the relevant existing label for every other owner). Those label strings and the Chapter C runner order remain unchanged: begin transaction → `txAfterBegin` hook → canonical writes → `txBeforeCommit` hook → database commit (including deferred constraints) → `txAfterCommit` hook/unknown-outcome convergence. D must not call `SET CONSTRAINTS` to change this production order; that command is an integration-test probe only. No D code calls `s.pool.BeginTx`, `tx.Commit`, or introduces a nested transaction. Transaction-scoped artifact helpers accept `pgx.Tx` and cannot begin/commit. The Chapter C structural guard is expanded to every D-mutated file, and Chapter C before-/after-commit recovery tests count D rows as canonical children.
...
| durable Claims | One passport/version each. Claim scalar fields come from the Claim entry; selected Evidence Link entries are nested into `Claim.evidence` by `(created_at, observation_id, relation, id)` after Claims are ordered by `(goal_version, plan_version, created_at, id)`. | Retained authorized subset with stable V1–V5 nesting; no top-level Evidence Link family is added. |
| durable Evidence Links (`research_claim_evidence`) | One `evidence_link` passport/version per selected link, with same-scope Claim/Observation closure. Independently hashed manifest entries project into the owning Claim rather than anonymous JSON. | Exhaustively retained when legacy `ListClaims` would nest the link, unless a named pre-D deny invariant excludes it; omission, re-parenting, or reorder is a shadow mismatch. |
...
2. scope-key DDL inspection proves every D version/result/context/entry/omission/input/supersession/lifecycle/policy-state/policy-grant/policy-mutation row and every existing feasible relationship in section 4.8 has non-null workspace/session and the specified composite FK; it asserts target+new-revision uniqueness without watermark and the complete generic passport, class, verification, supersession, lifecycle, grant, and append-only trigger inventory by stable name;
3. direct FK tests reject cross-workspace and cross-session Result↔Attempt, manifest↔Attempt/Task/policy grant, entry/omission↔manifest/version, Evidence Link↔Claim/Observation/verifier Task, input-reference endpoints, supersession endpoints, lifecycle↔passport/Decision, policy-mutation↔passport/policy grant, and version↔passport/Task/Attempt/Contract; same-scope controls pass and negative exposure fixtures include a same-scope allowed row;
4. deferred class/reciprocity tests reject missing domain row, missing passport, wrong kind/scope, orphan delete, producer Attempt/Task mismatch, Result/Attempt projection mismatch, and every one-sided or ledger-only artifact-create/current-version/access/lifecycle/verification/supersession/eligibility/grant transition. Duplicate ledger rows for one target+new revision fail even with different watermarks. Each invalid fixture and valid paired control runs once with `SET CONSTRAINTS ALL IMMEDIATE` and once through normal deferred commit;
5. version immutability and current-version deferred FK reject update/delete, missing current version, invalid CAS, revision skips/no-op bumps, wrong `mutation_kind` or old/new fact columns, and lifecycle/current-version/access/eligibility/grant change without the exact reciprocal policy row. Supersession edge↔passport and lifecycle event↔passport tests check exact revision/watermark in both directions. Direct `UPDATE` or `DELETE` of lifecycle and policy-mutation ledgers fails through named append-only guards. Concurrent writers leave one current version, one row per target revision, and a monotonic watermark;
6. legacy backfill distinguishes `legacy_stored` and `migration_recomputed`, skips malformed Results, and records partial/unknown counts without false acceptance time. For every impossible-to-FK relationship in section 4.8, a broken fixture records the exact migration diagnostic and denies D admission while its same-scope valid control admits; repair plus rerun clears the restartable diagnostic;
7. per-kind legacy admission tests cover `registered+partial`, `registered+unknown-producer`, succeeded/failed/cancelled/obsolete Task lineage, succeeded/failed/cancelled/lost Attempt lineage, and denial for unknown/constraint-invalid domain status, rejected verification, unknown scope/content/access/hash, V6, evaluation-private inference, stale/withdrawn/superseded policy state, and post-D artifacts. A succeeded Task + succeeded Attempt + valid Result is the mandatory positive shadow control; terminal rows never authorize dispatch/resume/result reuse; every denial has an allowed control;
8. shadow equivalence fixture contains legacy source, graph nodes/edges, messages/match-decision references, product round, thought strategy, structured report references, stage eval, succeeded Task/Attempt, and every durable Run subfamily and section 4.8 relationship including durable Evidence Link. It compares ordered IDs/kinds, hashes, representation bytes, nesting parent/ordinal, omissions, and Prompt hash. The equal-time Evidence tie fixture pins the query-first canonical order. Removing any family/reference/Evidence Link, moving a link, or swapping two ordinals fails;
...
15. Chapter C `txOpDispatchIntentCreate` after-begin, before-commit, successful commit, and after-commit-unknown tests preserve the existing label/runner order and count Attempt passport/version, manifest passport/version, their `artifact_create 0→1` rows, entries/omissions/edges, outbox, Task transition, lifecycle event/mutation, and Event as one atomic unit. FK-order fixtures prove target creation precedes ledger insert; immediate and ordinary-commit modes reject ledger-first, ledger-only, target-only, duplicate target revision with another watermark, missing version, wrong kind/watermark, and non-`0→1` creation;
...
18. result-acceptance race pauses after preflight and separately mutates a referenced or upsert-target Run, Method Decision, Question, Task/dependency, Source/projection, Observation, Claim, Evidence Link, Report/report-claim, Evaluation/Decision, lifecycle, version/access, verification, supersession, and policy watermark. Deterministic lock discovery and version/revision/ledger CAS reject stale facts and commit none of the complete acceptance set;
...
22. Chapter C `txOpResultAccept` fault matrix preserves the existing label/runner order and has a failure point after each create or semantic change in the complete section 8 set: Run; Method Decision; each Question; each Task/dependency; Attempt; Result/projection; each Source/projection, Observation, Claim, Evidence Link, Report/report-claim, Evaluation/Decision and other Decision; lineage/lifecycle/supersession; Event; circuit/probe settlement; terminal transition. It proves all passports/versions/revisions/ledger rows and sidecars are all-or-none and converge after unknown commit. Immediate and ordinary-commit fixtures reject ledger-first, ledger-only, target-only, duplicate, missing passport/version, wrong kind/watermark, and non-`0→1` creation for Result and every output kind;
23. human, historical task Agent, D-enabled task Agent, unbound Fleet Agent, evaluation subject, grader, projector, and cross-workspace principal each receive exactly the classified section 13 surface, including stable Claim/Evidence Link nesting. Tenant-isolation tests prove denied cross-scope IDs, hashes, omission metadata, and grant existence are not exposed while a same-scope allowed artifact remains visible;
24. supersession/withdrawal increments passport eligibility and session watermark, requires exact reciprocal supersession edge or lifecycle event and policy ledger row, excludes new context, blocks affected in-flight acceptance, preserves authorized audit, and never deletes history;
...
- add passport, version, Result Artifact, context manifest/entry/omission, input reference, supersession, lifecycle, policy-state/grant/mutation, and backfill-diagnostic tables;
