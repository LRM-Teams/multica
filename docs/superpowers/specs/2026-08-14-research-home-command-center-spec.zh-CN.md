...
- 首页内容区域使用 Research Constellation 子页面同款局部深色主题，不随全局浅色主题退化成白底业务卡片；左侧应用导航继续使用全局浅色样式，不被调研页局部主题覆盖。
...
中心节点只显示阶段、单行省略标题和 task 进度；标题必须在圆形内部截断，完整标题通过可访问名称或悬停提示提供，不得换行溢出节点边界。
