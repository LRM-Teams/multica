...
export async function seedD5LargeConstellationSession(
  ...
  const stamp = Date.now();
  const email = `d5-large-${stamp}@multica.ai`;
  const api = new TestApiClient();
  await api.login(email, label);
  const workspace = await api.ensureWorkspace(label, `d5-large-${stamp}`);
  const slug = workspace.slug;
  const workspaceId = workspace.id;
  ...
  await db(
    `INSERT INTO agent_runtime (
       ...
       visibility, device_info, metadata, last_seen_at
               ...
               'public', $3, '{}'::jsonb, now())`,
    [workspaceId, `d5-large-runtime-${stamp}`, label],
  );
  ...
  const goalRow = await db(
    `INSERT INTO research_graph_node (
     ...
     RETURNING id`,
    [workspaceId, sessionId, nodeTitles.goal],
  );
  ...
  const insertClusterNode = async (title: string, clusterId: string, round: number) => {
    const row = await db(
      `INSERT INTO research_graph_node (
       ...
       RETURNING id`,
      [workspaceId, sessionId, title, round, clusterId],
    );
    const nodeId = row.rows[0].id as string;
    await db(
      `INSERT INTO research_graph_edge (workspace_id, session_id, from_node_id, to_node_id, edge_type)
       VALUES ($1, $2, $3, $4, 'leads_to')`,
      [workspaceId, sessionId, goalId, nodeId],
    );
    return nodeId;
  };
  ...
  const stableRow = await db(
    `INSERT INTO research_graph_node (
     ...
     RETURNING id`,
    [workspaceId, sessionId, nodeTitles.stable],
  );
  const probeRow = await db(
    `INSERT INTO research_graph_node (
     ...
     RETURNING id`,
    [workspaceId, sessionId, nodeTitles.probe],
  );
  await db(
    `INSERT INTO research_graph_edge (workspace_id, session_id, from_node_id, to_node_id, edge_type)
     VALUES ($1, $2, $3, $4, 'supports')`,
    [workspaceId, sessionId, stableRow.rows[0].id, probeRow.rows[0].id],
  );
