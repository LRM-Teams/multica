...
describe("contentMentionsViewer", () => {
  ...
  it("treats @all as addressing the viewer", () => {
    ...
    // Even without a viewer id, @all is a broadcast signal — callers that
    // only have content can still opt into a wash when they know the viewer
    // is a workspace member. We require viewerUserId only for member matches;
    // for @all we return true regardless so row wash works with partial auth.
