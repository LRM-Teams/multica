...
func runAgentGet(cmd *cobra.Command, args []string) error {
	...
	headers := []string{"ID", "NAME", "STATUS", "RUNTIME", "AVATAR_URL", "DESCRIPTION"}
	rows := [][]string{{
		strVal(agent, "id"),
		strVal(agent, "name"),
		strVal(agent, "status"),
...
func runAgentUpdate(cmd *cobra.Command, args []string) error {
	...
	if len(body) == 0 {
		return fmt.Errorf("no fields to update; use --name, --description, --instructions, --runtime-id, --runtime-config, --model, --custom-args, --mcp-config, --status, or --max-concurrent-tasks (env vars now live behind `multica agent env set <id>`)")
	}
