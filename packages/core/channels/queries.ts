...
export function upsertChannelMessageInCache(qc: QueryClient, message: ChannelMessage) {
  ...
  qc.setQueryData<InfiniteData<ChannelMessagesPage>>(channelKeys.messagesPage(message.channel_id), (old) => {
    ...
    // LRM-218 / LRM-202: WS payloads sometimes omit author_avatar_url. Preserve /
    // backfill from the cached row or a same-author sibling before writing so
    // bubbles don't regress to glyph placeholders.
...
/**
 * WS `channel:message` payloads sometimes omit `author_avatar_url` (a publish
 * path forgot to attach it) while list fetches include it. Prefer the incoming
 * URL, else keep the cached row's, else copy from another same-author bubble
 * already in the thread — so consecutive messages don't flicker to glyphs
 * (LRM-202 / LRM-218).
 */
