...
/**
 * A2A agent↔agent DM gate rows (#692). When a supervised agent-pair DM hits a
 * budget/frequency gate, or its owner pauses/resumes the pair or all workspace
 * A2A DMs, the BE writes a quiet `type=system` row carrying a `system_event`
 * part (`event ∈ agent_dm_paused_*|agent_dm_resumed`, params = the shared
 * {@link import("@multica/core/dm").AgentDMPauseEventParams} shape). The row is
 * owner-private — only the two agents + the owning supervisor ever receive it.
 * The FE composes its own localized copy from the structured params (matter,
 * rounds, agent display names) — same discipline as the reminder/thread rows,
 * never the BE's English fallback `content`.
 */
export const AGENT_DM_PAUSE_EVENTS = {
  budget: "agent_dm_paused_budget",
  frequency: "agent_dm_paused_frequency",
  pair: "agent_dm_paused_pair",
  global: "agent_dm_paused_global",
  resumed: "agent_dm_resumed",
} as const;

export type AgentDMPauseSystemEventKind =
  (typeof AGENT_DM_PAUSE_EVENTS)[keyof typeof AGENT_DM_PAUSE_EVENTS];

const AGENT_DM_PAUSE_EVENT_KINDS = new Set<string>(Object.values(AGENT_DM_PAUSE_EVENTS));

export interface AgentDMPauseSystemEvent {
  event: AgentDMPauseSystemEventKind;
  /** ≤120-rune source-matter summary (BE already applies its「当前事项」fallback);
   *  only the `budget` copy interpolates it. */
  matter: string;
  /** Rounds used / the post-grant round limit — only the `budget` copy shows them. */
  round: number;
  roundLimit: number;
  /** The two agents' display names — only the `frequency` copy interpolates them.
   *  Falls back to the canonical @handle when a display name is absent. */
  agentAName: string;
  agentBName: string;
}

/**
 * Extract an A2A pause/resume system event. Returns null for any other system
 * message so the caller falls back to the canonical `content`. All five events
 * always carry the full param shape from the BE, so the only discriminator is
 * the event kind; individual fields degrade defensively (name→handle, missing
 * number→0) so a sparse row still renders quiet copy instead of nothing.
 */
export function parseAgentDMPauseSystemEvent(
  message: SystemEventSource,
): AgentDMPauseSystemEvent | null {
  if (message.type !== "system" || !Array.isArray(message.parts)) return null;
  for (const part of message.parts) {
    if (part.type !== "system_event" || !AGENT_DM_PAUSE_EVENT_KINDS.has(part.event)) continue;
    const params = part.event_params;
    return {
      event: part.event as AgentDMPauseSystemEventKind,
      matter: optString(params, "matter") ?? "",
      round: optNumber(params, "round") ?? 0,
      roundLimit: optNumber(params, "round_limit") ?? 0,
      agentAName:
        optString(params, "agent_a_name") ?? optString(params, "agent_a_handle") ?? "",
      agentBName:
        optString(params, "agent_b_name") ?? optString(params, "agent_b_handle") ?? "",
    };
  }
  return null;
}
