...
describe("computeNewMessagesDivider", () => {
  it("returns null when the read cursor is unknown", () => {
    expect(computeNewMessagesDivider(msgs(1, 2, 3), null, null)).toBeNull();
  });

  it("returns null when everything loaded is already read", () => {
    expect(computeNewMessagesDivider(msgs(1, 2, 3), 3, null)).toBeNull();
  });

  it("anchors at the first message past the cursor and counts trailing messages", () => {
    expect(computeNewMessagesDivider(msgs(1, 2, 3, 4, 5), 2, null)).toEqual({
      anchorMessageId: "m3",
      count: 3,
    });
  });
  ...
  it("anchors at the top when the whole loaded window is unread", () => {
    expect(computeNewMessagesDivider(msgs(4, 5, 6), 3, null)).toEqual({
      anchorMessageId: "m4",
      count: 3,
    });
  });
  ...
  it("returns null for an empty list", () => {
    expect(computeNewMessagesDivider([], 0, null)).toBeNull();
  });
  ...
  it("excludes the viewer's own messages — a message you send is not 'new'", () => {
    ...
    expect(computeNewMessagesDivider(messages, 3, "u1")).toBeNull();
  });
  ...
  it("anchors at the first unread from someone else, counting only others'", () => {
    ...
    expect(computeNewMessagesDivider(messages, 1, "u1")).toEqual({
      anchorMessageId: "m3",
      count: 2,
    });
  });
  ...
  it("counts all unread when there is no viewer id to exclude", () => {
    ...
    expect(computeNewMessagesDivider(messages, 0, null)).toEqual({
      anchorMessageId: "m1",
      count: 2,
    });
  });
...
describe("useNewMessagesDivider", () => {
  ...
  it("does not raise a divider for the viewer's own just-sent message (the bug)", () => {
