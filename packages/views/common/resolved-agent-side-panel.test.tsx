...
const useQueryCalls = vi.hoisted(() => ({
...
vi.mock("@tanstack/react-query", () => ({
  useQuery: (opts: { queryKey: readonly unknown[]; enabled?: boolean }) => {
    ...
    if (key[2] === "agent") {
      useQueryCalls.detailEnabled.push(opts.enabled !== false);
      return detailState;
    }
...
describe("ResolvedAgentSidePanel (LRM-292)", () => {
  ...
  beforeEach(() => {
    ...
    useQueryCalls.detailEnabled.length = 0;
  ...
  it("always GETs by id (never uses ListAgents.find as an open gate)", () => {
    ...
    expect(useQueryCalls.detailEnabled).toContain(true);
    expect(screen.getByTestId("agent-side-panel")).toHaveAttribute("data-agent-id", "agent-1");
    expect(toastError).not.toHaveBeenCalled();
  });

  it("opens when GetAgent succeeds for a channel-only agent", () => {
  ...
  it("shows snapshot name while GetAgent is pending", () => {
