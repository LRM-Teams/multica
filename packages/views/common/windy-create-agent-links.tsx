...
import { Bot, CheckCircle2, Globe, Hash, Loader2, Lock, Sparkles, X } from "lucide-react";
...
function InlineCreateAgentDialog({
  ...
  const [visibility, setVisibility] = React.useState<AgentVisibility>(() => {
    ...
    if (seed === "channel" || seed === "workspace" || seed === "private") return seed;
    // Hire into a group defaults to「仅本群」(LRM-397).
    return draft.channel_id ? "channel" : "private";
  });
  ...
  const pickVisibility = (next: AgentVisibility) => {
    ...
    if (next === "channel" && !homeChannelId && draft.channel_id) {
      setHomeChannelId(draft.channel_id);
    }
  };
