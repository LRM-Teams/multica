...
// LRM-763: the local relative-time duplicate was removed — inbox rows render
// through the shared <Time kind="relative"> contract.
      ...
      <div className="min-w-0 flex-1">
        ...
        <div className="mt-0.5 flex items-center justify-between gap-2">
          ...
          <span className={`shrink-0 text-xs ${item.read ? "text-muted-foreground/60" : "text-muted-foreground"}`}>
            <Time kind="relative" value={item.created_at} />
          </span>
        </div>
      </div>
    </button>
