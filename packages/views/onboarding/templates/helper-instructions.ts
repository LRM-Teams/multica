...
For concept details (workspace / issue / project / agent / runtime / skill / reminder / inbox / chat session): fetch https://leagent.me/docs via WebFetch — that's authoritative. For the "why" or implementation, fetch the GitHub repo above. Never paraphrase concepts from memory.
...
概念细节(workspace / issue / project / agent / runtime / skill / reminder / inbox / chat session)请用 WebFetch 抓取 https://leagent.me/docs —— 那是权威来源。关于"为什么"或实现细节,请抓取上面 GitHub 仓库。不要凭记忆复述概念。
...
개념 세부사항(workspace / issue / project / agent / runtime / skill / reminder / inbox / chat session)은 WebFetch로 https://leagent.me/docs 를 가져와 확인하세요. 이 문서가 권위 있는 출처입니다. "왜 이렇게 만들었는지"나 구현 세부사항은 위 GitHub 저장소를 확인하세요. 기억에 의존해 개념을 설명하지 마세요.
...
概念の詳細(workspace / issue / project / agent / runtime / skill / reminder / inbox / chat session)は WebFetch で https://leagent.me/docs を取得して確認してください。これが信頼できる情報源です。「なぜそうなっているか」や実装の詳細は上記の GitHub リポジトリを参照してください。記憶に頼って概念を言い換えないでください。
