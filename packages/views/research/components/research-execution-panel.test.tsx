...
describe("ResearchExecutionPanel", () => {
  it("shows who is doing what across every typed status", () => {
    ...
    const statuses = new Set(
      screen
        .getAllByTestId("research-execution-row")
        .map(
          (row) => row.getAttribute("data-status") as ResearchExecutionStatus,
        ),
    );
    expect(statuses).toEqual(
      new Set(["queued", "running", "done", "failed", "stale", "idle"]),
    );
    for (const label of ["排队", "执行中", "完成", "失败", "停滞", "空闲"])
  ...
  it("expands long text and failure reason, then requests location", () => {
    ...
    render(
      <ResearchExecutionPanel
        agents={researchExecutionPanelFixture}
        onLocate={onLocate}
      />,
    );
    fireEvent.click(screen.getByRole("button", { name: "展开Owen的执行详情" }));
    expect(
      screen.getByText("原始 CSV 包含重复表头，解析在第 482 行停止。"),
    ).toBeTruthy();
    expect(
      screen.getByText("无法识别日期列格式。请清洗输入后重试。"),
    ).toBeTruthy();
  ...
  it("keeps text and shape semantics when running motion is reduced", () => {
    ...
    const runningRow = screen
      .getAllByTestId("research-execution-row")
      .find((row) => row.dataset.status === "running");
  ...
  it("guards compact and 360px layouts from long-text overflow", () => {
    const { container } = render(
      <ResearchExecutionPanel agents={researchExecutionPanelFixture} />,
    );
    expect(container.firstElementChild?.className).toContain("min-w-0");
    expect(
      container.querySelector(".grid-cols-\\[auto_minmax\\(0\\,1fr\\)_auto\\]"),
    ).toBeTruthy();
