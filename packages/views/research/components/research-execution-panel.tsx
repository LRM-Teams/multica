...
const STATUS_PRESENTATION: Record<ResearchExecutionStatus, StatusPresentation> =
  {
    queued: {
      ...
      markerClass:
        "border-dashed border-muted-foreground/55 text-muted-foreground",
    },
    running: {
      label: "执行中",
      Icon: Activity,
    ...
    idle: {
      ...
      Icon: Pause,
      badgeClass: "bg-muted text-muted-foreground",
      markerClass: "border-muted-foreground/35 text-muted-foreground",
    },
  };
...
function ExecutionRow({
  ...
  const hasDetail = Boolean(
    agent.actionDetail || agent.failureReason || agent.locationLabel,
  );
  return (
    <article
      data-testid="research-execution-row"
      data-status={agent.status}
      className="relative min-w-0 overflow-hidden border-b border-border/60 px-3 py-2.5 last:border-b-0"
    >
      ...
      <div className="grid min-w-0 grid-cols-[auto_minmax(0,1fr)_auto] items-start gap-x-2">
        ...
        <div className="min-w-0">
          ...
          <p
            className="mt-0.5 truncate text-xs leading-5 text-foreground"
            title={agent.action}
          >
            {agent.action}
          </p>
            ...
            {agent.locationLabel ? (
              <>
                ...
                <span className="truncate">
                  {LOCATABLE_PREFIX} {agent.locationLabel}
                </span>
              </>
            ) : null}
          </div>
        ...
        <div className="flex items-center gap-1">
          ...
          {hasDetail ? (
            <Button
              ...
              variant="ghost"
              size="icon"
              className="size-7 shrink-0 rounded-md focus-visible:ring-2"
              aria-label={`${expanded ? "收起" : "展开"}${agent.name}的执行详情`}
      ...
      {expanded ? (
        <div
          id={detailId}
          className="ml-10 mt-2 min-w-0 rounded-lg border border-border/70 bg-muted/35 p-2.5"
        >
          {agent.actionDetail ? (
          ...
          {agent.failureReason ? (
            <p className="mt-1.5 flex gap-1.5 text-xs leading-5 text-destructive-strong">
              <TriangleAlert
                ...
                aria-hidden="true"
              />
              <span className="break-words">{agent.failureReason}</span>
            </p>
          ) : null}
          {agent.locationLabel && onLocate ? (
            <Button
              type="button"
              variant="outline"
              size="sm"
            ...
            >
              ...
              <span className="truncate">
                {LOCATE_PREFIX} {agent.locationLabel}
              </span>
            </Button>
          ) : null}
        </div>
    ...
    >
      <header className="flex min-h-10 min-w-0 items-center justify-between gap-3 border-b border-border/70 px-3 py-2">
        ...
        <p className="shrink-0 text-[11px] tabular-nums text-muted-foreground">
          {runningCount > 0
            ? `${runningCount} 个智能体执行中`
            : "暂无智能体执行"}
        </p>
      </header>
      <div className="min-w-0">
        {agents.map((agent) => (
          <ExecutionRow key={agent.id} agent={agent} onLocate={onLocate} />
        ))}
      </div>
    </section>
  );
