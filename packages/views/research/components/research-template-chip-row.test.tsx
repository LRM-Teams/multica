...
vi.mock("../../i18n/use-t", () => ({
  useT: () => ({
    t: (picker: (keys: Record<string, unknown>) => unknown) => {
      const keys = { home: { templates_label: "模板" } };
...
/**
 * LRM-1189: the template chip row was the last research surface carrying a raw
 * hex (`shadow-[0_0_0_2px_#dbeafe]`) and had no dark variant for either the
 * selected or the hover tone. It must now match the LRM-1175 frozen "template"
 * blue triple used by `research-template-inject-tag.tsx`, keep light values
 * untouched, and drop the selection halo (selection reads from border + tint +
 * text; a halo would sit concentric with the focus-visible ring).
 */
describe("LRM-1189 research template chip row · tokens and dark parity", () => {
  it("has no raw hex and no arbitrary shadow anywhere in the source", () => {
    ...
    expect(SOURCE).not.toContain("#dbeafe");
  });
