...
export function GenericNode(props: {
  ...
  return (
    <div
      ...
      style={{ border: "1px dashed var(--destructive)", borderRadius: 8, padding: 8, maxWidth: 260 }}
    >
      ...
      <div style={{ color: "var(--muted-foreground)", fontSize: 12 }}>{viewModel.summary}</div>
...
export function V6NodeView(props: {
  ...
  return (
    <div
      ...
      style={{ border: "1px solid var(--border)", borderRadius: 8, padding: 8, maxWidth: 260 }}
    >
      <div style={{ fontWeight: 700 }}>
        {known.label}
        <span style={{ marginLeft: 6, fontFamily: "monospace", fontWeight: 400, fontSize: 12, color: "var(--muted-foreground)" }}>
          {known.group}
        </span>
      </div>
      <div>{known.title || known.nodeId}</div>
      <div style={{ color: "var(--muted-foreground)", fontSize: 12 }}>{known.summary}</div>
      <div style={{ fontSize: 12, color: meta ? "var(--muted-foreground)" : undefined }}>{known.status}</div>
    </div>
  );
}
