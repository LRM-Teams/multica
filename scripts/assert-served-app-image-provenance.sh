...
phase=${1:?usage: assert-served-app-image-provenance.sh <phase> <deploy-dir> <selfhost-compose> <host-compose> <backend-ref> <frontend-ref>}
...
backend_ref=${5:?missing backend image reference}
frontend_ref=${6:?missing frontend image reference}
