...
func TestBranchDispatchResponseShapeIsPinned(t *testing.T) {
	...
	const want = `{"channel_id":"ch-1","project_id":"proj-1","rollouts":[{"channel_id":"ch-1","leader_run_id":"run-1","agent_sandboxes":{"a-1":{"status":"ready","sandbox_instance_id":"inst-1","runtime_id":"rt-1"}},"env_id":"env-child-1","project_id":"proj-1","issue_id":"issue-1","chat_session_id":"cs-1","agent_run_id":"run-1","error":"boom","traceback":"trace","sandbox_refs":[{"instance_id":"inst-1","workspace_id":"ws-1","node_id":"node-1"}],"agent_sandbox_refs":{"a-1":{"instance_id":"inst-1","workspace_id":"ws-1","node_id":"node-1"}}}],"message":"dispatched"}`
	if string(got) != want {
