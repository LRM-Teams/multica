...
func newFakeBindingStore() *fakeBindingStore {
	return &fakeBindingStore{items: map[string]WorkspaceBinding{}}
}

func (f *fakeBindingStore) UpsertScoped(req BindingRequest, b WorkspaceBinding) error {
	f.mu.Lock()
	defer f.mu.Unlock()
...
func (f *fakeBindingStore) RevokeScoped(req BindingRequest) ([]string, error) {
	f.mu.Lock()
	defer f.mu.Unlock()
	delete(f.items, req.TargetWorkspaceID)
	return []string{"revoked-token-hash"}, nil
}
func (f *fakeBindingStore) ListScoped(computerID, userID string) ([]WorkspaceBinding, error) {
	f.mu.Lock()
	defer f.mu.Unlock()
	out := make([]WorkspaceBinding, 0, len(f.items))
	for _, b := range f.items {
		if b.ComputerID == computerID && (userID == "" || b.UserID == userID) {
			out = append(out, b)
		}
	}
	return out, nil
...
func acts() BindingRequest {
	return BindingRequest{ActorUserID: "u", TargetComputerID: "c", TargetWorkspaceID: "ws"}
}
...
func TestBindingServiceRevokeOnePreservesSiblings(t *testing.T) {
	...
	if _, err := svc.Revoke(BindingRequest{ActorUserID: "u", TargetComputerID: "c", TargetWorkspaceID: "ws-1"}, "ws-1"); err != nil {
...
func TestBindingServiceRevokeFailsClosed(t *testing.T) {
	...
	if _, err := svc.Revoke(BindingRequest{ActorUserID: "u", TargetComputerID: "c", TargetWorkspaceID: "missing"}, "missing"); err == nil {
		t.Fatal("revoking a non-existent binding must fail closed")
	}
	// No actor → fail closed.
	if _, err := svc.Revoke(BindingRequest{TargetComputerID: "c", TargetWorkspaceID: "ws"}, "ws"); err == nil {
