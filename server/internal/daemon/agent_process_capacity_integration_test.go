...
// These tests intentionally create different WorkspaceDaemons but one Daemon
...
func TestManagedCapacityMultiWorkspaceFIFOAndRunnerRemoval(t *testing.T) {
	...
	first, err := d.newWorkspaceSession("workspace-a")
	if err != nil {
		t.Fatal(err)
	}
	second, err := d.newWorkspaceSession("workspace-b")
	if err != nil {
		t.Fatal(err)
	}
	third, err := d.newWorkspaceSession("workspace-c")
	if err != nil {
		t.Fatal(err)
	}
...
func startManagedForCapacityTest(t *testing.T, runner *workspaceSession, agentID, runtimeID, dispatchID string) protocol.AgentStartAckPayload {
	t.Helper()
