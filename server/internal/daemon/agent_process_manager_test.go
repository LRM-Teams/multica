...
func TestAgentProcessManagerIdempotentStartQueueAndRecovery(t *testing.T) {
	...
	first, err := manager.Start(agentProcessStartRequest{AgentID: "agent-a", RuntimeID: "runtime-1", LaunchID: "dispatch-a"})
	if err != nil {
	...
	replayed, err := manager.Start(agentProcessStartRequest{AgentID: "agent-a", RuntimeID: "runtime-1", LaunchID: "dispatch-a"})
	if err != nil || replayed != first {
		t.Fatalf("replayed start = %+v, %v; want cached %+v", replayed, err, first)
	}

	queued, err := manager.Start(agentProcessStartRequest{AgentID: "agent-b", RuntimeID: "runtime-1", LaunchID: "dispatch-b"})
	if err != nil {
...
func TestAgentProcessManagerFencesStaleCallbacksAndCreatesNewLaunches(t *testing.T) {
	...
	second, err := manager.Restart(agentProcessCallback{AgentID: "agent-a", LaunchID: first.LaunchID}, agentProcessStartRequest{AgentID: "agent-a", RuntimeID: "runtime-1", LaunchID: "dispatch-b"})
	if err != nil {
...
func TestAgentProcessManagerRejectsRevokedCapacityGrant(t *testing.T) {
	...
	manager.newID = sequentialIDs()
	accepted, err := manager.Start(agentProcessStartRequest{AgentID: "agent-a", RuntimeID: "runtime-1", LaunchID: "dispatch-a"})
	if err != nil {
...
func TestAgentProcessManagerSharesCanonicalCapacityAcrossRunners(t *testing.T) {
	...
	firstAck, err := first.Start(agentProcessStartRequest{AgentID: "agent-a", RuntimeID: "runtime-a", LaunchID: "dispatch-a"})
	if err != nil || firstAck.QueueState != protocol.AgentStartQueueStarting {
		t.Fatalf("Start(first) = %+v, %v; want starting", firstAck, err)
	}
	secondAck, err := second.Start(agentProcessStartRequest{AgentID: "agent-b", RuntimeID: "runtime-b", LaunchID: "dispatch-b"})
...
func TestAgentProcessManagerCancelsGloballyQueuedLaunch(t *testing.T) {
	...
	firstAck, _ := first.Start(agentProcessStartRequest{AgentID: "agent-a", RuntimeID: "runtime-a", LaunchID: "dispatch-a"})
	secondAck, err := second.Start(agentProcessStartRequest{AgentID: "agent-b", RuntimeID: "runtime-b", LaunchID: "dispatch-b"})
	if err != nil || secondAck.QueueState != protocol.AgentStartQueueQueued {
	...
	thirdAck, err := third.Start(agentProcessStartRequest{AgentID: "agent-c", RuntimeID: "runtime-c", LaunchID: "dispatch-c"})
...
func TestManagedCapacityCountsResidentProcessAndEvictsOnlyIdle(t *testing.T) {
	...
	ack, err := manager.Start(agentProcessStartRequest{AgentID: "agent-b", RuntimeID: "runtime-b", LaunchID: "dispatch-b"})
...
func TestAgentProcessManagerRequiresStopBeforeRuntimeReassignment(t *testing.T) {
	...
	if _, err := manager.Start(agentProcessStartRequest{AgentID: "agent-a", RuntimeID: "runtime-2", LaunchID: "dispatch-b"}); err == nil {
		t.Fatal("cross-Runtime start bypassed explicit stop")
	}
	if err := manager.Stop(agentProcessCallback{AgentID: "agent-a", LaunchID: first.LaunchID}); err != nil {
		t.Fatalf("Stop(first): %v", err)
	}
	second, err := manager.Start(agentProcessStartRequest{AgentID: "agent-a", RuntimeID: "runtime-2", LaunchID: "dispatch-b"})
	if err != nil {
...
func TestAgentProcessManagerReadinessPoliciesAndActivationAreIndependent(t *testing.T) {
	manager := newAgentProcessManager("workspace-1", newCanonicalAgentRuntimePool().managedProcessAdmission(), time.Now, nil)
	manager.newID = sequentialIDs()
	accepted, err := manager.Start(agentProcessStartRequest{AgentID: "agent-a", RuntimeID: "runtime-1", LaunchID: "dispatch-a", ReadinessPolicy: agentRuntimeReadinessInitialTurn, DeliveryMode: agentInitialDeliveryStdin})
	if err != nil {
		t.Fatalf("Start: %v", err)
	}
