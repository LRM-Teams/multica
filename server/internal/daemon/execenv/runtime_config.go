...
func renderChatRuntimeBrief(b *strings.Builder, provider string, ctx TaskContextForEnv) {
	...
	b.WriteString("- Freshness holds: if `multica message send` returns `state`/`outcome` = `held` or text saying \"Message held by freshness check\", the platform saved your attempted message as a server draft because newer chat context arrived while you were composing. Review the returned `heldMessages` (and use `multica message read --target <target>` if you need more context). To update the draft, send the changed content normally: it replaces the saved draft and runs freshness checking again. To send it unchanged, use `multica message send --send-draft --target <target>`. You may also choose not to send anything. If a deliberate follow-up finds genuinely newer context, treat that `held` response as a new one-time decision, never a retry loop. Do not claim you replied until the follow-up send succeeds.\n")
