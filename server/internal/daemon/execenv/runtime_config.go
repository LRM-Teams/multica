...
func renderMemoryOperatingGuide(b *strings.Builder, ctx TaskContextForEnv) {
	b.WriteString("### Memory Operating Guide (v0.3)\n\n")
	...
	b.WriteString("- **User preferences and profile facts**: when `MULTICA_USER_MEMORY_DIR` is present, write the current member's durable preferences to its `USER.md`. Never put multiple members into one flat file or inspect another member directory. Project-limited preferences must also carry the current project ID in a governed candidate. If collective wording tells the addressed agents or team to remember a safe attributed preference, keep the user-scoped local entry and also add a high-confidence workspace/shared candidate for governed team curation; sharing the fact does not make the preference apply to other members.\n")
