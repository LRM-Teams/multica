...
func buildMetaSkillContent(provider string, ctx TaskContextForEnv) string {
	...
	if ctx.QuickCreatePrompt != "" {
		...
		if terminalAssignment {
			...
			b.WriteString("Before substantive execution, choose exactly one path. `DIRECT`: complete a tightly coupled deliverable that fits one bounded context yourself. `GRAPH`: when there are two or more independently deliverable units, useful parallelism, distinct roles or permissions, long external waits, or a real independent-verification boundary, write one JSON plan and create it atomically with `multica issue graph create --plan-file <path> --idempotency-key <uuid>`. `PROPOSE_GRAPH`: if decomposition would materially expand scope, cost, permissions, or runtime, explain the proposed split and obtain human approval before creating it. Task length alone is not a reason to split work; a long task with strongly shared context can remain DIRECT in one persistent Job. Do not create graph nodes for a greeting, one tool call, a small low-risk change, or a step that cannot be accepted independently. A planner that delegates bounded work must define each child's deliverable and completion boundary, and must not also implement work already delegated. The server is authoritative for dependency readiness, issue enqueue, permissions, budget and completion gates; never manually promote a graph-managed queued Issue or claim that Prompt prose satisfied a gate.\n\n")
...
func renderStandaloneChatRuntimeBrief(b *strings.Builder, provider string, ctx TaskContextForEnv) {
	...
	b.WriteString("- Chat reactions/history: use `multica message react --message-id <id> --emoji \"...\" [--remove]`; use `multica message read [--target ...] [--limit N] --output json`, `multica message search [query] [--target ...] [--sender user:<uuid>|agent:<uuid>] [--before RFC3339] [--after RFC3339] [--sort newest|oldest] [--limit N] [--offset N] --output json`, or `multica message resolve <message-id>` when more bounded chat context is needed. Search does not consume context coverage.\n")
