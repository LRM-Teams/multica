...
func buildMetaSkillContent(provider string, ctx TaskContextForEnv) string {
	...
	b.WriteString("- `multica issue complete <id> --summary \"...\" --evidence 'INDEX=KIND:REF' [--artifact KIND:REF] [--risk \"...\"]` — Agent-only typed completion; covers every criterion, creates the visible report comment, and enters `in_review`.\n")
	b.WriteString("- `multica issue review <id> --report <uuid> [--reject-criterion 'INDEX=REASON' --reason \"...\"]` — independent review; no rejection flags accepts every criterion.\n")
	b.WriteString("- `multica issue completion-reports <id>` — list immutable report/review history.\n")
	...
	if ctx.QuickCreatePrompt != "" {
		...
		if terminalAssignment {
			...
			b.WriteString("6. Before completion: only if this run produced a fact that clears the high bar (important AND likely to be re-read by future runs on this same issue, e.g. a new PR URL or deploy URL), or you noticed a metadata key from entry that is now stale, pin or clear it via `multica issue metadata set` / `multica issue metadata delete`. Most runs write nothing here — that is the expected outcome, not a gap. When in doubt, do not write. See the `## Issue Metadata` section above for the full bar.\n")
			fmt.Fprintf(&b, "7. **Submit the typed completion report — this is the mandatory final delivery step**: run `multica issue complete %s --summary \"...\" --evidence '0=test:<ref>' ...`. Evidence uses zero-based `INDEX=KIND:REF`; repeat it as needed and cover every acceptance criterion. Add `--artifact KIND:REF` for PR/deploy artifacts and `--risk \"...\"` for known follow-ups. The command creates the visible Issue comment and atomically moves the Issue to `in_review`; do not also run `issue comment add` or `issue status ... in_review`. Text in your terminal or final assistant output is NOT delivered.\n", ctx.IssueID)
			b.WriteString("8. Never self-review the report. An independent Agent with a task-scoped Issue wake or a human uses `multica issue review`; accepted moves to `done`, rejected creates the successor Run.\n")
	...
	case isAssignmentTriggered:
		b.WriteString("⚠️ **Final results MUST be delivered via `multica issue complete`.** The typed report creates the visible Issue comment and enters `in_review`; terminal output, assistant chat text, and run logs are not user-visible delivery. Cover every acceptance criterion with evidence and never self-review.\n")
		b.WriteString(compactCloseoutStatusInstruction)
		b.WriteString("\n")
		b.WriteString("When referencing an issue in completion evidence, use the visible issue key/title or URL from the CLI output; do not invent raw mention links.\n")
	default:
