...
func renderChatRuntimeBrief(b *strings.Builder, provider string, ctx TaskContextForEnv) {
	...
	b.WriteString("- Freshness holds: if `multica message send` returns `state`/`outcome` = `held` (CLI also exits non-zero) or text saying \"Message held by freshness check\", the platform already saved your attempted message as a server draft because newer chat context arrived while you were composing. This is not delivery. In the **same turn**, review the returned `heldMessages` (use `multica message read` only if you need more context), then choose exactly one path before ending the run: rerun `multica message send --send-draft --target <target>` to send the saved draft unchanged, or run a normal `multica message send --target <target> ...` with revised content (overwrites the saved draft). Do not end the turn after only a held send, and do not claim you replied until a follow-up send exits 0 with `message.id`.\n")
