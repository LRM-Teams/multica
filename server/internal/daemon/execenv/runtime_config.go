...
func renderMemoryOperatingGuide(b *strings.Builder, ctx TaskContextForEnv) {
	b.WriteString("### Memory Operating Guide (v0.8)\n\n")
	...
	b.WriteString("- **Optional memory signal (not a second write path)**: after a durable preference/feedback write, you may append one JSON line to `MULTICA_AGENT_SYNC_QUEUE_DIR/memory-signal.jsonl` such as `{\"action\":\"write\",\"kind\":\"feedback\",\"scope\":\"user\",\"topic\":\"progress_feedback\",\"summary\":\"长任务开始前确认并持续反馈进度\"}`. Use a short stable `topic` so later self-review can dedupe. If nothing durable should be remembered, omit the file or write `{\"action\":\"none\"}`. The platform uses this for missed-write detection only; the Markdown file write remains authoritative.\n")
