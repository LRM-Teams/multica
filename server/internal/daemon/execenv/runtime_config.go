...
func renderMemoryOperatingGuide(b *strings.Builder, ctx TaskContextForEnv) {
	b.WriteString("### Memory Operating Guide (v0.6)\n\n")
	...
	b.WriteString("- **Durable facts and decisions**: write project-specific knowledge to `MULTICA_PROJECT_MEMORY_DIR/MEMORY.md` or `DECISIONS.md`. Use agent-global `memory/MEMORY.md` only for knowledge that genuinely applies across projects.\n")
	...
	b.WriteString("- **Daily journal (hot path) vs self-review/curator (cold path)**: keep live-task memory cheap so short turns stay fast. On substantive closeout (code change, decision, issue progress, non-trivial investigation), append a few lines to `memory/daily/YYYY-MM-DD.md` — what happened, temporary conclusions, process notes. Do **not** run a full self-review or curator-style promotion inside every chat/task turn; that adds latency and feels like stuttering on greetings and one-line answers. Skip Daily for pure greetings, stickers, thanks, and throwaway tests. Preferences and handoffs still write immediately to `USER.md` / `RELATIONSHIP.md` (and project/channel files when scoped there) — never leave durable facts only in Daily. Background L1 daily recording, L2 agent self-review, and team Curator (scheduled, active agents only) backfill Daily from evidence and promote/dedupe into formal files; they own the heavy path and must not block chat latency.\n")
