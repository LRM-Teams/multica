...
func renderStandaloneChatRuntimeBrief(b *strings.Builder, provider string, ctx TaskContextForEnv) {
	...
	b.WriteString("- Chat reactions/history: use `multica message react --message-id <id> --emoji \"...\"`; use `multica message read [--target ...] [--limit N] --output json`, `multica message search \"query\" [--target ...] --output json`, or `multica message resolve <message-id>` when more bounded chat context is needed.\n")
