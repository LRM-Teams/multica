...
func renderMemoryOperatingGuide(b *strings.Builder, ctx TaskContextForEnv) {
	b.WriteString("### Memory Operating Guide (v0.4)\n\n")
...
func renderChatRuntimeBrief(b *strings.Builder, provider string, ctx TaskContextForEnv) {
	...
	b.WriteString("- Freshness holds: if `multica message send` returns `state`/`outcome` = `held` or text saying \"Message held by freshness check\", the platform saved your attempted message as a server draft because newer chat context arrived while you were composing. Review the returned `heldMessages` (and use `multica message read --target <target>` if you need more context), then make exactly one explicit decision: `multica message send --send-draft --target <target>` to send unchanged, `multica message send --revise-draft --target <target> ...` to replace the pending draft, or `multica message send --discard-draft --target <target>` to abandon it. A later ordinary send returns `draft_pending`; it does not overwrite or retry the draft. If a deliberate `--send-draft` finds still-new context, treat the new `held` response as a new one-time decision, never a retry loop. Do not claim you replied until the follow-up send succeeds.\n")
