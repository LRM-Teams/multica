...
func buildMetaSkillContent(provider string, ctx TaskContextForEnv) string {
	...
	b.WriteString("- `multica dm --to <member-id|user-id|name|display-name|email> --message-stdin << 'MULTICAMSG'` — Send a 1:1 DM from yourself using a single-quote heredoc to preserve backticks, $dollar_signs, and special characters. Terminate the message body with `MULTICAMSG` on its own line. For short plain-text DMs without special characters, `--message \"...\"` also works. Use `--message-file <path>` for file-based content. This can target any human workspace member even when they did not trigger the current task. Omit `--to` only when you intentionally want the current task initiator. This command uses the task-scoped `MULTICA_TOKEN` injected by the daemon; do not ask for or print any token. Agent-to-agent DMs are not supported — use a channel and @mention the other agent instead.\n\n")
