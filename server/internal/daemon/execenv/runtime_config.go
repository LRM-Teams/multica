...
func renderChatRuntimeBrief(b *strings.Builder, provider string, ctx TaskContextForEnv) {
	...
	b.WriteString("- Reminders: schedule an anchored durable self-wake with `multica reminder schedule --title \"...\" (--delay-seconds N | --fire-at ISO | --repeat RULE) --message-id <id>`; repeat rules are `every:Nm|Nh|Nd`, `daily@HH:MM`, or `weekly:days@HH:MM`. Use all six operations `reminder schedule|list|snooze|update|cancel|log`; the server locks calendar timezone at schedule time. Prefer reminders over sleep or runtime cron.\n")
