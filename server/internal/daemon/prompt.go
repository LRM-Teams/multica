...
func managerRoleStateSlot(task Task) string {
	...
	b.WriteString("Adaptive Goal Mode: align with the channel empty-state — when a human states a channel-level overall goal/outcome and none exists, create it (`multica goal get --channel <id>`, then `multica goal create --channel <id> --title \"...\" --objective \"...\" --criterion \"...\"`). Use only this wake's server-claimed channel id. Never substitute a Cursor/tool goal, a verbal summary, or a goal on another channel. Skip greetings, pure questions, and trivial one-step asks. Explicit create triggers include asking you to set/start goal mode, naming a 总目标/overall goal, \"this is your goal\", directing `multica goal create`, or directing you to own a multi-step delivery outcome. If success criteria are thin, still create with concrete criteria inferred from the stated outcome; propose in-channel only when the desired outcome itself is unclear. Never claim the Goal is set until create succeeds; report the returned Goal id in-channel. On failure report the error instead of affirming in chat. You own decomposition, coordination, revision, and evidence-based completion; executors only checkpoint progress.\n")
	b.WriteString("When none of these channels has a current Goal, do not auto-schedule a Reminder. While a current Goal is active or paused, keep exactly one self-owned follow-up Reminder per that channel. List first; schedule only if absent with `multica reminder schedule --title \"Goal follow-up: <goal-id>\" --delay-seconds 900 --msg-id <current-source-message-id>`. On later wakes, snooze that same Reminder with exactly one of `--delay-seconds 900|1800|2700|3600` (15/30/45/60 minutes): 900 if blocked or a next step is overdue; 1800 if work is moving; 2700 or 3600 if waiting on healthy review/human. Cancel it when the Goal becomes completed or cancelled. Do not create extra Reminders or use free-form intervals. Before acting on any Reminder, re-check this current role list.\n\n")
	return b.String()
}
...
func channelGoalStateSlot(task Task) string {
	...
	if c := goal.Coordination; c != nil {
		...
		if c.AgentMemberCount > 1 || c.ExecutionAdmission == "unavailable" {
			...
			if isManager && c.OpenProjectIssueTotal > 0 {
				b.WriteString("On each Goal follow-up Reminder wake, inspect channel-linked Project Issues, unblock or reassign stale work, dispatch newly-ready parallel Issues, drive in_review to independent review, and checkpoint the Goal.\n")
			}
	...
	if isManager {
		fmt.Fprintf(&b, "Goal follow-up Reminder: keep exactly one self-owned Reminder for this channel while the Goal is active or paused. List first; if absent, `multica reminder schedule --title \"Goal follow-up: %s\" --delay-seconds 900 --msg-id <current-source-message-id>`. On each wake snooze with exactly one of 900, 1800, 2700, or 3600 seconds (15/30/45/60 minutes) — 900 if blocked or idle-overdue, longer if work is healthy — then checkpoint. Cancel the Reminder when the Goal becomes terminal.\n", goal.ID)
