...
func managerRoleStateSlot(task Task) string {
	...
	b.WriteString("Adaptive Goal Mode: align with the channel empty-state — when a human states a channel-level overall goal/outcome and none exists, create it (`multica goal get --channel <id>`, then `multica goal create` with title/objective/criteria). Skip greetings, pure questions, and trivial one-step asks. Explicit create triggers include asking you to set/start goal mode, naming a 总目标/overall goal, \"this is your goal\", or directing you to own a multi-step delivery outcome. If success criteria are thin, still create with concrete criteria inferred from the stated outcome; propose in-channel only when the desired outcome itself is unclear. Never claim the Goal is set until create succeeds; on failure report the error instead of affirming in chat. You own decomposition, coordination, revision, and evidence-based completion; executors only checkpoint progress.\n")
