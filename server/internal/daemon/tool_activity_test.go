...
func TestToolActivityFact(t *testing.T) {
	cases := []struct {
		...
		},
		{
			...
			wantDetail:  "tool_started",
			wantSummary: "Using cursor-agent",
		},
		{
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			detail, summary := toolActivityFact(tc.tool, tc.input)
			if detail != tc.wantDetail || summary != tc.wantSummary {
				t.Fatalf("toolActivityFact(%q, %v) = (%q, %q), want (%q, %q)",
					tc.tool, tc.input, detail, summary, tc.wantDetail, tc.wantSummary)
			}
		})
	}
}
...
func TestToolActivityFactBoundsTheCommand(t *testing.T) {
	_, summary := toolActivityFact("bash", map[string]any{"command": strings.Repeat("x", maxActivityCommandRunes+100)})
	if got := len([]rune(summary)); got > maxActivityCommandRunes+1 {
		t.Fatalf("command not bounded at the source: %d runes", got)
	}
	if !strings.HasSuffix(summary, "…") {
		t.Fatalf("truncated command should carry an ellipsis: %q", summary)
	}
}
