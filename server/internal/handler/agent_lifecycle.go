...
func (h *Handler) CreateAgentLifecycleOperation(w http.ResponseWriter, r *http.Request) {
	...
	insertAgentActivityEvent(
		r.Context(), tx, lockedAgent.WorkspaceID, lockedAgent.ID, runtime.ID, pgtype.UUID{},
		activityKindSystem, "agent_lifecycle_"+status, "info",
		"agent", lockedAgent.ID, "",
		"", "Agent lifecycle operation "+status,
		map[string]any{
			"operation_id":  op.ID,
			"actor_user_id": requestUserID(r),
			"action_kind":   string(req.ActionKind),
			"status":        status,
		},
	)
