...
func TestListWorkspaceAgentTaskSnapshotIncludesActorIdentity(t *testing.T) {
	...
	insertTask := func(agentID, status string) string {
		var id string
		if err := testPool.QueryRow(ctx, `
			INSERT INTO agent_task_queue (agent_id, runtime_id, status, priority, context)
			VALUES ($1, $2, $3, 0, $4::jsonb)
			RETURNING id`, agentID, handlerTestRuntimeID(t), status, quickCreateContext).Scan(&id); err != nil {
			t.Fatalf("insert snapshot identity task: %v", err)
		}
		return id
	}
	visibleTaskID := insertTask(visibleAgentID, "queued")
	privateTaskID := insertTask(privateAgentID, "running")
	t.Cleanup(func() {
		testPool.Exec(ctx, `DELETE FROM agent_task_queue WHERE id IN ($1, $2)`, visibleTaskID, privateTaskID)
	})
