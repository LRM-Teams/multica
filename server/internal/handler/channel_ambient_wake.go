...
func (h *Handler) buildChannelAmbientUnreadPromptWithDB(ctx context.Context, exec db.DBTX, ch ChannelResponse, agent db.Agent, trigger ChannelMessageResponse, cursorSeq, pendingToSeq int64) string {
	...
	b.WriteString("If a lightweight acknowledgement is enough outside an all-hands welcome/greeting request, prefer a message-scoped reaction action instead of a text reply: return exactly one JSON object like {\"action\":\"react\",\"reaction\":{\"message_id\":\"CURRENT_MESSAGE\",\"emoji\":\"👍\"}} and nothing else.\n")
