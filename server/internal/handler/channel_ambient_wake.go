...
func (h *Handler) buildChannelAmbientUnreadPromptWithDB(ctx context.Context, exec db.DBTX, ch ChannelResponse, agent db.Agent, trigger ChannelMessageResponse, cursorSeq, pendingToSeq int64) string {
	...
	b.WriteString("If the bundle explicitly addresses everyone/all members/all agents (for example 全体, 大家, everyone, all agents) and asks for a welcome, greeting, reaction, or response, treat it as relevant to you and write one short visible plain-text message. Do not stay silent, do not print no_reply, and do not use a reaction-only command/result for that case.\n")
