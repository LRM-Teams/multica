...
func (h *Handler) buildChannelAmbientUnreadPromptWithDB(ctx context.Context, exec db.DBTX, ch ChannelResponse, agent db.Agent, trigger ChannelMessageResponse, cursorSeq, pendingToSeq int64) string {
	...
	b.WriteString("If the bundle explicitly addresses everyone/all members/all agents (for example 全体, 大家, everyone, all agents) and asks for a welcome, greeting, reaction, or response, treat it as relevant to you and run `multica send` with one short visible message. Do not stay silent or print no_reply for that case.\n")
