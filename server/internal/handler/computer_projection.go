...
func computerConnectionProjection(daemonID, ownerID string, lastSeen pgtype.Timestamptz, runnerConnected, workJournalEnabled bool) computerConnectionResponse {
	var seen *string
	if lastSeen.Valid {
...
SELECT b.daemon_id, b.user_id::text, h.last_seen_at, COALESCE(o.work_journal_enabled, FALSE)
...
ORDER BY b.created_at, b.daemon_id`, parseUUID(workspaceID))
	...
	for rows.Next() {
		...
		if err := rows.Scan(&daemonID, &ownerID, &lastSeen, &workJournalEnabled); err != nil {
		...
		result = append(result, computerConnectionProjection(daemonID, ownerID, lastSeen, connected, workJournalEnabled))
	}
