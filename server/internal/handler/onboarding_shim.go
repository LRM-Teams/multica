...
func (h *Handler) BootstrapOnboardingRuntime(w http.ResponseWriter, r *http.Request) {
	...
	if assistantCreated {
		...
		h.publishAgentVisibilityEvent(protocol.EventAgentCreated, req.WorkspaceID, "member", userID, assistant, map[string]any{"agent": resp})
...
func zhNoRuntimeIssueDescription() string {
	return strings.Join([]string{
		...
		"   multica daemon restart",
		"   如果你用桌面端，重启 app 即可。",
		"6. 回到 Runtimes 页面刷新。你应该能看到一个在线的 Codex 运行时。",
		"7. 用这个运行时创建第一个智能体，再把一个 issue 分配给它，并把状态切到 todo。",
		"",
		"Codex 参考文档：https://developers.openai.com/codex/cli",
		"",
