...
func buildResearchAdaptivePlan(goal string) researchAdaptivePlan {
	...
	plan.Dimensions = researchDimensionSeedsForDomain(fine, delivery)
	return plan
}
...
func researchDimensionSeedsForDomain(fine string, deliveryLike bool) []researchDimensionSeed {
	...
	case researchDomainGame:
		return []researchDimensionSeed{
			...
			{Family: "human_ai_boundary", Title: "人机制作边界", Summary: "AI 上限、缺人做不了的环节、人做 vs AI 做", SourceHints: []string{"x", "community"}, Required: deliveryLike},
	...
	case researchDomainAIEngineering:
		return []researchDimensionSeed{
			...
			{Family: "human_ai_boundary", Title: "人在回路与安全红线", Summary: "标注/评测/产品决策哪些必须有人", SourceHints: []string{"blog", "x"}, Required: deliveryLike},
	...
	case researchDomainAcademicPapers:
		return []researchDimensionSeed{
			...
			{Family: "human_ai_boundary", Title: "自动化检索 vs 研究判断", Summary: "文献检索可高度自动；问题 intra/伦理偏人", SourceHints: []string{"lab_blog"}, Required: false},
	...
	case researchDomainFinance:
		return []researchDimensionSeed{
			...
			{Family: "human_ai_boundary", Title: "签字与适当性回路", Summary: "缺人则做不下去的合规环节", SourceHints: []string{"regulator", "ir"}, Required: deliveryLike},
	...
	case researchDomainDesignVisual:
		return []researchDimensionSeed{
			...
			{Family: "human_ai_boundary", Title: "人机创作边界", Summary: "品牌终审偏人；批量变体可 AI", SourceHints: []string{"x", "blog"}, Required: deliveryLike},
	...
	default:
		return []researchDimensionSeed{
			...
			{Family: "human_ai_boundary", Title: "人机边界", Summary: "AI 上限 / 必须有人 / 人 vs AI", SourceHints: []string{"x", "blog"}, Required: deliveryLike},
...
func adaptiveKickoffLeadPrompt(goal string, plan researchAdaptivePlan) string {
	...
	b.WriteString("New research session ready on the exploration canvas.\n")
	b.WriteString(fmt.Sprintf("Goal: %s\n", goal))
	b.WriteString(fmt.Sprintf("Detected fine domain: %s (coarse seeds: %s)\n", plan.FineDomain, strings.Join(plan.CoarseDomains, ",")))
	b.WriteString("Soft probe budget (stage gate, NOT product Round N): ≤5 probe rounds / ≤15 minutes before stage gate; on budget ceiling deliver partial conclusions + uncovered checklist.\n")
	...
	b.WriteString("- Seeded dimension families:\n")
	...
	b.WriteString("\n\nRefine the dimension tree, dispatch probes via multica research graph-append / message --target / source-upsert (with --why), and keep the user updated through 罗纳尔多 voice only.\n")
	return b.String()
}
