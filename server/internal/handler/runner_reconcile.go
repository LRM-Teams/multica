...
func reduceRunnerLaunches(desired []runnerDesiredLaunch, observed []runnerObservedLaunch) []runnerReconcileAction {
	...
	for _, agentID := range ordered {
		...
		if mismatched {
			actions = append(actions, runnerReconcileAction{eventType: protocol.EventDaemonAgentStop, payload: protocol.AgentStopPayload{AgentID: have.agentID, LaunchID: have.launchID}})
		}
		...
		if wanted && !mismatched && !running {
			actions = append(actions, runnerReconcileAction{eventType: protocol.EventDaemonAgentStart, payload: protocol.AgentStartPayload{AgentID: want.agentID, RuntimeID: want.runtimeID, LaunchID: want.launchID, StartDispatchID: want.startDispatchID, Config: protocol.AgentStartConfig{SessionID: want.sessionID}}})
		}
	}
	if len(actions) == 0 {
		return nil
