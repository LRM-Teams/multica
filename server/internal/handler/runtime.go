...
func (h *Handler) ArchiveAgentsAndDeleteRuntime(w http.ResponseWriter, r *http.Request) {
	...
	for _, a := range archivedAgents {
		...
		if err := h.terminalizeAndRemoveReminderOwner(r.Context(), uuidToString(rt.ID), uuidToString(a.ID), "agent_runtime_removed"); err != nil {
			slog.Warn("terminalize reminders on runtime removal failed", "error", err, "agent_id", uuidToString(a.ID), "runtime_id", uuidToString(rt.ID))
		}
	}
	h.publish(protocol.EventDaemonRegister, wsID, "member", userID, map[string]any{
		"action": "delete",
