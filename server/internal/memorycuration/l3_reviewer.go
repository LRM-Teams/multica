...
func (r *AgentStageRunner) RunStage(ctx context.Context, input StageAgentInput) (StageAgentOutput, error) {
	...
	payload["workflow"] = "agent_self_review writes the target agent's daily/review/proposals from today's evidence; team_curation consumes proposed memories/skills from active agents and emits clean team knowledge, dedupe, merge, and conflict decisions. Preserve explicit project/channel/task/expiry applicability; never infer stable IDs that are absent from evidence. Treat the speaker as provenance, not automatically as the applicability subject. Clear collective directives such as 'you all remember this', 'everyone note this', Chinese '都给我记住' or '你们记一下', and equivalent wording request workspace/team memory even without the exact phrase 'all agents'; do not widen secrets or clearly member-only behavior."
