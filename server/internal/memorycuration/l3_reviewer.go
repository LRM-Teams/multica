...
func stageAgentContract(stage Stage) string {
	...
	case StageAgentSelfReview:
		return "Run one background self-review for the target agent. Read the target agent root and DB evidence, then update memory/daily/YYYY-MM-DD.md first. Put uncertain or promotable items in memory/REVIEW.md and/or sync_queue proposal files. Output strict JSON {\"summary\":\"...\",\"candidates\":[{\"type\":\"memory|user_preference|state|skill|team_memory|follow_up\",\"scope\":\"agent|user|workspace|team\",\"title\":\"...\",\"content\":\"...\",\"confidence\":0.0,\"evidence_refs\":[\"kind:id\"],\"applies\":{\"project_ids\":[],\"channel_ids\":[],\"task_types\":[],\"expires_at\":\"RFC3339\"}}]}. Include applies only when exact stable IDs or expiry are supported by evidence; never guess IDs. A speaker's identity is provenance, not user scope. Collective remember directives apply to the agents addressed by the source message and do not by themselves justify a workspace/team candidate; each recipient may keep its own agent-local memory. Use workspace/team scope only with evidence of a broader audience or canonical workspace/team knowledge. Personal preferences remain attributed and must not govern other members. Be selective; no chat logs."
