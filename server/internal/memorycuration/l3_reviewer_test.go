...
func TestStageAgentContractsRequireConciseDailyAndMemory(t *testing.T) {
	...
	for _, want := range []string{"terse event index", "at most 240 characters", "one stable fact or rule per bullet", "at most 180 characters", "never copy steps, logs, file lists, or chat", "compaction_flush", "memory-search-hits.jsonl"} {
