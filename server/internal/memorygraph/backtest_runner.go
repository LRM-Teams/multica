...
// defaults.
func NewExploreBacktestRunner(store *Store, emb *CachedEmbedder, backend AgentBackend, retrCfg RetrievalConfig, expCfg ExploreConfig, provider string) *ExploreBacktestRunner {
	if retrCfg.TopK <= 0 {
		retrCfg = DefaultRetrievalConfig()
	}
	return &ExploreBacktestRunner{
		store:    store,
		emb:      emb,
		backend:  backend,
		retrCfg:  retrCfg,
