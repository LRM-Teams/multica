...
// caught it. Fixing these 33 is explicitly OUT OF SCOPE for task #97 itself
...
// fixed (add the remap UPDATE, then delete the entry here) — never delete
// an entry just because the list "looks long"; TestAllMigrations_* will
// catch that mismatch immediately (the real down.sql would still trip the
// checker with the entry gone).
var knownPreExistingUnsafeNarrowings = map[knownUnsafeNarrowingKey]bool{
	...
	{"107_comment_system_author", "comment_author_type_check"}:                                    true,
	{"111_issue_origin_lark_chat", "issue_origin_type_check"}:                                     true,
	{"141_sandbox_resume", "sandbox_job_type_check"}:                                              true,
	{"143_sandbox_reconfigure", "sandbox_job_type_check"}:                                         true,
	...
	{"181_sandbox_create_template", "sandbox_job_type_check"}:                                     true,
	{"182_sandbox_snapshot", "sandbox_job_type_check"}:                                            true,
	{"186_env_dispatch_message_channels", "sandbox_job_type_check"}:                               true,
	...
	{"207_channel_agent_onboarding", "agent_inbox_event_reason_check"}:                            true,
	...
	{"254_dm_peer_state_channel_key", "dm_peer_state_peer_type_check"}:                            true,
