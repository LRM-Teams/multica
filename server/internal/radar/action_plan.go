...
func BuildIdleNudgeChannelPrompt(markdown string) string {
	...
	b.WriteString("- For each open issue that should be moving, emit a mention_agent to its owner: tell them to do the work now and push the issue to in_review/done, updating its status — not just reply. Reference the issue by #number.\n")
