...
同一一级研究方向累计节点数超过 5 个后进入方向饱和门禁。Director Brief 会在该方向的 Branch scope 中提供 direction_node_count、saturation_threshold=5 和 saturation_gate，并在当前 Frontier 节点摘要中提供最高层节点结论。此后每次创建该方向的 atomic Work 前，必须把当前 Goal、该方向当前最高节点结论和本次小问题进行价值比较，并在 Work payload 中填写 direction_gate_node_count（必须等于 Brief 当前值）、direction_gate_decision=continue_direction 和 direction_gate_rationale。理由必须说明这个小问题如何改变用户 Goal 的判断；不能为了补细节、重复来源或追求局部确定性继续深挖。若信息已经足够，停止该方向，不创建 Work；若只是该方向覆盖不足，优先创建新的一级研究方向而不是继续堆积本方向。
当所有方向的最高节点已经足以回答用户 Goal、没有高价值待回答问题且没有必须解决的冲突时，停止全部 Research Work，创建或刷新报告 Work 交给“报告老板”，由报告老板整合所有一级方向的最高节点生成最终报告；不要为了等待更多小问题而继续调研。只有发现 Goal 的重要维度尚未覆盖且仍有容量时，才创建新的一级方向。
...
晋级规则固定为：同一个一级研究方向必须先积累至少 3 个 fresh S，才能在该方向内 promotion 为 M；禁止把不同一级方向各自唯一的 S 直接合成 M。至少两个 fresh M promotion 为 L，至少两个 fresh L promotion 为 XL，至少两个 fresh XL promotion 为 XXL；一个高层节点吸收相关低层输入时保持原 tier；两个 XXL 融合仍为 XXL。形成 M/L/XL 后，如果仍有目标维度未覆盖且容量允许，应在后续周期并行扩展新的一级方向，不得因一次融合就停止全部探索。出现任意 XXL 后，必须重新比较所有一级方向的最高节点与用户 Goal：若已经足够回答 Goal，立即停止 Research Work、创建或刷新报告并进入结束流程；只有仍存在重要未覆盖维度时才继续派发。发布报告前，所有 material unabsorbed content 必须已吸收、明确排除、终止或列为未解决缺口。
