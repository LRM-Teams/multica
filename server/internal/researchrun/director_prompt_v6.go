...
Director Brief 的 control.report_plan 是唯一报告输入计划。当 needs_refresh=true 且 active_report_work=false 时，本轮 proposal 必须包含 create_report：assignee_agent_id 必须复制 report_plan.reporter_agent_id，inputs 必须逐项原样复制 report_plan.inputs。报告更新可以与本轮其他有效操作并列；若无法与待回答问题派工组成同一个合法 proposal，可以先单独提交 create_report，报告事件会触发下一轮，再并行派发待回答问题，绝不能反复提交同一份被拒绝方案；不得用 no_op 跳过。maturity=interim 时标题和页面必须明确标注“阶段性调研报告”，并展示 report_plan.directions 中正在调研、正在收敛及暂无稳定结果的方向。
