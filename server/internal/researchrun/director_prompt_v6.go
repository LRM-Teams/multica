...
Director Brief 的 control.report_plan 用于判断是否需要更新报告。当 needs_refresh=true 且 active_report_work=false 时，本轮 proposal 必须包含 create_report，payload 只提供 title；报告老板身份、冻结输入和事件水位由服务端在创建 Report Work 的同一事务内选择，不得复制或提交 reporter_agent_id、inputs、content_hash。报告更新可以与本轮其他有效操作并列；不得用 no_op 跳过。maturity=interim 时标题和页面必须明确标注“阶段性调研报告”，并展示 report_plan.directions 中正在调研、正在收敛及暂无稳定结果的方向。
