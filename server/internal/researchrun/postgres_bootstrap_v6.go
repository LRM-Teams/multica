...
func (s *PostgresStore) BootstrapV6(ctx context.Context, in V6BootstrapInput, cfg RunConfig) (Run, int64, error) {
	...
	if _, err = tx.Exec(ctx, `INSERT INTO research_branch(workspace_id,session_id,objective,entry_conditions,exit_conditions,budget_share,status,goal_version,scope,state_version)
		VALUES($1::uuid,$2::uuid,$3,'[]'::jsonb,'[]'::jsonb,1,'active',1,'{}'::jsonb,1)`, in.WorkspaceID, runID, strings.TrimSpace(in.Goal)); err != nil {
		return Run{}, 0, err
	}
