...
func (s *PostgresStore) ListSourceSnapshots(ctx context.Context, sessionID string) ([]SourceSnapshotView, error) {
	...
	for rows.Next() {
		...
		if err = rows.Scan(
			...
			&item.Publisher, &item.SourceClass, &item.IndependenceKey,
...
func (s *PostgresStore) ListClaims(ctx context.Context, sessionID string) ([]Claim, error) {
	...
	for rows.Next() {
		var item Claim
		if err = rows.Scan(
			&item.ID, &item.ProducedByTaskID, &item.ClientKey, &item.Text,
	...
	evidenceRows, err := s.pool.Query(ctx, `
		SELECT claim_id::text, observation_id::text, relation, strength,
	...
	for evidenceRows.Next() {
		...
		if err = evidenceRows.Scan(
			&claimID, &item.ObservationID, &item.Relation, &item.Strength,
