...
func materializeSources(ctx context.Context, tx pgx.Tx, state acceptedResultState, result ResultEnvelope) (map[string]string, int, error) {
	...
	for _, source := range result.Sources {
		...
		if errors.Is(err, pgx.ErrNoRows) {
			...
			if err = ensureDomainArtifactPassportWithAccessTx(ctx, tx, ArtifactKindLegacySource, state.workspaceID, state.run.SessionID, legacySourceID, time.Now(), nil, nil, state.outputAccess); err != nil {
				return nil, 0, err
			}
