...
func buildTaskPromptV6(run Run, task Task, attempt Attempt, snapshot RunSnapshot, members []FleetMember) string {
	...
	case "research_evidence_v6":
		b.WriteString("5. Execute only Search Plans frozen in this Task. Return each Query Execution with adapter, cursors, timing, outcome, cost and safety facts; return every Source Candidate with canonical URL, content hash, independence family, and an explicit plan-based Screening Decision. `include` does not claim the URL is already evidence. When acceptance_criteria.mode is dispute_review, use only the isolated Manifest inputs for the assigned Position, do not reconstruct another Position from summaries, and do not submit a Dispute status transition.\n")
	case "research_integration_v6":
		b.WriteString("5. The frozen acceptance criteria contains `integration_round_id`, `input_event_sequence`, `input_state_hash`, exact `input_artifact_ids`, and exact `input_version_ids`. Use that round ID verbatim and compare only those immutable versions. Return at least one Integration Contribution; create Insights only from two or more accepted Claim/Insight inputs with genuine semantic gain. Each Dispute position object must contain `author_agent_id`, `statement`, `scope`, `claim_refs`, `evidence_refs`, and `conflict_basis`; the author must own an accepted round input. A deterministic basis declares logical/source_interpretation/version/unit and includes normalized entity, metric, time-window, scope, proposition, polarity, unit, version, Source Snapshot, and citation-meaning fields for server recomputation. A scope/method/semantic Agent candidate instead supplies a concrete reason and a null fact. Record status changes, material conflicts, and targeted follow-up work explicitly.\n")
