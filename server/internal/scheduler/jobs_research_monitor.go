...
const JobNameResearchMonitor = "research_monitor_cycle"

func ResearchMonitorJob(h *handler.Handler) JobSpec {
	return JobSpec{
		Name:              JobNameResearchMonitor,
		Cadence:           time.Minute,
		ScheduleDelay:     0,
		...
		Handler: func(ctx context.Context, _ HandlerInput) (HandlerResult, error) {
			...
			processed, err := h.ProcessDueResearchMonitors(ctx, 50)
			if err != nil {
