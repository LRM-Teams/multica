...
| Fact | Source |
| --- | --- |
| `dm` command + `--message` / `--message-stdin` / `--message-file` flags | `server/cmd/multica/cmd_dm.go` |
| Registered on the root command | `server/cmd/multica/main.go` (`rootCmd.AddCommand(dmCmd)`) |
| Sends `{ "content": ... }` to `POST /api/chat/agent-dm` | `server/cmd/multica/cmd_dm.go` (`runDM`) |
...
| Recipient = task initiator, else the agent's owner; must be a human member | `server/internal/handler/chat_agent_dm.go` (`resolveDMRecipient`) |
| No human recipient → 400 telling the agent to use a channel + @-mention | `server/internal/handler/chat_agent_dm.go` (`resolveDMRecipient` / handler) |
