...
| A capable busy runtime receives only content-free Pending metadata; Pi steers through RPC, Codex uses fenced `turn/steer`, Grok uses native interjection, Claude stream-json writes only at observed safe boundaries, Cursor uses its native concurrent ACP follow-up, and OpenCode durably admits a queued Session prompt for its next safe drain boundary. Unsupported or unsafe writes retain Pending. A content-free Notice routes through `multica inbox check`; `multica message check` then drains paginated concrete Messages through the current-turn Credential Proxy until empty, with a 50-round safety cap and per-batch coverage commits before output | `server/pkg/agent/pi_rpc.go`, `codex_resident.go`, `grok_acp.go`, `claude_resident.go`, `cursor_acp.go`, and `opencode_serve.go`, `AcceptPendingNotice`; `server/internal/daemon/agent_runtime_pool.go`, `deliverBusyInboxNotice`; `server/internal/daemon/credential_proxy_http.go`, `credentialProxyMessageCheckHandler`; `server/internal/daemon/message_coordinator.go`, `Check`; `server/cmd/multica/cmd_inbox.go`, `runInboxCheck`; `server/cmd/multica/cmd_message.go`, `runAgentMessageCheck` |
| Channel/DM send, read, search, resolve, react, and resident turn-capture paths use two launch-scoped credentials: an `mpt_` token authenticates the Agent to the machine-local Proxy, while the Proxy holds the server-issued `sk_agent_` credential. Neither credential has an idle-time refresh boundary; both are revoked when the launch ends | `server/internal/daemon/agent_proxy_credential.go`; `agent_credential_cache.go`; `message_send_proxy.go`; `agent_credential_api_proxy.go`; `message_runtime.go`; `server/internal/handler/agent_credential.go` |
