...
- **写汇报** — the human owns collect scope. There is one current plan
  ...
  `context_note_page_id`. When they ask 写汇报, the platform opens the
  plan card. If the plan board is `status: none`, call
  `multica notes period-brief plan --chat-session-id <id>` **this turn**
  with only the session id (page id optional) **before any bubble text**.
  The tool must return a plan object, not `null`. Do not PUT window,
