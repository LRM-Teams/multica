...
| Speech / FAB 写汇报: soft-confirm first; 「是」 opens plan card; 开始采集 walks every selected computer then synthesizes from this run only | `tryHandlePeriodBriefPlanAsk`; `looksLikePeriodBriefPlanAsk`; `awaiting_intent` / `source_ask`; `POST /api/notes/period-briefs` (`CreateNotePeriodBrief`); Highlights decide harvest (`periodBriefPackHasHarvest`); no ready pack after retry blocks a new official brief (`periodBriefOfficialBriefBlocked`); partial ready still synthesizes |
