...
| Continuous Integration trigger contract (G, durable round wiring pending) | `server/internal/researchrun/integration_trigger.go`, `integration_trigger_test.go`; versioned trigger policy, fixed accepted Result Artifact inputs, multi-Agent threshold, budget/active-round gates and deterministic idempotent Round Key |
