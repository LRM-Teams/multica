...
| Retrieval failure recovery contract (F, Query Execution wiring pending) | `server/internal/researchrun/retrieval_recovery.go`, `retrieval_recovery_test.go`; failure-licensed retry/cursor/query/Adapter/scope/candidate actions, frozen non-target request fields, bounded attempts and stable recovery key |
