...
| Artifact passport (D): policy, manifest freeze, dispatch outbox binding, result artifact lineage | migrations `318`–`330`, `346`–`349`; `server/internal/researchrun/artifact*.go`, `artifact_context.go`, `artifact_manifest.go`, `artifact_dispatch_prompt.go`, `artifact_representation.go`, `artifact_taint.go`, `postgres_artifact.go`, `postgres_tasks.go`, `postgres_result.go`, `postgres_evidence.go`; Source Snapshot/Observation/Claim reads decode frozen bytes; Quality Gate/Citation Audit use `evaluation` purpose with separate normal + evaluation-private grants and only their assigned grader Prompt receives frozen Stage Evaluation context; manifest hash binds scope/purpose/watermarks/representations; dispatch/replay use frozen principal header; accepted Result and materialized outputs inherit the most sensitive manifest access level, and unsafe reuse fails closed; Agent snapshot requires assigned `attempt_id`; `packages/core/research/schemas.ts` parses bounded `attempt_context` |
