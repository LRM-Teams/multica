...
| Latest cross-Agent Integration Context selector (G, persistence/Manifest wiring pending) | `server/internal/researchrun/integration_context.go`, `integration_context_test.go`; exact Goal/Plan and Event/State watermark, latest-only Artifact Version, access/evaluation compartment omission, contributor identity and stable audit fingerprint |
