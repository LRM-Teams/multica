...
| Artifact passport (D): policy, manifest freeze, dispatch outbox binding, result artifact lineage | migrations `318`–`330`; `server/internal/researchrun/artifact*.go`, `artifact_context.go`, `artifact_manifest.go`, `artifact_dispatch_prompt.go`, `postgres_artifact.go`, `postgres_tasks.go`, `postgres_result.go`, `postgres_evidence.go`; dispatch CAS on eligibility + content hash; execution skips caller prompt when D-enabled; `replayDispatchPromptFromManifest`; session snapshot accepts `attempt_id`; `packages/core/research/schemas.ts` parses bounded `attempt_context` |
