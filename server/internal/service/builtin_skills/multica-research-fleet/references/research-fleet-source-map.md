...
| Search/Corpus lineage (F) | migration `353_research_search_corpus_lineage`; `server/internal/researchrun/search_lineage.go`, `postgres_search_lineage.go`, `postgres_result.go`; Search Plan → Query Execution → Source Candidate → Screening Decision is an atomic idempotent passport-backed chain, while ordinary Result sources are explicitly `agent_direct_evidence` and screened retrieval must cite an accepted matching Decision |
