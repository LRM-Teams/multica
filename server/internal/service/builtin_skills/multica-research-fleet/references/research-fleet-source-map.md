...
| Capability-gap action Adapter (J, mutation wiring pending) | `server/internal/researchrun/capability_gap_action.go`; stale-gap rejection, activate/configure/formation/block ordering, least-change existing-Agent choice, Contract/capacity/budget/tool/source fences and stable action identity; route output, Agent mutation, Formation authorization, Decision/Event and state readback are not yet wired |
