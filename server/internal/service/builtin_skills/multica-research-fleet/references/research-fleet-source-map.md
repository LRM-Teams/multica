...
| Search/Corpus URL identity safety (F) | `server/internal/researchrun/result.go` (`CanonicalURL`) and `result_test.go`; canonical source identities reject embedded URL credentials before persistence, deduplication, or future Retrieval Adapter use |
