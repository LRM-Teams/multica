...
| Source ingestion admission (F, persistence wiring pending) | `server/internal/researchrun/source_ingestion.go`; five mutually exclusive ingestion kinds, exact accepted Search/Screening lineage for retrieval, no fabricated Search lineage for direct evidence, canonical URL/credential/time/hash validation, deterministic audit fingerprint; durable schema and Result materialization are not yet wired |
