...
| Integration merge decision (G, persistence wiring pending) | `server/internal/researchrun/integration_merge.go`; symmetric Claim/Question/Hypothesis candidate evaluation, server-signal thresholds, stable rejection reasons and audit fingerprint; similarity Adapter, durable candidate rows, merge transaction and Event are not yet wired |
