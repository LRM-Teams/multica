...
| Inbox dispatch idempotency, frozen Manifest ID/hash context binding, and event projection | `server/internal/researchrun/dispatch_request.go`, `server/internal/researchrun/artifact_dispatch_prompt.go`, `server/internal/handler/research_run_adapter.go` |
...
| WS events | `server/pkg/protocol/research_events.go`; `server/internal/handler/research_run_adapter.go` keeps legacy graph broadcasts and emits run-scoped `research_projection_v6:delta` envelopes from committed Event sequence using the same Node/Edge identity mapper as the V6 Snapshot route |
