...
| Monitoring Query Execution diff admission (L, persistence/wiring pending) | `server/internal/researchrun/monitoring_diff.go`; a Cycle must execute every pinned Search Plan canonical query key + hash, bind current result artifacts to one Query Execution, prove version/hash-consistent added/removed/modified/unchanged diffs, and derive materiality without an Agent-supplied aggregate score; durable Monitor/Cycle/Execution/diff storage and scheduler wiring are not implemented yet |
