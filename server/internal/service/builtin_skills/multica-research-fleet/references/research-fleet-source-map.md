...
| Artifact passport (D): policy, manifest freeze, dispatch outbox binding, result artifact lineage | migrations `318`–`330`; `server/internal/researchrun/artifact*.go`, `artifact_context.go`, `artifact_manifest.go`, `postgres_artifact.go`, `postgres_tasks.go`, `postgres_result.go`, `postgres_evidence.go`; D-enabled session snapshot accepts `attempt_id` query param for manifest-filtered reads |
