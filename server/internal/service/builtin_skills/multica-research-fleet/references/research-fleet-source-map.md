...
| Evaluation Result identity fence (M2c) | `server/internal/researchrun/evaluation_subject_policy.go`, `result_acceptance.go`, `postgres_result.go`; frozen Contract binds subject hash + Document allowlist, and both preflight plus locked transaction reject forged Source/Observation evaluation identities while ordinary Runs remain unchanged |
