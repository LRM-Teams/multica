...
| Artifact passport (D): policy, manifest freeze, dispatch outbox binding, result artifact lineage | migrations `318`–`330`; `server/internal/researchrun/artifact*.go`, `artifact_context.go`, `artifact_manifest.go`, `artifact_dispatch_prompt.go`, `postgres_artifact.go`, `postgres_tasks.go`, `postgres_result.go`, `postgres_evidence.go`; manifest hash binds workspace/session, Attempt/Task, purpose, policy/state watermarks, ordered version IDs and representation hashes; dispatch CAS on eligibility + content hash; execution skips caller prompt when D-enabled; `replayDispatchPromptFromManifest`; Agent session snapshot requires an assigned `attempt_id` and omits unfiltered legacy presentation rows; `packages/core/research/schemas.ts` parses bounded `attempt_context` |
