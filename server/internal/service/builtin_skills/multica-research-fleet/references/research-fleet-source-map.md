...
| Strict structured result envelope, same-session structured Report source admission and migration diagnostics, pinned V1–V4 behavior, V4+ Method/evidence-standard and executable delivery-path contract, V5 structured evaluation defects, and Claim-level evidence-fitness Gate | migration `351_research_report_relationship_diagnostics`; `server/internal/researchrun/result.go`, `result_v2.go`, `result_v3.go`, `result_v4.go`, `result_v5.go`, `result_test.go`, `result_v5_test.go`, `postgres_result.go`, `postgres_gate.go`, `postgres_gate_v4.go` |
