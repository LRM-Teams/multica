...
| Dispute delivery gate (H, persistence wiring pending) | `server/internal/researchrun/dispute_delivery_gate.go`; aggregate blocking/open dispute checks, human-gate receipt, exact current-report disclosure for conditional/irreducible outcomes, ordered findings and stable audit fingerprint; canonical reads and deliveryGateModule routing are not yet wired |
