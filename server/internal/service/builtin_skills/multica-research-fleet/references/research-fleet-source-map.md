...
| Exploration candidate generation (I, persistence wiring pending) | `server/internal/researchrun/exploration_candidate_generation.go`; versioned canonical-need matrix, bounded score/cost facts, stable candidate identity, hard-constraint preservation, evidence/counterevidence expansion and divergence probes; signal queries, Candidate Set persistence, portfolio adapter, Decision/Event and Task creation are not yet wired |
