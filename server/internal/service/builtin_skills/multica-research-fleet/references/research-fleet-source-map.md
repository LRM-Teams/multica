...
| Artifact passport (D): policy, manifest freeze, dispatch outbox binding, result artifact lineage | migrations `318`–`330`, `346`; `server/internal/researchrun/artifact*.go`, `artifact_context.go`, `artifact_manifest.go`, `artifact_dispatch_prompt.go`, `artifact_taint.go`, `postgres_artifact.go`, `postgres_tasks.go`, `postgres_result.go`, `postgres_evidence.go`; manifest hash binds authorization scope and ordered representations; accepted Result and materialized outputs inherit the most sensitive manifest access level, and unsafe reuse fails closed; Agent session snapshot requires an assigned `attempt_id` and omits unfiltered legacy presentation rows; `packages/core/research/schemas.ts` parses bounded `attempt_context` |
