...
| Reporter input admission (K, dispatch wiring pending) | `server/internal/researchrun/reporter_input.go`; one Reporter Attempt is fenced to the latest completed Integration Snapshot, exact canonical scope/version/watermarks/state hash, exact frozen input-set hash, and accepted verified task-readable non-evaluation-private artifact versions; canonical reads, persisted dispatch-manifest binding, and Report Revision materialization are not wired yet |
