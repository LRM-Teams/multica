...
- `server/internal/handler/machine_upgrade_handler.go` owns the canonical daemon Machine Upgrade lifecycle, the exact predecessor-to-candidate Computer generation takeover CAS, and its legacy runtime response projection.
- `server/internal/daemon/machine_upgrade.go`, `machine_upgrade_takeover.go`, and `machine_upgrade_recovery.go` own the durable local Machine Upgrade journal, local-first detached candidate proof, exact rollback, and fail-closed startup recovery. A detached candidate cannot start server preflight before its takeover CAS. Only a proven later Active generation may supersede a retained `candidate_ready` marker.
