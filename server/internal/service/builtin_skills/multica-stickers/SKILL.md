...
IS the reply. Return structured message parts JSON, not a text token:

```json
{"parts":[{"type":"sticker","sticker_id":"got-it"}]}
```
...
Put the id into structured parts JSON. That's it: no command, no file read.
...
that id in structured parts JSON. Do not paste the token as your final message.
...
- **Don't** output `:sticker:<id>:` as message content. The formal protocol is
  `parts`, for example `{"parts":[{"type":"sticker","sticker_id":"hi"}]}`.
