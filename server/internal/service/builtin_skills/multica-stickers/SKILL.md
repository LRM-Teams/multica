...
IS the reply. Return a `send_channel_message` action with structured message
parts JSON, not a text token:

```json
{"action":"send_channel_message","parts":[{"type":"sticker","sticker_id":"got-it"}]}
```
...
Put the id into `parts[]` inside the `send_channel_message` action. That's it:
...
that id in structured parts JSON inside `send_channel_message`. Do not paste
...
- **Don't** output `:sticker:<id>:` as message content. The formal protocol is
  `send_channel_message` with `parts`, for example
  `{"action":"send_channel_message","parts":[{"type":"sticker","sticker_id":"hi"}]}`.
