...
| CLI passes repeatable `--attachment-id` in order as `attachment_ids`; Agent chat uploads require a target-bound Upload Session and the Server verifies its completed object before the ID may be sent | `server/cmd/multica/cmd_attachment.go` (`runAgentAttachmentUploadSession`), `server/internal/handler/agent_attachment_upload_session.go` (`CompleteAgentAttachmentUploadSession`, `linkVerifiedAgentUploadAttachmentsToChannelMessage`) |
