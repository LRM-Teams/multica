...
| Active Goal top-level group Messages execute manager-first; standard Goal state changes are durably coalesced into one manager reconciliation Run while worker Runs remain Issue-owned | `server/internal/handler/channel_message_delivery.go`; `server/internal/handler/goal_controller.go`; `server/internal/scheduler/jobs_goal_controller.go`; migration `449_goal_controller_events`; guarded by `TestActiveGoalTopLevelMessagesExecuteManagerFirst` and `TestGoalControllerJobContract` |
| Atomic Issue decomposition requires per-node acceptance criteria and inherits the parent's `channel_goal_id` / `goal_required` scope | `server/internal/workgraph/decompose.go`, `normalizeDecompose` and `DecomposeIssue`; guarded by `TestNormalizeDecomposeRequiresAcceptanceCriteria` and `TestDecomposeIssueCreatesParallelRootsAndParkedJoin` |
