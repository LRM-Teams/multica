...
func TestResumedTaskContinuesItsOwnRecordedSession(t *testing.T) {
	...
	event := db.AgentInboxEvent{
		Status:    "pending",
		RuntimeID: rt,
		SessionID: text("sess-mid-flight"),
		WorkDir:   text("/work/proj"),
	}
...
func TestSessionIsNotHandedToADifferentRuntime(t *testing.T) {
	...
	source := db.AgentInboxEvent{
		...
		SessionID: text("sess-src"),
		WorkDir:   text("/work/src"),
	}
	...
	for _, status := range []string{"acked", "failed", "suppressed"} {
		t.Run(status, func(t *testing.T) {
			event := db.AgentInboxEvent{
				Status:    status,
				RuntimeID: rt,
				SessionID: text("sess-maybe-poisoned"),
			}
...
func TestWorkDirIsReturnedWithoutASession(t *testing.T) {
	rt := runtimeUUID(t, 1)
	event := db.AgentInboxEvent{Status: "draining", RuntimeID: rt, WorkDir: text("/work/proj")}
...
func TestNoSessionWithoutARuntime(t *testing.T) {
	event := db.AgentInboxEvent{Status: "pending", SessionID: text("sess-1")}
