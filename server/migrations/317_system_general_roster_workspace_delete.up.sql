...
-- Deployment/apply note: apply before the server handles Workspace deletion.
-- The general roster remains protected from direct writes. A Workspace cascade
-- is different: PostgreSQL has already removed the owning Workspace row, so
-- its generated channel_member deletes must be allowed to continue.
...
BEGIN
  ...
  IF TG_OP = 'DELETE'
     AND old_is_general
     AND NOT EXISTS (
       SELECT 1 FROM workspace WHERE id = target_workspace_id
     ) THEN
    RETURN OLD;
  END IF;
