-- Project trusted attachment state after the existing owner-event trigger.
...
CREATE TRIGGER z_agent_attachment_projection_trigger
AFTER INSERT OR UPDATE OR DELETE ON agent
FOR EACH ROW EXECUTE FUNCTION project_agent_attachment_projection();
