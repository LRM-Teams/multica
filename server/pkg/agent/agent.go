...
type ResidentMessageAcceptance struct {
	...
	// Capture carries the daemon-owned, redacted native-provider observation
	// for the settled turn. Backends that cannot observe this boundary leave it
	// nil; callers must then report a capture gap rather than fabricating one.
	Capture <-chan ResidentTurnCapture
}

// ResidentTurnCapture is a complete native-provider observation for one
// resident logical turn. It is intentionally transport-neutral: the daemon
// adds runtime credential identity before uploading it to the server.
type ResidentTurnCapture struct {
	RunID           string
	RunAgentID      string
	PiSessionID     string
	...
	ProviderCalls   []ResidentProviderCallCapture
	PayloadHash     string
}

// ResidentProviderCallCapture preserves only redacted, final provider input
// and typed final assistant output. It deliberately contains no credentials or
// server-authoritative action/consumption records.
type ResidentProviderCallCapture struct {
	CallID                string
	CallOrdinal           int64
	Provider              string
	Model                 string
	APIKind               string
	RawProviderRequest    json.RawMessage
	FinalAssistantMessage json.RawMessage
	Status                string
	StopReason            string
...
type ExecOptions struct {
	...
	// piCaptureExtension is generated by the resident Pi backend for a bound
