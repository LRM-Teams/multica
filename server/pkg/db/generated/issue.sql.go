...
const createIssueWithMetadata = `-- name: CreateIssueWithMetadata :one
INSERT INTO issue (
    workspace_id, title, description, status, priority,
    assignee_type, assignee_id, creator_type, creator_id,
...
func (q *Queries) CreateIssueWithMetadata(ctx context.Context, arg CreateIssueWithMetadataParams) (Issue, error) {
	row := q.db.QueryRow(ctx, createIssueWithMetadata,
		arg.WorkspaceID,
		arg.Title,
		arg.Description,
		arg.Status,
...
SELECT id, workspace_id, title, description, status, priority, assignee_type, assignee_id, creator_type, creator_id, parent_issue_id, acceptance_criteria, context_refs, position, due_date, created_at, updated_at, number, project_id, origin_type, origin_id, first_executed_at, start_date, metadata, forked_from_issue_id, forked_at_seq, forked_at_task_id, channel_goal_id, goal_required, execution_revision, execution_attempt_sequence, goal_version_at_creation FROM issue
WHERE workspace_id = $1
...
func (q *Queries) ListIssuesByProject(ctx context.Context, arg ListIssuesByProjectParams) ([]Issue, error) {
	rows, err := q.db.Query(ctx, listIssuesByProject, arg.ProjectID, arg.WorkspaceID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
